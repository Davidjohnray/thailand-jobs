      ...
      <div style={{ background: 'linear-gradient(135deg, #0891b2, #06b6d4)', padding: '52px 24px', color: 'white' }}>
        <div style={{ maxWidth: '760px', margin: '0 auto' }}>
          ...
          <div style={{ marginTop: '20px' }}>
            ...
            <h1 style={{ fontSize: '36px', fontWeight: 'bold', margin: '0 0 10px' }}>Word Association Chain</h1>
            <p style={{ opacity: 0.9, fontSize: '16px', margin: '0 0 16px' }}>Each student says a word connected to the previous one. No hesitation or repetition allowed.</p>
      ...
      <div style={{ maxWidth: '760px', margin: '0 auto', padding: '40px 24px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '24px' }}>
          <div style={{ background: 'white', borderRadius: '12px', padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
            ...
            {['any topic vocabulary', 'free association', 'themed categories', 'lesson words'].map(v => <div key={v} style={{ color: '#555', fontSize: '13px', marginBottom: '4px' }}>• {v}</div>)}
          </div>
          <div style={{ background: 'white', borderRadius: '12px', padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
            <h3 style={{ fontWeight: 'bold', color: '#1a1a2e', margin: '0 0 12px', fontSize: '15px' }}>🛠 Materials</h3>
            {['No materials needed'].map(m => <div key={m} style={{ color: '#555', fontSize: '13px', marginBottom: '4px' }}>• {m}</div>)}
          </div>
        </div>
        <div style={{ background: 'white', borderRadius: '12px', padding: '28px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: '20px' }}>
          ...
          <ol style={{ margin: 0, paddingLeft: '20px' }}>
            {[
              ...
              'Teacher says a starting word — for example: "SCHOOL"',
              'The next child must say a word connected to the previous word.',
              'For example: school → teacher → class → student → book → library',
              'Each connection must make sense — the child must explain if challenged.',
              'Rules: no hesitating for more than 3 seconds, no repeating a word already said.',
              'Anyone who hesitates or repeats a word sits out for one round.',
        ...
        <div style={{ background: '#e0f7fa', borderRadius: '12px', padding: '28px', marginBottom: '20px', border: '1px solid rgba(8,145,178,0.2)' }}>
          ...
          {[
            'Write the chain on the board so everyone can see the connections.',
            'Accept any reasonable connection — the game encourages creative thinking.',
            'Use a topic-specific starting word to keep the game on the lesson theme.',
            'This game is excellent for expanding vocabulary beyond the lesson wordlist.',
        ...
        <div style={{ background: 'white', borderRadius: '12px', padding: '28px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: '32px' }}>
          ...
          {[
            'Restrict to one category — all words must be food, or all must be adjectives.',
            'Words must start with the last letter of the previous word.',
            'Play in teams — teams alternate and the team that breaks the chain loses a point.',
