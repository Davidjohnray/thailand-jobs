...
export default function HotPotatoQuizGame() {
  return (
      ...
      <div style={{ background: 'linear-gradient(135deg, #0891b2, #06b6d4)', padding: '52px 24px', color: 'white' }}>
        <div style={{ maxWidth: '760px', margin: '0 auto' }}>
          ...
          <div style={{ marginTop: '20px' }}>
            ...
            <h1 style={{ fontSize: '36px', fontWeight: 'bold', margin: '0 0 10px' }}>Hot Potato Quiz</h1>
            <p style={{ opacity: 0.9, fontSize: '16px', margin: '0 0 16px' }}>Pass an object around — whoever is holding it when the timer stops must answer a question.</p>
      ...
      <div style={{ maxWidth: '760px', margin: '0 auto', padding: '40px 24px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '24px' }}>
          <div style={{ background: 'white', borderRadius: '12px', padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
            ...
            {['any lesson vocabulary', 'spelling', 'definitions', 'grammar questions', 'comprehension'].map(v => <div key={v} style={{ color: '#555', fontSize: '13px', marginBottom: '4px' }}>• {v}</div>)}
          </div>
          <div style={{ background: 'white', borderRadius: '12px', padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
            <h3 style={{ fontWeight: 'bold', color: '#1a1a2e', margin: '0 0 12px', fontSize: '15px' }}>🛠 Materials</h3>
            {['A small object to pass — eraser, ball or toy', 'Timer or phone', 'Question list'].map(m => <div key={m} style={{ color: '#555', fontSize: '13px', marginBottom: '4px' }}>• {m}</div>)}
          </div>
        </div>
        <div style={{ background: 'white', borderRadius: '12px', padding: '28px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: '20px' }}>
          ...
          <ol style={{ margin: 0, paddingLeft: '20px' }}>
            {[
              ...
              'Start the timer — set it for a random time between 10-30 seconds.',
              'Children pass the object around the circle as fast as possible.',
              'When the timer goes off whoever is holding the object must answer a question.',
              'Teacher asks the question — "Spell the word BEAUTIFUL" or "What is the past tense of RUN?"',
              'If they answer correctly they earn a point for their team.',
              'If they cannot answer they sit out for one round.',
        ...
        <div style={{ background: '#e0f7fa', borderRadius: '12px', padding: '28px', marginBottom: '20px', border: '1px solid rgba(8,145,178,0.2)' }}>
          ...
          {[
            'Use a random timer so children cannot predict when it will stop.',
            'Prepare questions of varying difficulty to suit different ability levels.',
            'The suspense of not knowing when the timer stops creates genuine excitement.',
            'This works brilliantly as a lesson review in the last 10 minutes of class.',
        ...
        <div style={{ background: 'white', borderRadius: '12px', padding: '28px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: '32px' }}>
          ...
          {[
            'Play music instead of a timer — stop the music randomly.',
            'The catcher can choose their own question from a pile of cards.',
            'Play in teams — each team passes their own object simultaneously.',
