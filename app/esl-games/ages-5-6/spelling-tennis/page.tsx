...
export default function SpellingTennisGame() {
  return (
      ...
      <div style={{ background: 'linear-gradient(135deg, #7C3AED, #9f67f5)', padding: '52px 24px', color: 'white' }}>
        <div style={{ maxWidth: '760px', margin: '0 auto' }}>
          ...
          <div style={{ marginTop: '20px' }}>
            ...
            <h1 style={{ fontSize: '36px', fontWeight: 'bold', margin: '0 0 10px' }}>Spelling Tennis</h1>
            <p style={{ opacity: 0.9, fontSize: '16px', margin: '0 0 16px' }}>Two teams take turns spelling a word one letter at a time like a tennis rally.</p>
      ...
      <div style={{ maxWidth: '760px', margin: '0 auto', padding: '40px 24px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '24px' }}>
          <div style={{ background: 'white', borderRadius: '12px', padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
            ...
            {['spelling practice', 'any lesson vocabulary', 'high frequency words', 'topic words'].map(v => <div key={v} style={{ color: '#555', fontSize: '13px', marginBottom: '4px' }}>• {v}</div>)}
          </div>
          <div style={{ background: 'white', borderRadius: '12px', padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
            <h3 style={{ fontWeight: 'bold', color: '#1a1a2e', margin: '0 0 12px', fontSize: '15px' }}>🛠 Materials</h3>
            {['Whiteboard to write letters as they are called', 'Vocabulary word list'].map(m => <div key={m} style={{ color: '#555', fontSize: '13px', marginBottom: '4px' }}>• {m}</div>)}
          </div>
        </div>
        <div style={{ background: 'white', borderRadius: '12px', padding: '28px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: '20px' }}>
          ...
          <ol style={{ margin: 0, paddingLeft: '20px' }}>
            {[
              'Divide the class into two teams sitting on opposite sides.',
              'Teacher announces a word to spell — "CAT!"',
              'Team A starts by calling the first letter — "C!"',
              'Team B calls the second letter — "A!"',
              'Team A calls the third letter — "T!"',
              'The team that calls the final letter correctly wins the point.',
              'If a team calls the wrong letter they lose the point.',
              'Write letters on the board as they are called so everyone can follow.',
        ...
        <div style={{ background: '#fdf4ff', borderRadius: '12px', padding: '28px', marginBottom: '20px', border: '1px solid rgba(124,58,237,0.2)' }}>
          ...
          {[
            'Start with short 3-letter words and gradually increase difficulty.',
            'Write each letter on the board as it is said — keeps everyone engaged.',
            'This game makes spelling practice genuinely exciting.',
            'Use only words from the current lesson for maximum learning value.',
        ...
        <div style={{ background: 'white', borderRadius: '12px', padding: '28px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: '32px' }}>
          ...
          {[
            'Individual tennis — two children face off instead of two teams.',
            'After spelling correctly the team must also say the word in a sentence.',
            'Backwards spelling — teams spell the word in reverse order for extra challenge.',
