...
export default function ActionRaceGame() {
  return (
      ...
      <div style={{ background: 'linear-gradient(135deg, #7C3AED, #9f67f5)', padding: '52px 24px', color: 'white' }}>
        <div style={{ maxWidth: '760px', margin: '0 auto' }}>
          ...
          <div style={{ marginTop: '20px' }}>
            ...
            <h1 style={{ fontSize: '36px', fontWeight: 'bold', margin: '0 0 10px' }}>Action Race</h1>
            <p style={{ opacity: 0.9, fontSize: '16px', margin: '0 0 16px' }}>Teacher calls an action word — first child to do it correctly wins a point.</p>
      ...
      <div style={{ maxWidth: '760px', margin: '0 auto', padding: '40px 24px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '24px' }}>
          <div style={{ background: 'white', borderRadius: '12px', padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
            ...
            {['action verbs', 'jump', 'clap', 'spin', 'wave', 'stomp', 'hop', 'crawl'].map(v => <div key={v} style={{ color: '#555', fontSize: '13px', marginBottom: '4px' }}>• {v}</div>)}
          </div>
          <div style={{ background: 'white', borderRadius: '12px', padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
            <h3 style={{ fontWeight: 'bold', color: '#1a1a2e', margin: '0 0 12px', fontSize: '15px' }}>🛠 Materials</h3>
            {['No materials needed', 'Optional: action word cards'].map(m => <div key={m} style={{ color: '#555', fontSize: '13px', marginBottom: '4px' }}>• {m}</div>)}
          </div>
        </div>
        <div style={{ background: 'white', borderRadius: '12px', padding: '28px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: '20px' }}>
          ...
          <ol style={{ margin: 0, paddingLeft: '20px' }}>
            {[
              'Children stand up with space to move.',
              'Teacher calls an action word — "JUMP!"',
              'All children perform the action.',
              'The first child to do the action correctly earns a point.',
              'Keep a running score on the board.',
              'Call actions faster and faster to increase excitement.',
              'Mix in a trick — call one action but show a different one to see who follows the voice.',
              'Child with the most points at the end wins.',
        ...
        <div style={{ background: '#fdf4ff', borderRadius: '12px', padding: '28px', marginBottom: '20px', border: '1px solid rgba(124,58,237,0.2)' }}>
          ...
          {[
            'This is the perfect warm-up game for any lesson — gets energy up immediately.',
            'Combine with body parts — "Clap your HANDS! Stomp your FEET!"',
            'Use this to pre-teach action verbs before a reading or listening activity.',
            'Let a child be the caller once they know the vocabulary confidently.',
        ...
        <div style={{ background: 'white', borderRadius: '12px', padding: '28px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: '32px' }}>
          ...
          {[
            'Combine two actions — "Jump AND clap at the same time!"',
            'Play in teams — team with the most correct actions wins.',
            'Use a sequence — children must remember and repeat 3 actions in order.',
