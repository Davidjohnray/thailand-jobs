...
export default function NumberJumpGame() {
  return (
      ...
      <div style={{ background: 'linear-gradient(135deg, #f97316, #fb923c)', padding: '52px 24px', color: 'white' }}>
        <div style={{ maxWidth: '760px', margin: '0 auto' }}>
          ...
          <div style={{ marginTop: '20px' }}>
            ...
            <h1 style={{ fontSize: '36px', fontWeight: 'bold', margin: '0 0 10px' }}>Number Jump</h1>
            <p style={{ opacity: 0.9, fontSize: '16px', margin: '0 0 16px' }}>Numbers on the floor — teacher calls a number and children jump to it.</p>
      ...
      <div style={{ maxWidth: '760px', margin: '0 auto', padding: '40px 24px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '24px' }}>
          <div style={{ background: 'white', borderRadius: '12px', padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
            ...
            {['numbers 1-10', 'number recognition', 'counting', 'more', 'less'].map(v => <div key={v} style={{ color: '#555', fontSize: '13px', marginBottom: '4px' }}>• {v}</div>)}
          </div>
          <div style={{ background: 'white', borderRadius: '12px', padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
            <h3 style={{ fontWeight: 'bold', color: '#1a1a2e', margin: '0 0 12px', fontSize: '15px' }}>🛠 Materials</h3>
            {['Large number cards (1-10) taped to the floor', 'Open floor space'].map(m => <div key={m} style={{ color: '#555', fontSize: '13px', marginBottom: '4px' }}>• {m}</div>)}
          </div>
        </div>
        <div style={{ background: 'white', borderRadius: '12px', padding: '28px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: '20px' }}>
          ...
          <ol style={{ margin: 0, paddingLeft: '20px' }}>
            {[
              'Tape large number cards (1-10) to the floor spread around the room.',
              'Walk children around touching each number and counting aloud together.',
              'Children stand in the middle of the room.',
              'Teacher calls a number — "SEVEN!"',
              'Children race to jump onto the number 7 card.',
              'Last child to reach the correct number sits out for one round.',
              'Continue calling numbers in random order.',
              'For a gentler version, all children jump together with no elimination.',
        ...
        <div style={{ background: '#fff3ed', borderRadius: '12px', padding: '28px', marginBottom: '20px', border: '1px solid rgba(249,115,22,0.2)' }}>
          ...
          {[
            'Start with numbers 1-5 for very young children before extending to 10.',
            'Call the number in a sentence — "Jump to NUMBER FOUR!"',
            'Combine with colours — colour each number card differently.',
            'This game also works brilliantly with letters of the alphabet.',
        ...
        <div style={{ background: 'white', borderRadius: '12px', padding: '28px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: '32px' }}>
          ...
          {[
            'Call a simple sum — "What is 2 plus 3?" — children jump to 5.',
            'Replace numbers with letters — jump to the letter the teacher calls.',
            'Teams compete — first team with all members on the correct number wins.',
