...
export default function AnimalWalkGame() {
  return (
      ...
      <div style={{ background: 'linear-gradient(135deg, #f97316, #fb923c)', padding: '52px 24px', color: 'white' }}>
        <div style={{ maxWidth: '760px', margin: '0 auto' }}>
          ...
          <div style={{ marginTop: '20px' }}>
            ...
            <h1 style={{ fontSize: '36px', fontWeight: 'bold', margin: '0 0 10px' }}>Animal Walk</h1>
            <p style={{ opacity: 0.9, fontSize: '16px', margin: '0 0 16px' }}>Teacher calls an animal — children walk around the room moving like that animal.</p>
      ...
      <div style={{ maxWidth: '760px', margin: '0 auto', padding: '40px 24px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '24px' }}>
          <div style={{ background: 'white', borderRadius: '12px', padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
            ...
            {['elephant', 'snake', 'rabbit', 'crab', 'frog', 'butterfly', 'penguin', 'lion'].map(v => <div key={v} style={{ color: '#555', fontSize: '13px', marginBottom: '4px' }}>• {v}</div>)}
          </div>
          <div style={{ background: 'white', borderRadius: '12px', padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
            <h3 style={{ fontWeight: 'bold', color: '#1a1a2e', margin: '0 0 12px', fontSize: '15px' }}>🛠 Materials</h3>
            {['Open floor space', 'Optional: animal flashcards to show'].map(m => <div key={m} style={{ color: '#555', fontSize: '13px', marginBottom: '4px' }}>• {m}</div>)}
          </div>
        </div>
        <div style={{ background: 'white', borderRadius: '12px', padding: '28px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: '20px' }}>
          ...
          <ol style={{ margin: 0, paddingLeft: '20px' }}>
            {[
              'Clear a space in the classroom for children to move around.',
              'Demonstrate each animal movement before starting — stomp like an elephant, hop like a frog.',
              'Call an animal — "Be a SNAKE!"',
              'Children immediately move around the room like that animal.',
              'After 10-15 seconds call a new animal — "Now be a RABBIT!"',
              'Children switch immediately to the new animal movement.',
              'Speed up the changes to increase excitement.',
              'Let a child be the caller once they know the animals well.',
        ...
        <div style={{ background: '#fff3ed', borderRadius: '12px', padding: '28px', marginBottom: '20px', border: '1px solid rgba(249,115,22,0.2)' }}>
          ...
          {[
            'Always demonstrate the movements first — children copy best when they can see it.',
            'Add the animal sound too — stomp AND trumpet like an elephant.',
            'This is perfect as a brain break between seated activities.',
            'Use animals from the current lesson to reinforce new vocabulary.',
        ...
        <div style={{ background: 'white', borderRadius: '12px', padding: '28px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: '32px' }}>
          ...
          {[
            'Freeze — teacher calls FREEZE and children must stop in the animal position.',
            'Guess the animal — child moves like an animal without it being named and class guesses.',
            'Two animals — children must combine two animals in one movement.',
