...
import { foodQuestions } from '../questions'

const SESSION_KEY = 'premium_games_session'
const TOTAL_TIME = 20
const TEAM_COLORS = ['#ef4444', '#f59e0b', '#22c55e', '#6366f1']
const OPTION_COLORS = ['#ef4444', '#f59e0b', '#22c55e', '#6366f1']
const OPTION_BG = ['#fef2f2', '#fffbeb', '#f0fdf4', '#eef2ff']
...
function FoodTVInner() {
  const qs = foodQuestions
  ...
  if (!authed) return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #fef2f2, #fffbeb)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <Link href="/esl-games/live/premium/food-fruit" style={{ color: '#ef4444', fontWeight: 'bold' }}>← Please log in first</Link>
    </main>
  )

  if (phase === 'setup') return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #fef2f2, #fffbeb, #f0fdf4)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ background: 'white', borderRadius: '32px', padding: '48px 40px', maxWidth: '520px', width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.12)', border: '3px solid #fca5a5' }}>
        <div style={{ fontSize: '64px', marginBottom: '12px' }}>📺</div>
        <h1 style={{ fontSize: '28px', fontWeight: '900', color: '#1e1b4b', marginBottom: '6px' }}>TV Classroom Mode</h1>
        <p style={{ color: '#6b7280', fontSize: '15px', marginBottom: '32px' }}>🍎 Food & Fruit — {qs.length} questions</p>
        <div style={{ marginBottom: '24px' }}>
          <p style={{ fontWeight: '800', color: '#374151', marginBottom: '12px' }}>How many teams?</p>
          <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
            {[2, 3, 4].map(n => (
              <button key={n} onClick={() => setNumTeams(n)}
                style={{ padding: '12px 24px', borderRadius: '12px', border: '3px solid', borderColor: numTeams === n ? '#ef4444' : '#e5e7eb', background: numTeams === n ? '#ef4444' : 'white', color: numTeams === n ? 'white' : '#374151', fontWeight: '800', fontSize: '18px', cursor: 'pointer' }}>
                {n}
              </button>
            ))}
          </div>
        ...
        <button onClick={() => setPhase('game')}
          style={{ width: '100%', padding: '18px', borderRadius: '16px', border: 'none', background: 'linear-gradient(135deg, #ef4444, #f59e0b)', color: 'white', fontWeight: '900', fontSize: '20px', cursor: 'pointer', boxShadow: '0 8px 24px rgba(239,68,68,0.4)' }}>
          🍎 Start the Game!
        </button>
      </div>
    </main>
  ...
  if (phase === 'finished') {
    ...
    return (
      <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #fef2f2, #fffbeb, #f0fdf4)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
        <div style={{ background: 'white', borderRadius: '32px', padding: '48px', maxWidth: '560px', width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.12)', border: '3px solid #fca5a5' }}>
          <div style={{ fontSize: '80px', marginBottom: '16px' }}>🏆</div>
          <h1 style={{ fontSize: '36px', fontWeight: '900', color: '#1e1b4b', marginBottom: '24px' }}>Game Over!</h1>
          {sorted.map((t, i) => (
            <div key={i} style={{ background: i === 0 ? '#fef2f2' : '#f9fafb', borderRadius: '16px', padding: '16px 24px', marginBottom: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: i === 0 ? '3px solid #fca5a5' : '2px solid #e5e7eb' }}>
              <span style={{ fontWeight: '800', fontSize: '20px', color: '#1e1b4b' }}>{['🥇','🥈','🥉','4️⃣'][i]} {t.emoji} {t.name}</span>
          ...
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', marginTop: '24px' }}>
            <button onClick={() => { setCurrent(0); setTeams(DEFAULT_TEAMS); setPhase('setup'); setRevealed(false); setTimerRunning(false) }}
              style={{ background: 'linear-gradient(135deg, #ef4444, #f59e0b)', color: 'white', padding: '14px 28px', borderRadius: '16px', border: 'none', fontWeight: '800', fontSize: '16px', cursor: 'pointer' }}>Play Again!</button>
            <Link href="/esl-games/live/premium/food-fruit" style={{ background: '#f3f4f6', color: '#374151', padding: '14px 24px', borderRadius: '16px', textDecoration: 'none', fontWeight: '700', fontSize: '15px' }}>← Back</Link>
          </div>
        </div>
      </main>
    )
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #fef2f2 0%, #fffbeb 50%, #f0fdf4 100%)', display: 'flex', flexDirection: 'column', padding: '16px', gap: '12px' }}>
      <style>{`@keyframes foodBounce{0%,100%{transform:scale(1) rotate(-4deg)}50%{transform:scale(1.08) rotate(4deg)}}`}</style>
      ...
      <div style={{ background: 'white', borderRadius: '28px', padding: '24px', textAlign: 'center', boxShadow: '0 8px 32px rgba(0,0,0,0.1)', border: '3px solid #fca5a5', flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ fontSize: '120px', animation: 'foodBounce 2s ease-in-out infinite', marginBottom: '12px' }}>{q.emoji}</div>
        <h2 style={{ fontSize: '32px', fontWeight: '900', color: '#1e1b4b', margin: 0 }}>{q.question}</h2>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
        {q.options.map((opt, idx) => {
          ...
          let anim = ''
          if (revealed) {
            if (isCorrect) { bg = '#dcfce7'; border = '3px solid #16a34a'; anim = 'correct 0.4s ease' }
      ...
      <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
        {!revealed ? (
          <button onClick={revealAnswer}
            style={{ background: 'linear-gradient(135deg, #f59e0b, #ef4444)', color: 'white', padding: '14px 36px', borderRadius: '16px', border: 'none', fontWeight: '800', fontSize: '18px', cursor: 'pointer', boxShadow: '0 6px 20px rgba(245,158,11,0.3)' }}>
            👁 Show Answer!
          </button>
        ) : (
          <button onClick={nextQuestion}
            style={{ background: 'linear-gradient(135deg, #ef4444, #f59e0b)', color: 'white', padding: '14px 36px', borderRadius: '16px', border: 'none', fontWeight: '800', fontSize: '18px', cursor: 'pointer', boxShadow: '0 6px 20px rgba(239,68,68,0.4)' }}>
            {current + 1 >= qs.length ? '🏁 See Results!' : 'Next! →'}
          </button>
        )}
        <Link href="/esl-games/live/premium/food-fruit" style={{ background: 'white', color: '#6b7280', padding: '14px 20px', borderRadius: '16px', textDecoration: 'none', fontWeight: '700', fontSize: '15px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>Exit</Link>
      </div>
    </main>
  )
}

export default function FoodTVPage() {
  return (
    <Suspense fallback={<main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #fef2f2, #fffbeb)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontSize: '64px' }}>🍎</div></main>}>
      <FoodTVInner />
    </Suspense>
  )
}
