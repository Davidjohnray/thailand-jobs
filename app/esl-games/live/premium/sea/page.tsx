...
export default function SeaPage() {
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #ecfeff 0%, #e0f2fe 50%, #f0f9ff 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px', position: 'relative', overflow: 'hidden' }}>
      <style>{`@keyframes float{0%,100%{transform:translateY(0)}50%{transform:translateY(-14px)}} @keyframes swim{0%,100%{transform:translateX(-8px) scaleX(1)}50%{transform:translateX(8px) scaleX(1)}}`}</style>
      ...
      <div style={{ background: 'white', borderRadius: '32px', padding: '48px 40px', maxWidth: '480px', width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.12)', border: '3px solid #67e8f9', position: 'relative', zIndex: 1 }}>
        <div style={{ fontSize: '88px', marginBottom: '8px', animation: 'float 3s ease-in-out infinite' }}>🌊</div>
        <h1 style={{ fontSize: '34px', fontWeight: '900', color: '#0c4a6e', margin: '0 0 6px' }}>Sea Creatures!</h1>
        <p style={{ color: '#0ea5e9', fontSize: '16px', marginBottom: '32px', fontWeight: '700' }}>Fish, sharks, dolphins and more! 🌟</p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '32px' }}>
          {MODES.map(m => (
            <button key={m.id} onClick={() => setMode(m.id)}
              style={{ padding: '18px 24px', borderRadius: '16px', border: '3px solid', borderColor: mode === m.id ? m.color : '#e0f2fe', background: mode === m.id ? m.color : '#ecfeff', color: mode === m.id ? 'white' : '#0c4a6e', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '14px', transition: 'all 0.2s', transform: mode === m.id ? 'scale(1.03)' : 'scale(1)', boxShadow: mode === m.id ? `0 6px 20px ${m.color}50` : 'none' }}>
        ...
        <button onClick={() => mode && router.push(`/esl-games/live/premium/sea/${mode}`)} disabled={!mode}
          style={{ width: '100%', padding: '18px', borderRadius: '16px', border: 'none', background: mode ? 'linear-gradient(135deg, #0ea5e9, #06b6d4)' : '#e0f2fe', color: mode ? 'white' : '#67e8f9', fontWeight: '900', fontSize: '20px', cursor: mode ? 'pointer' : 'not-allowed', boxShadow: mode ? '0 8px 28px rgba(14,165,233,0.4)' : 'none', transition: 'all 0.2s' }}>
          {mode ? '🌊 Dive In!' : 'Choose a mode first!'}
        </button>
        <Link href="/esl-games/live/premium" style={{ display: 'block', marginTop: '20px', color: '#67e8f9', textDecoration: 'none', fontSize: '14px', fontWeight: '700' }}>← Back to Premium Games</Link>
      </div>
    </main>
  )
}
