...
const teamColors = ['#2D6BE4', '#E85D26', '#16a34a', '#7C3AED']
...
function TVGame() {
  ...
  // Timer — only runs when running=true and not yet revealed
  useEffect(() => {
      ...
      <div style={{ maxWidth: '600px', margin: '0 auto' }}>
        ...
        <div style={{ background: 'white', borderRadius: '20px', padding: '36px', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', marginTop: '20px' }}>
          <div style={{ textAlign: 'center', marginBottom: '28px' }}>
            ...
            <p style={{ color: '#666', fontSize: '14px', marginTop: '6px' }}>🏅 Sports & Games — {ageGroup}</p>
          </div>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              {[2, 3, 4].map(n => (
                <button key={n} onClick={() => setNumTeams(n)}
                  style={{ flex: 1, padding: '12px', borderRadius: '10px', border: `2px solid ${numTeams === n ? COLOR : '#e5e7eb'}`, background: numTeams === n ? '#eff6ff' : 'white', fontWeight: 'bold', fontSize: '18px', cursor: 'pointer', color: numTeams === n ? COLOR : '#666' }}>
                  {n}
                </button>
              ))}
            </div>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px', flexWrap: 'wrap', gap: '8px' }}>
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            <span style={{ background: 'rgba(255,255,255,0.15)', color: 'white', padding: '6px 14px', borderRadius: '20px', fontSize: '14px', fontWeight: 'bold' }}>🏅 Sports & Games</span>
          ...
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            ...
            <span style={{ fontSize: '20px', fontWeight: 'bold', color: running ? timerColor : 'rgba(255,255,255,0.5)' }}>{timeLeft}s</span>
          </div>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: '16px' }}>
          <div>
            {/* Timer bar — only animates when running */}
            <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: '16px', padding: '8px', marginBottom: '12px' }}>
              <div style={{ background: running ? timerColor : 'rgba(255,255,255,0.15)', height: '6px', borderRadius: '6px', width: running ? `${(timeLeft / 20) * 100}%` : '100%', transition: running ? 'width 1s linear' : 'none' }} />
            </div>

            {/* Question */}
            <div style={{ background: '#eff6ff', borderRadius: '14px', padding: '20px', marginBottom: '12px', borderLeft: `5px solid ${COLOR}` }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              {!running && !revealed && (
                <button onClick={startTimer}
                  style={{ background: '#1a1a2e', color: 'white', padding: '14px 24px', borderRadius: '10px', border: '2px solid rgba(255,255,255,0.2)', fontWeight: 'bold', fontSize: '16px', cursor: 'pointer', flex: 1, letterSpacing: '0.5px' }}>
                  ▶ Start Timer
                </button>
              )}
              {running && !revealed && (
