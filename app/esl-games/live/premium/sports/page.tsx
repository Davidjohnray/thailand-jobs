    ...
    <main style={{ background: '#f8f9fa', minHeight: '100vh', padding: '32px 24px' }}>
      <div style={{ maxWidth: '700px', margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', margin: '24px 0 32px' }}>
          ...
          <h1 style={{ fontSize: '32px', fontWeight: 'bold', color: '#1a1a2e', margin: '0 0 8px' }}>Sports & Games</h1>
          <p style={{ color: '#666', fontSize: '15px', margin: 0 }}>Sports vocabulary, rules & competition English — 20 questions from a pool of 40</p>
        </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
            {ageGroups.map(g => (
              <button key={g.id} onClick={() => setSelectedAge(g.id)}
                style={{ padding: '16px', borderRadius: '12px', border: `2px solid ${selectedAge === g.id ? COLOR : '#e5e7eb'}`, background: selectedAge === g.id ? '#eff6ff' : 'white', cursor: 'pointer', textAlign: 'left' }}>
        ...
        <div style={{ background: 'white', borderRadius: '16px', padding: '28px', boxShadow: '0 4px 20px rgba(0,0,0,0.07)', marginBottom: '24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <button onClick={() => router.push(`/esl-games/live/premium/sports/solo?ageGroup=${selectedAge}`)}
              ...
              <div><div>Solo Mode</div><div style={{ fontWeight: 'normal', fontSize: '13px', opacity: 0.85 }}>Play alone — build your sports vocabulary</div></div>
            </button>
            <button onClick={() => router.push(`/esl-games/live/premium/sports/host?ageGroup=${selectedAge}`)}
            ...
            <button onClick={() => router.push(`/esl-games/live/premium/sports/tv?ageGroup=${selectedAge}`)}
