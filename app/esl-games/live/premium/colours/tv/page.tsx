...
import { coloursQuestions } from '../questions'

const SESSION_KEY = 'premium_games_session'
const TOTAL_TIME = 20
const TEAM_COLORS = ['#f59e0b', '#ec4899', '#3b82f6', '#22c55e']
const OPTION_COLORS = ['#f59e0b', '#ec4899', '#3b82f6', '#22c55e']
const OPTION_BG = ['#fffbeb', '#fdf2f8', '#eff6ff', '#f0fdf4']
...
function ColoursTVInner() {
  const qs = coloursQuestions
  ...
  if (!authed) return <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #fef9c3, #fdf2f8)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Link href="/esl-games/live/premium/colours" style={{ color: '#f59e0b', fontWeight: 'bold' }}>← Please log in first</Link></main>

  if (phase === 'setup') return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #fef9c3, #fdf2f8, #eff6ff)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ background: 'white', borderRadius: '32px', padding: '48px 40px', maxWidth: '520px', width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.12)', border: '3px solid #fde68a' }}>
        ...
        <h1 style={{ fontSize: '28px', fontWeight: '900', color: '#78350f', marginBottom: '6px' }}>TV Classroom Mode</h1>
        <p style={{ color: '#f59e0b', fontSize: '15px', marginBottom: '32px', fontWeight: '700' }}>🌈 Colours — {qs.length} questions</p>
        <div style={{ marginBottom: '24px' }}>
          <p style={{ fontWeight: '800', color: '#374151', marginBottom: '12px' }}>How many teams?</p>
          <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
            {[2,3,4].map(n => <button key={n} onClick={() => setNumTeams(n)} style={{ padding: '12px 24px', borderRadius: '12px', border: '3px solid', borderColor: numTeams === n ? '#f59e0b' : '#fef9c3', background: numTeams === n ? '#f59e0b' : '#fffbeb', color: numTeams === n ? 'white' : '#78350f', fontWeight: '800', fontSize: '18px', cursor: 'pointer' }}>{n}</button>)}
          </div>
        </div>
        ...
        <button onClick={() => setPhase('game')} style={{ width: '100%', padding: '18px', borderRadius: '16px', border: 'none', background: 'linear-gradient(135deg, #f59e0b, #ec4899)', color: 'white', fontWeight: '900', fontSize: '20px', cursor: 'pointer', boxShadow: '0 8px 28px rgba(245,158,11,0.4)' }}>
          🌈 Start the Game!
        </button>
      </div>
    </main>
    ...
    return (
      <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #fef9c3, #fdf2f8, #eff6ff)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
        <div style={{ background: 'white', borderRadius: '32px', padding: '48px', maxWidth: '560px', width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.12)', border: '3px solid #fde68a' }}>
          ...
          <h1 style={{ fontSize: '36px', fontWeight: '900', color: '#78350f', marginBottom: '24px' }}>Game Over!</h1>
          {sorted.map((t, i) => <div key={i} style={{ background: i === 0 ? '#fffbeb' : '#f9fafb', borderRadius: '16px', padding: '16px 24px', marginBottom: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: i === 0 ? '3px solid #fde68a' : '2px solid #e5e7eb' }}><span style={{ fontWeight: '800', fontSize: '20px', color: '#78350f' }}>{['🥇','🥈','🥉','4️⃣'][i]} {t.emoji} {t.name}</span><span style={{ fontWeight: '900', fontSize: '28px', color: TEAM_COLORS[i] }}>{t.score}</span></div>)}
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', marginTop: '24px' }}>
            <button onClick={() => { setCurrent(0); setTeams(DEFAULT_TEAMS); setPhase('setup'); setRevealed(false); setTimerRunning(false) }} style={{ background: 'linear-gradient(135deg, #f59e0b, #ec4899)', color: 'white', padding: '14px 28px', borderRadius: '16px', border: 'none', fontWeight: '800', fontSize: '16px', cursor: 'pointer' }}>Play Again!</button>
            <Link href="/esl-games/live/premium/colours" style={{ background: '#fffbeb', color: '#78350f', padding: '14px 24px', borderRadius: '16px', textDecoration: 'none', fontWeight: '700', fontSize: '15px' }}>← Back</Link>
          </div>
        </div>
      </main>
    )
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #fef9c3 0%, #fdf2f8 50%, #eff6ff 100%)', display: 'flex', flexDirection: 'column', padding: '16px', gap: '12px' }}>
      <style>{`@keyframes rainbowSpin{0%{transform:rotate(-5deg) scale(1)}50%{transform:rotate(5deg) scale(1.1)}100%{transform:rotate(-5deg) scale(1)}}`}</style>
      <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap' }}>
        {teams.slice(0, numTeams).map((team, idx) => (
          <div key={idx} style={{ background: 'white', borderRadius: '20px', padding: '10px 16px', display: 'flex', alignItems: 'center', gap: '10px', boxShadow: '0 4px 16px rgba(0,0,0,0.08)', border: `3px solid ${TEAM_COLORS[idx]}30` }}>
            ...
            <span style={{ fontWeight: '800', color: '#78350f', fontSize: '15px' }}>{team.name}</span>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <button onClick={() => removePoint(idx)} style={{ background: '#fffbeb', border: 'none', width: '28px', height: '28px', borderRadius: '8px', cursor: 'pointer', fontWeight: '800', fontSize: '16px', color: '#f59e0b' }}>−</button>
      ...
      <div style={{ background: 'white', borderRadius: '16px', padding: '10px 20px', display: 'flex', alignItems: 'center', gap: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
        ...
        <div style={{ flex: 1, background: '#fffbeb', borderRadius: '8px', height: '12px', overflow: 'hidden' }}>
        ...
        {timerRunning && <button onClick={() => setTimerRunning(false)} style={{ background: '#fffbeb', color: '#f59e0b', border: 'none', padding: '10px 16px', borderRadius: '12px', fontWeight: '800', fontSize: '14px', cursor: 'pointer' }}>⏸ Pause</button>}
      </div>
      <div style={{ background: 'white', borderRadius: '28px', padding: '24px', textAlign: 'center', boxShadow: '0 8px 32px rgba(0,0,0,0.1)', border: '3px solid #fde68a', flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ fontSize: '120px', animation: 'rainbowSpin 3s ease-in-out infinite', marginBottom: '12px' }}>{q.emoji}</div>
        <h2 style={{ fontSize: '30px', fontWeight: '900', color: '#78350f', margin: 0 }}>{q.question}</h2>
      </div>
      ...
      <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
        {!revealed ? <button onClick={revealAnswer} style={{ background: 'linear-gradient(135deg, #f59e0b, #ec4899)', color: 'white', padding: '14px 36px', borderRadius: '16px', border: 'none', fontWeight: '800', fontSize: '18px', cursor: 'pointer', boxShadow: '0 6px 20px rgba(245,158,11,0.4)' }}>👁 Show Answer!</button>
          : <button onClick={nextQuestion} style={{ background: 'linear-gradient(135deg, #ec4899, #f59e0b)', color: 'white', padding: '14px 36px', borderRadius: '16px', border: 'none', fontWeight: '800', fontSize: '18px', cursor: 'pointer', boxShadow: '0 6px 20px rgba(236,72,153,0.4)' }}>{current + 1 >= qs.length ? '🏁 See Results!' : 'Next! →'}</button>}
        <Link href="/esl-games/live/premium/colours" style={{ background: 'white', color: '#f59e0b', padding: '14px 20px', borderRadius: '16px', textDecoration: 'none', fontWeight: '700', fontSize: '15px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>Exit</Link>
      </div>
    </main>
  )
}

export default function ColoursTVPage() {
  return (
    <Suspense fallback={<main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #fef9c3, #fdf2f8)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontSize: '64px' }}>🌈</div></main>}>
      <ColoursTVInner />
    </Suspense>
  )
}
