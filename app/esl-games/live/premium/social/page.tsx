...
export default function SocialQuizPage() {
  ...
  if (!authed) return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #1a1a2e, #2d2d4e)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ background: 'white', borderRadius: '16px', padding: '40px', maxWidth: '420px', width: '100%', textAlign: 'center' }}>
        ...
        <Link href="/esl-games/live/premium" style={{ background: '#0891b2', color: 'white', padding: '12px 28px', borderRadius: '8px', textDecoration: 'none', fontWeight: 'bold', fontSize: '15px', display: 'inline-block' }}>🔓 Get Premium Access</Link>
      </div>
    </main>
  )
  ...
  return (
    <main style={{ fontFamily: 'sans-serif', background: '#f8f9fa', minHeight: '100vh' }}>
      <div style={{ background: 'linear-gradient(135deg, #0891b2, #0e7490)', padding: '52px 24px', color: 'white', textAlign: 'center' }}>
        ...
        <h1 style={{ fontSize: '36px', fontWeight: 'bold', margin: '0 0 10px' }}>Social Studies Quiz</h1>
        <p style={{ opacity: 0.85, fontSize: '16px', margin: 0 }}>Geography, history, culture & citizenship — P1 to M3</p>
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '40px 24px' }}>
        <div style={{ background: 'white', borderRadius: '16px', padding: '28px', marginBottom: '28px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
            {ageGroups.map(ag => (
              <button key={ag} onClick={() => setSelectedAge(ag)} style={{ padding: '12px 20px', borderRadius: '12px', fontSize: '14px', fontWeight: '600', cursor: 'pointer', border: '2px solid', borderColor: selectedAge === ag ? '#0891b2' : '#e2e8f0', background: selectedAge === ag ? '#ecfeff' : 'white', color: selectedAge === ag ? '#0891b2' : '#444', transition: 'all 0.15s', textAlign: 'center' as const }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {[
            { href: `/esl-games/live/premium/social/solo?ageGroup=${selectedAge}`, icon: '👤', title: 'Solo Play', desc: 'Play on your own — 20 questions, 15 seconds each.', color: '#0891b2', bg: '#ecfeff', btnBg: '#0891b2', label: 'Play Solo →' },
            { href: `/esl-games/live/premium/social/host?ageGroup=${selectedAge}`, icon: '📱', title: 'Online Multiplayer', desc: 'Create a room. Students join on their phones at jobsinthailand.net/play', color: '#7C3AED', bg: '#f5f3ff', btnBg: '#7C3AED', label: 'Host Game →' },
            { href: `/esl-games/live/premium/social/tv?ageGroup=${selectedAge}`, icon: '📺', title: 'TV Classroom Mode', desc: 'Show on a smart TV or projector. Up to 4 teams — no phones needed.', color: '#E85D26', bg: '#fff7ed', btnBg: '#E85D26', label: 'TV Mode →' },
          ].map(m => (
            <Link key={m.href} href={m.href} style={{ textDecoration: 'none' }}>
              <div style={{ background: 'white', borderRadius: '16px', padding: '24px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', display: 'flex', gap: '20px', alignItems: 'center', cursor: 'pointer', border: '2px solid transparent', transition: 'border-color 0.2s' }}
                onMouseEnter={e => (e.currentTarget.style.borderColor = m.color)} onMouseLeave={e => (e.currentTarget.style.borderColor = 'transparent')}>
