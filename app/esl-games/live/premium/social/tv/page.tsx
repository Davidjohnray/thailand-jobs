...
function TVGame() {
  ...
  const [teamNames, setTeamNames] = useState<string[]>(TEAM_NAMES.slice(0, 2))
  ...
  function awardPoint(i: number) {
    if (pointsAwarded[i]) { setScores(prev => prev.map((s, idx) => idx === i ? s - 1 : s)); setPointsAwarded(prev => prev.map((p, idx) => idx === i ? false : p)) }
    else { setScores(prev => prev.map((s, idx) => idx === i ? s + 1 : s)); setPointsAwarded(prev => prev.map((p, idx) => idx === i ? true : p)) }
  }
  function next() { if (current + 1 >= questions.length) { setFinished(true); return }; setCurrent(c => c + 1); setRevealed(false); setRunning(false); setTimeLeft(20); setPointsAwarded([false, false, false, false]) }
    ...
    <div style={{ background: '#1a1a2e', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '32px' }}>
      <div style={{ background: 'white', borderRadius: '20px', padding: '40px', maxWidth: '560px', width: '100%' }}>
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          ...
          <p style={{ color: '#666', fontSize: '15px', margin: 0 }}>🌍 Social Studies — {ageGroup}</p>
        </div>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            {[2, 3, 4].map(n => (
              <button key={n} onClick={() => { setTeamCount(n); setTeamNames(TEAM_NAMES.slice(0, n)) }}
                style={{ flex: 1, padding: '14px', borderRadius: '10px', border: teamCount === n ? '3px solid #0891b2' : '2px solid #eee', background: teamCount === n ? '#ecfeff' : 'white', fontWeight: 'bold', fontSize: '18px', color: teamCount === n ? '#0891b2' : '#1a1a2e', cursor: 'pointer' }}>
                {n} Teams
              </button>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {Array.from({ length: teamCount }).map((_, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <input value={teamNames[i] || `Team ${i + 1}`} onChange={e => setTeamNames(prev => prev.map((n, idx) => idx === i ? e.target.value : n))}
                  style={{ flex: 1, padding: '10px 14px', borderRadius: '8px', border: '2px solid #eee', fontSize: '15px', outline: 'none', fontWeight: 'bold' }} placeholder={`Team ${i + 1}`} />
              </div>
            ))}
          </div>
        </div>
        <button onClick={() => setSetupDone(true)} style={{ width: '100%', background: '#0891b2', color: 'white', padding: '16px', borderRadius: '12px', border: 'none', fontWeight: 'bold', fontSize: '18px', cursor: 'pointer' }}>🎮 Start Game →</button>
      </div>
    </div>
  )
  ...
  if (finished) {
    ...
    return (
      <div style={{ background: '#1a1a2e', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '24px', padding: '40px' }}>
        ...
        <p style={{ color: '#fbbf24', fontSize: '28px', fontWeight: 'bold' }}>{winners.length === 1 ? `🥇 Winner: ${winners[0]}!` : `🤝 Draw: ${winners.join(' & ')}!`}</p>
        ...
        <div style={{ display: 'flex', gap: '16px', marginTop: '16px' }}>
          <button onClick={() => window.location.reload()} style={{ background: '#0891b2', color: 'white', padding: '14px 36px', borderRadius: '12px', border: 'none', fontSize: '18px', fontWeight: 'bold', cursor: 'pointer' }}>Play Again</button>
          <Link href="/esl-games/live/premium/social" style={{ background: 'rgba(255,255,255,0.15)', color: 'white', padding: '14px 36px', borderRadius: '12px', textDecoration: 'none', fontSize: '18px', fontWeight: 'bold' }}>Change Age Group</Link>
        </div>
      </div>
    )
  }
  ...
  return (
    <div style={{ background: '#1a1a2e', minHeight: '100vh', padding: '24px 40px', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        <div style={{ color: 'white', fontSize: '18px', fontWeight: 'bold', opacity: 0.7 }}>🌍 Social Studies — {ageGroup}</div>
      ...
      <div style={{ display: 'flex', gap: '24px', flex: 1 }}>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
          <div style={{ background: 'rgba(255,255,255,0.08)', borderRadius: '20px', padding: '28px 36px', textAlign: 'center', marginBottom: '20px' }}>
            <div style={{ color: 'white', fontSize: '30px', fontWeight: 'bold', lineHeight: '1.4' }}>{q.q}</div>
            {running && !revealed && <div style={{ color: timerColor, fontSize: '56px', fontWeight: 'bold', marginTop: '12px' }}>{timeLeft}</div>}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '20px' }}>
            {q.options.map((opt, i) => {
              ...
              return (
                <div key={i} style={{ background: bg, border, borderRadius: '12px', padding: '16px 20px', display: 'flex', alignItems: 'center', gap: '12px', transition: 'all 0.3s' }}>
                  <div style={{ width: '40px', height: '40px', background: 'rgba(255,255,255,0.2)', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', fontWeight: 'bold', color: 'white', flexShrink: 0 }}>{optionLabels[i]}</div>
                  <div style={{ color: 'white', fontSize: '18px', fontWeight: 'bold' }}>{opt}</div>
                  {revealed && isCorrect && <div style={{ marginLeft: 'auto', fontSize: '28px' }}>✅</div>}
                </div>
              )
            })}
          </div>
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
            {!running && !revealed && <button onClick={() => setRunning(true)} style={{ background: '#16a34a', color: 'white', padding: '12px 28px', borderRadius: '10px', border: 'none', fontSize: '16px', fontWeight: 'bold', cursor: 'pointer' }}>▶ Start Timer</button>}
            {!revealed && <button onClick={() => { setRevealed(true); setRunning(false) }} style={{ background: '#E85D26', color: 'white', padding: '12px 28px', borderRadius: '10px', border: 'none', fontSize: '16px', fontWeight: 'bold', cursor: 'pointer' }}>💡 Reveal Answer</button>}
            {revealed && <button onClick={next} style={{ background: '#0891b2', color: 'white', padding: '12px 28px', borderRadius: '10px', border: 'none', fontSize: '16px', fontWeight: 'bold', cursor: 'pointer' }}>{current + 1 >= questions.length ? '🏆 See Final Scores' : 'Next Question →'}</button>}
            <Link href="/esl-games/live/premium/social" style={{ background: 'rgba(255,255,255,0.1)', color: 'white', padding: '12px 28px', borderRadius: '10px', textDecoration: 'none', fontSize: '16px', fontWeight: 'bold' }}>✕ Exit</Link>
          </div>
        </div>
        <div style={{ width: '220px', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          {teamNames.slice(0, teamCount).map((name, i) => (
            <div key={i} style={{ background: TEAM_COLORS[i], borderRadius: '14px', padding: '16px', textAlign: 'center' }}>
              ...
              {revealed && <button onClick={() => awardPoint(i)} style={{ marginTop: '8px', width: '100%', padding: '8px', borderRadius: '8px', border: 'none', fontWeight: 'bold', fontSize: '13px', cursor: 'pointer', background: pointsAwarded[i] ? 'rgba(0,0,0,0.3)' : 'rgba(255,255,255,0.25)', color: 'white' }}>{pointsAwarded[i] ? '✓ Awarded' : '+ Give Point'}</button>}
            </div>
          ))}
        </div>
      </div>
