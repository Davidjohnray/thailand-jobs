...
function HostGame() {
  ...
  useEffect(() => { if (!authed) return; supabase.from('live_game_rooms').insert([{ code: roomCode, game_type: 'social-quiz', topic: ageGroup, status: 'waiting', current_question: 0, question_order: JSON.stringify(questions) }]).then(({ error }: any) => { if (error && error.code !== '23505') console.error(error.message) }) }, [authed])
      ...
      <div style={{ maxWidth: '700px', margin: '0 auto' }}>
        ...
        <div style={{ background: 'white', borderRadius: '20px', padding: '40px', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', marginTop: '20px', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', marginBottom: '24px' }}>
            <span style={{ background: '#ecfeff', color: '#0891b2', padding: '6px 16px', borderRadius: '20px', fontSize: '13px', fontWeight: '600' }}>🌍 Social Studies</span>
          ...
          <button onClick={startGame} disabled={players.length === 0} style={{ background: players.length > 0 ? '#0891b2' : '#cbd5e1', color: 'white', padding: '14px 48px', borderRadius: '12px', border: 'none', fontSize: '18px', fontWeight: 'bold', cursor: players.length > 0 ? 'pointer' : 'default', width: '100%' }}>
    ...
    <main style={{ fontFamily: 'sans-serif', background: '#1a1a2e', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ maxWidth: '600px', width: '100%', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
          ...
          <Link href="/esl-games/live/premium/social" style={{ background: 'rgba(255,255,255,0.15)', color: 'white', padding: '12px 28px', borderRadius: '10px', textDecoration: 'none', fontWeight: 'bold', fontSize: '16px' }}>Back to Social Studies</Link>
        </div>
      </div>
    </main>
  )
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '10px' }}>
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            ...
            <span style={{ background: '#ecfeff', color: '#0891b2', padding: '6px 16px', borderRadius: '20px', fontSize: '14px', fontWeight: '600' }}>🌍 {ageGroup}</span>
          </div>
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: '20px' }}>
          <div style={{ background: 'white', borderRadius: '20px', padding: '28px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
            ...
            <h2 style={{ fontSize: '19px', fontWeight: 'bold', color: '#1a1a2e', margin: '0 0 24px', lineHeight: '1.4' }}>{q.q}</h2>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
              {q.options.map((opt, i) => {
                ...
                return (
                  <div key={i} style={{ background: bg, border, borderRadius: '10px', padding: '12px', display: 'flex', alignItems: 'center', gap: '8px', transition: 'all 0.3s' }}>
                    <div style={{ width: '28px', height: '28px', background: revealed ? (isCorrect ? '#16a34a' : '#e2e8f0') : 'rgba(255,255,255,0.25)', borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', color: 'white', fontSize: '13px', flexShrink: 0 }}>{optionLabels[i]}</div>
                    <span style={{ fontWeight: '600', fontSize: '13px', color }}>{opt}</span>
            ...
            <div style={{ display: 'flex', gap: '12px', marginTop: '20px' }}>
              ...
              {revealed && <button onClick={nextQuestion} style={{ background: '#0891b2', color: 'white', padding: '12px 24px', borderRadius: '10px', border: 'none', fontWeight: 'bold', fontSize: '15px', cursor: 'pointer', flex: 1 }}>{current + 1 >= questions.length ? '🏆 Final Scores' : 'Next Question →'}</button>}
            </div>
          </div>
