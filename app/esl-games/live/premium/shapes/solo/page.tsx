...
function ShapesSoloInner() {
  ...
  if (finished) {
    ...
    return (
      <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #faf5ff, #fce7f3, #eff6ff)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
        ...
        <div style={{ background: 'white', borderRadius: '32px', padding: '48px', maxWidth: '420px', width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.12)', border: '3px solid #d8b4fe', animation: 'popIn 0.5s ease' }}>
          <div style={{ fontSize: '80px', marginBottom: '12px' }}>{pct === 100 ? '🏆' : pct >= 80 ? '🌟' : pct >= 60 ? '😊' : '💪'}</div>
          <h2 style={{ fontSize: '34px', fontWeight: '900', color: '#3b0764', marginBottom: '8px' }}>{pct === 100 ? 'PERFECT!' : pct >= 80 ? 'Amazing!' : pct >= 60 ? 'Good job!' : 'Keep trying!'}</h2>
          <p style={{ color: '#a855f7', fontSize: '18px', marginBottom: '8px' }}>You got <strong style={{ fontSize: '24px' }}>{score}</strong> out of <strong>{qs.length}</strong>!</p>
          <div style={{ fontSize: '52px', fontWeight: '900', color: '#ec4899', marginBottom: '28px' }}>{pct}%</div>
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
            <button onClick={restart} style={{ background: 'linear-gradient(135deg, #a855f7, #ec4899)', color: 'white', padding: '14px 32px', borderRadius: '16px', border: 'none', fontWeight: '900', fontSize: '18px', cursor: 'pointer', boxShadow: '0 6px 20px rgba(168,85,247,0.4)' }}>🔄 Again!</button>
            <Link href="/esl-games/live/premium/shapes" style={{ background: '#faf5ff', color: '#3b0764', padding: '14px 24px', borderRadius: '16px', textDecoration: 'none', fontWeight: '700', fontSize: '16px' }}>← Back</Link>
          </div>
        </div>
      </main>
    )
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #faf5ff 0%, #fce7f3 50%, #eff6ff 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
      <style>{`
      ...
      <div style={{ width: '100%', maxWidth: '560px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
          <Link href="/esl-games/live/premium/shapes" style={{ background: 'white', padding: '8px 16px', borderRadius: '20px', textDecoration: 'none', color: '#6b7280', fontWeight: '700', fontSize: '14px', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>← Exit</Link>
          <div style={{ background: 'white', padding: '8px 20px', borderRadius: '20px', fontWeight: '900', fontSize: '20px', color: '#a855f7', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>⭐ {score}</div>
        </div>
        ...
        <div style={{ background: 'white', borderRadius: '28px', padding: '28px', textAlign: 'center', marginBottom: '14px', boxShadow: '0 8px 32px rgba(0,0,0,0.1)', border: '3px solid #d8b4fe' }}>
          <p style={{ color: '#a855f7', fontSize: '15px', fontWeight: '800', marginBottom: '10px', textTransform: 'uppercase', letterSpacing: '1px' }}>Question {current + 1} of {qs.length}</p>
          <div style={{ fontSize: '100px', marginBottom: '10px', animation: 'shapeSpin 3s ease-in-out infinite' }}>{q.emoji}</div>
          <h2 style={{ fontSize: '24px', fontWeight: '900', color: '#3b0764', margin: 0 }}>{q.question}</h2>
        </div>
...
export default function ShapesSoloPage() {
  return (
