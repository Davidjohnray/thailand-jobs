...
export default function CountMatchPage() {
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #fef3c7 0%, #fde8d8 50%, #fce7f3 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px', fontFamily: 'system-ui, sans-serif' }}>
      <style>{`
      ...
      <div style={{ background: 'white', borderRadius: '32px', padding: '48px 40px', maxWidth: '480px', width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.12)', border: '3px solid #fcd34d', position: 'relative', zIndex: 1 }}>
        ...
        <p style={{ color: '#6b7280', fontSize: '16px', marginBottom: '32px' }}>Count the objects and find the right number! 🌟</p>
        ...
        <button onClick={() => mode && router.push(`/esl-games/live/premium/count-match/${mode}`)} disabled={!mode}
          style={{ width: '100%', padding: '18px', borderRadius: '16px', border: 'none', background: mode ? 'linear-gradient(135deg, #f59e0b, #ec4899)' : '#e5e7eb', color: mode ? 'white' : '#9ca3af', fontWeight: '900', fontSize: '20px', cursor: mode ? 'pointer' : 'not-allowed', boxShadow: mode ? '0 8px 24px rgba(245,158,11,0.4)' : 'none', transition: 'all 0.2s' }}>
