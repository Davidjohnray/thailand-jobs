...
const VOCAB = [
  { emoji: '0️⃣', word: 'zero',  phonetic: '/ˈzɪə.rəʊ/',  spelling: 'z – e – r – o',             sentence: 'Zero means nothing at all.',         tip: 'Zero is nothing — an empty hand has zero fingers up!' },
  { emoji: '1️⃣', word: 'one',   phonetic: '/wʌn/',         spelling: 'o – n – e',                 sentence: 'I have one nose.',                   tip: 'Hold up one finger — that is one!' },
  { emoji: '2️⃣', word: 'two',   phonetic: '/tuː/',          spelling: 't – w – o',                 sentence: 'I have two eyes.',                   tip: 'You have two eyes and two ears!' },
  { emoji: '3️⃣', word: 'three', phonetic: '/θriː/',         spelling: 't – h – r – e – e',         sentence: 'I have three pets.',                 tip: 'A triangle has three sides — count them!' },
  { emoji: '4️⃣', word: 'four',  phonetic: '/fɔːr/',         spelling: 'f – o – u – r',             sentence: 'A cat has four legs.',               tip: 'A square has four sides — count them!' },
  { emoji: '5️⃣', word: 'five',  phonetic: '/faɪv/',         spelling: 'f – i – v – e',             sentence: 'I have five fingers on one hand.',   tip: 'Hold up one hand — that is five fingers!' },
  { emoji: '6️⃣', word: 'six',   phonetic: '/sɪks/',         spelling: 's – i – x',                 sentence: 'An insect has six legs.',            tip: 'Six is one more than five — add one more finger!' },
  { emoji: '7️⃣', word: 'seven', phonetic: '/ˈsev.ən/',     spelling: 's – e – v – e – n',         sentence: 'There are seven days in a week.',    tip: 'Monday to Sunday — that is seven days!' },
  { emoji: '8️⃣', word: 'eight', phonetic: '/eɪt/',          spelling: 'e – i – g – h – t',         sentence: 'A spider has eight legs.',           tip: 'Eight sounds like "ate" — you ate eight biscuits!' },
  { emoji: '9️⃣', word: 'nine',  phonetic: '/naɪn/',         spelling: 'n – i – n – e',             sentence: 'A cat has nine lives.',              tip: 'Nine is one less than ten — almost there!' },
  { emoji: '🔟', word: 'ten',   phonetic: '/ten/',           spelling: 't – e – n',                 sentence: 'I have ten fingers in total.',        tip: 'Hold up both hands — that is ten fingers!' },
]
    ...
    return (
      <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #eff6ff 0%, #f5f3ff 50%, #fef9c3 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
        <style>{`
          ...
          @keyframes numBounce{0%,100%{transform:scale(1)}50%{transform:scale(1.15)}}
        ...
        <div key={lessonIdx} style={{ background: 'white', borderRadius: '32px', padding: '32px 28px', maxWidth: '560px', width: '100%', textAlign: 'center', boxShadow: '0 12px 40px rgba(0,0,0,0.12)', border: '3px solid #93c5fd', animation: 'wordPop 0.35s ease', marginBottom: '14px' }}>
          ...
          {/* Emoji */}
          <div style={{ fontSize: '100px', marginBottom: '10px', animation: 'numBounce 2s ease-in-out infinite' }}>{v.emoji}</div>

          {/* Word + audio + phonetic */}
          <div style={{ marginBottom: '14px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', marginBottom: '6px' }}>
              <div style={{ fontSize: '40px', fontWeight: '900', color: '#1e3a5f', letterSpacing: '2px' }}>{v.word}</div>
              <button onClick={() => speakWord(v.word)}
                style={{ background: 'linear-gradient(135deg, #3b82f6, #8b5cf6)', border: 'none', borderRadius: '50%', width: '44px', height: '44px', fontSize: '20px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 12px rgba(59,130,246,0.4)', animation: 'audioPulse 2s ease-in-out infinite', flexShrink: 0 }}
                title="Hear the word">
                🔊
              </button>
            </div>
          ...
          {!showSpelling ? (
            <button onClick={() => { setShowSpelling(true); speakWord(v.word) }}
              style={{ background: 'linear-gradient(135deg, #3b82f6, #8b5cf6)', color: 'white', border: 'none', padding: '12px 28px', borderRadius: '12px', fontWeight: '800', fontSize: '15px', cursor: 'pointer', marginBottom: '10px', boxShadow: '0 4px 14px rgba(59,130,246,0.35)', width: '100%' }}>
              🔤 Show Spelling
            </button>
          ) : (
              ...
              <div style={{ background: '#eff6ff', border: '2px solid #93c5fd', borderRadius: '16px', padding: '16px' }}>
                ...
                <div style={{ display: 'flex', gap: '6px', justifyContent: 'center', flexWrap: 'wrap', marginBottom: '10px' }}>
                  {v.word.split('').map((letter, i) => (
                    <div key={i} style={{ background: 'white', border: '2px solid #3b82f6', borderRadius: '10px', width: '36px', height: '42px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: '900', fontSize: '20px', color: '#3b82f6', boxShadow: '0 2px 6px rgba(59,130,246,0.2)' }}>{letter}</div>
                  ))}
                </div>
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #eff6ff 0%, #f5f3ff 50%, #fef9c3 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ background: 'white', borderRadius: '32px', padding: '48px 40px', maxWidth: '480px', width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.12)', border: '3px solid #93c5fd', position: 'relative', zIndex: 1 }}>
        ...
        <p style={{ color: '#3b82f6', fontSize: '16px', marginBottom: '28px', fontWeight: '700' }}>zero, one, two ... all the way to ten! 🌟</p>
        ...
        <div style={{ background: '#eff6ff', border: '2px solid #93c5fd', borderRadius: '20px', padding: '20px', marginBottom: '14px', textAlign: 'left' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
            ...
            <div style={{ fontWeight: '800', color: '#1e3a5f', fontSize: '16px' }}>📖 Teach the Numbers First</div>
          </div>
          <p style={{ color: '#1e40af', fontSize: '13px', margin: '0 0 12px', lineHeight: '1.6' }}>Each number has its picture, audio 🔊, phonetic spelling and a real-world example sentence.</p>
          ...
          <button onClick={startLesson} disabled={!mode}
            style={{ width: '100%', background: mode ? 'linear-gradient(135deg, #3b82f6, #8b5cf6)' : '#e5e7eb', color: mode ? 'white' : '#9ca3af', border: 'none', padding: '14px', borderRadius: '12px', fontWeight: '900', fontSize: '16px', cursor: mode ? 'pointer' : 'not-allowed', boxShadow: mode ? '0 6px 20px rgba(59,130,246,0.35)' : 'none' }}>
        ...
        {/* Skip lesson */}
        <button onClick={() => mode && router.push(`/esl-games/live/premium/numbers/${mode}`)} disabled={!mode}
