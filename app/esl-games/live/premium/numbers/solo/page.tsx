...
function NumbersSoloInner() {
  ...
  if (finished) {
    ...
    return (
      <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #eff6ff, #f5f3ff, #fef9c3)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
        ...
        <div style={{ background: 'white', borderRadius: '32px', padding: '48px', maxWidth: '420px', width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.12)', border: '3px solid #93c5fd', animation: 'popIn 0.5s ease' }}>
          <div style={{ fontSize: '80px', marginBottom: '12px' }}>{pct === 100 ? '🏆' : pct >= 80 ? '🌟' : pct >= 60 ? '😊' : '💪'}</div>
          <h2 style={{ fontSize: '34px', fontWeight: '900', color: '#1e3a5f', marginBottom: '8px' }}>{pct === 100 ? 'PERFECT!' : pct >= 80 ? 'Amazing!' : pct >= 60 ? 'Good job!' : 'Keep trying!'}</h2>
          <p style={{ color: '#3b82f6', fontSize: '18px', marginBottom: '8px' }}>You got <strong style={{ fontSize: '24px' }}>{score}</strong> out of <strong>{qs.length}</strong>!</p>
          <div style={{ fontSize: '52px', fontWeight: '900', color: '#8b5cf6', marginBottom: '28px' }}>{pct}%</div>
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
            <button onClick={restart} style={{ background: 'linear-gradient(135deg, #3b82f6, #8b5cf6)', color: 'white', padding: '14px 32px', borderRadius: '16px', border: 'none', fontWeight: '900', fontSize: '18px', cursor: 'pointer' }}>🔄 Again!</button>
            <Link href="/esl-games/live/premium/numbers" style={{ background: '#eff6ff', color: '#1e3a5f', padding: '14px 24px', borderRadius: '16px', textDecoration: 'none', fontWeight: '700', fontSize: '16px' }}>← Back</Link>
          </div>
        </div>
      </main>
    )
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #eff6ff 0%, #f5f3ff 50%, #fef9c3 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
      <style>{`
      ...
      <div style={{ width: '100%', maxWidth: '560px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
          <Link href="/esl-games/live/premium/numbers" style={{ background: 'white', padding: '8px 16px', borderRadius: '20px', textDecoration: 'none', color: '#6b7280', fontWeight: '700', fontSize: '14px', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>← Exit</Link>
          <div style={{ background: 'white', padding: '8px 20px', borderRadius: '20px', fontWeight: '900', fontSize: '20px', color: '#3b82f6', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>⭐ {score}</div>
        </div>
        <div style={{ display: 'flex', gap: '5px', justifyContent: 'center', marginBottom: '14px', flexWrap: 'wrap' }}>
          {qs.map((_, i) => <div key={i} style={{ width: '10px', height: '10px', borderRadius: '50%', background: i < current ? '#22c55e' : i === current ? '#3b82f6' : '#e5e7eb', transition: 'background 0.3s' }} />)}
        </div>
        <div style={{ background: 'white', borderRadius: '16px', padding: '10px 16px', marginBottom: '14px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <div style={{ flex: 1, background: '#eff6ff', borderRadius: '8px', height: '10px', overflow: 'hidden' }}>
            <div style={{ background: timerRunning || timeLeft < TOTAL_TIME ? timerColor : '#93c5fd', height: '10px', borderRadius: '8px', width: `${timerPct}%`, transition: 'width 1s linear, background 0.3s' }} />
          </div>
          ...
          {timerRunning && (
            <button onClick={() => setTimerRunning(false)} style={{ background: '#eff6ff', color: '#3b82f6', border: 'none', padding: '8px 12px', borderRadius: '10px', fontWeight: '800', fontSize: '13px', cursor: 'pointer' }}>⏸</button>
          )}
        </div>
        <div style={{ background: 'white', borderRadius: '28px', padding: '28px', textAlign: 'center', marginBottom: '14px', boxShadow: '0 8px 32px rgba(0,0,0,0.1)', border: '3px solid #93c5fd' }}>
          <p style={{ color: '#3b82f6', fontSize: '15px', fontWeight: '800', marginBottom: '10px', textTransform: 'uppercase', letterSpacing: '1px' }}>Question {current + 1} of {qs.length}</p>
          <div style={{ fontSize: '80px', marginBottom: '10px', animation: 'numBounce 2s ease-in-out infinite', letterSpacing: '4px' }}>{q.emoji}</div>
          <h2 style={{ fontSize: '24px', fontWeight: '900', color: '#1e3a5f', margin: 0 }}>{q.question}</h2>
        </div>
          ...
          {q.options.map((opt, idx) => {
            ...
            return (
              <button key={idx} onClick={() => handleAnswer(idx)} disabled={selected !== null}
                ...
                <div style={{ fontSize: '44px', marginBottom: '6px' }}>{opt.emoji}</div>
...
export default function NumbersSoloPage() {
  return (
