...
export default function RoomsPage() {
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #f0f9ff 0%, #f5f3ff 50%, #f0fdf4 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px', position: 'relative', overflow: 'hidden' }}>
      <style>{`@keyframes float{0%,100%{transform:translateY(0)}50%{transform:translateY(-14px)}} @keyframes bounce{0%,100%{transform:scale(1)}50%{transform:scale(1.12)}}`}</style>
      ...
      <div style={{ background: 'white', borderRadius: '32px', padding: '48px 40px', maxWidth: '480px', width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.12)', border: '3px solid #7dd3fc', position: 'relative', zIndex: 1 }}>
        ...
        <p style={{ color: '#0ea5e9', fontSize: '16px', marginBottom: '32px', fontWeight: '700' }}>Bedroom, kitchen, bathroom and more! 🌟</p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '32px' }}>
          {MODES.map(m => (
            <button key={m.id} onClick={() => setMode(m.id)}
              style={{ padding: '18px 24px', borderRadius: '16px', border: '3px solid', borderColor: mode === m.id ? m.color : '#e0f2fe', background: mode === m.id ? m.color : '#f0f9ff', color: mode === m.id ? 'white' : '#0c4a6e', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '14px', transition: 'all 0.2s', transform: mode === m.id ? 'scale(1.03)' : 'scale(1)', boxShadow: mode === m.id ? `0 6px 20px ${m.color}50` : 'none' }}>
        ...
        <button onClick={() => mode && router.push(`/esl-games/live/premium/rooms/${mode}`)} disabled={!mode}
          style={{ width: '100%', padding: '18px', borderRadius: '16px', border: 'none', background: mode ? 'linear-gradient(135deg, #0ea5e9, #8b5cf6)' : '#e0f2fe', color: mode ? 'white' : '#7dd3fc', fontWeight: '900', fontSize: '20px', cursor: mode ? 'pointer' : 'not-allowed', boxShadow: mode ? '0 8px 28px rgba(14,165,233,0.4)' : 'none', transition: 'all 0.2s' }}>
          {mode ? '🏠 Explore the House!' : 'Choose a mode first!'}
        </button>
