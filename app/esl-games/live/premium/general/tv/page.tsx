...
const teamColors = ['#E85D26', '#2D6BE4', '#16a34a', '#7C3AED']
      ...
      <div style={{ maxWidth: '600px', margin: '0 auto' }}>
        ...
        <div style={{ background: 'white', borderRadius: '20px', padding: '36px', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', marginTop: '20px' }}>
          <div style={{ textAlign: 'center', marginBottom: '28px' }}>
            ...
            <p style={{ color: '#666', fontSize: '14px', marginTop: '6px' }}>🌟 General Knowledge — {ageGroup}</p>
          </div>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              {[2, 3, 4].map(n => (
                <button key={n} onClick={() => setNumTeams(n)}
                  style={{ flex: 1, padding: '12px', borderRadius: '10px', border: `2px solid ${numTeams === n ? COLOR : '#e5e7eb'}`, background: numTeams === n ? '#fff3ed' : 'white', fontWeight: 'bold', fontSize: '18px', cursor: 'pointer', color: numTeams === n ? COLOR : '#666' }}>
                  {n}
                </button>
              ))}
            </div>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px', flexWrap: 'wrap', gap: '8px' }}>
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            <span style={{ background: 'rgba(255,255,255,0.15)', color: 'white', padding: '6px 14px', borderRadius: '20px', fontSize: '14px', fontWeight: 'bold' }}>🌟 General Knowledge</span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: '16px' }}>
          <div>
            ...
            <div style={{ background: 'white', borderRadius: '16px', padding: '24px', marginBottom: '14px' }}>
              <h2 style={{ fontSize: '24px', fontWeight: 'bold', color: '#1a1a2e', margin: 0, lineHeight: '1.4' }}>{q.q}</h2>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '14px' }}>
              {q.options.map((opt, i) => {
                ...
                const color = revealed ? (isCorrect ? '#15803d' : 'rgba(255,255,255,0.5)') : 'white'
                ...
                return (
                  <div key={i} style={{ background: bg, border, borderRadius: '12px', padding: '16px', display: 'flex', alignItems: 'center', gap: '12px', transition: 'all 0.3s' }}>
                    ...
                    <span style={{ fontWeight: '600', fontSize: '16px', color: textColor }}>{opt}</span>
