    ...
    <main style={{ minHeight: '100vh', background: '#1a1a2e', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ maxWidth: '500px', width: '100%', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
          <button onClick={() => { setCurrent(0); setScore(0); setSelected(null); setRevealed(false); setTimeLeft(15); setRunning(true); setFinished(false) }}
            style={{ background: COLOR, color: 'white', padding: '12px 28px', borderRadius: '10px', border: 'none', fontWeight: 'bold', fontSize: '16px', cursor: 'pointer' }}>
            🔄 Play Again
          </button>
          <Link href="/esl-games/live/premium/general" style={{ background: 'rgba(255,255,255,0.15)', color: 'white', padding: '12px 28px', borderRadius: '10px', textDecoration: 'none', fontWeight: 'bold', fontSize: '16px' }}>
            Back
          </Link>
        </div>
      </div>
      ...
      <div style={{ maxWidth: '640px', margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '8px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            <span style={{ background: '#fff3ed', color: COLOR, padding: '6px 14px', borderRadius: '20px', fontSize: '14px', fontWeight: '600' }}>Q{current + 1}/{questions.length}</span>
        ...
        <div style={{ background: 'white', borderRadius: '20px', padding: '28px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
            <span style={{ color: '#888', fontSize: '14px' }}>🌟 General Knowledge — {ageGroup}</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '20px' }}>
            {q.options.map((opt, i) => {
              ...
              const color = revealed ? (isCorrect ? '#15803d' : isSelected ? '#dc2626' : '#888') : 'white'
              return (
                <button key={i} onClick={() => handleSelect(opt)} disabled={revealed}
                  style={{ background: bg, border, borderRadius: '10px', padding: '14px', display: 'flex', alignItems: 'center', gap: '10px', cursor: revealed ? 'default' : 'pointer', textAlign: 'left' }}>
