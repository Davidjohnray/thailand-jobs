...
function HostGame() {
  ...
  useEffect(() => { if (!authed) return; supabase.from('live_game_rooms').insert([{ code: roomCode, game_type: 'general-quiz', topic: ageGroup, status: 'waiting', current_question: 0, question_order: JSON.stringify(questions) }]).then(({ error }: any) => { if (error && error.code !== '23505') console.error(error.message) }) }, [authed])
  ...
  if (phase === 'lobby') return (
    <main style={{ fontFamily: 'sans-serif', background: '#f8f9fa', minHeight: '100vh', padding: '32px 24px' }}>
      <div style={{ maxWidth: '700px', margin: '0 auto' }}>
        <Link href="/esl-games/live/premium/general" style={{ color: '#888', textDecoration: 'none', fontSize: '14px' }}>← Back</Link>
        <div style={{ background: 'white', borderRadius: '20px', padding: '40px', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', marginTop: '20px', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', marginBottom: '24px' }}>
            <span style={{ background: '#fff3ed', color: COLOR, padding: '6px 16px', borderRadius: '20px', fontSize: '13px', fontWeight: '600' }}>🌟 General Knowledge</span>
          ...
          <div style={{ background: '#fff3ed', borderRadius: '12px', padding: '16px', marginBottom: '28px' }}>
    ...
    <main style={{ fontFamily: 'sans-serif', background: '#1a1a2e', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ maxWidth: '600px', width: '100%', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
          ...
          <Link href="/esl-games/live/premium/general" style={{ background: 'rgba(255,255,255,0.15)', color: 'white', padding: '12px 28px', borderRadius: '10px', textDecoration: 'none', fontWeight: 'bold', fontSize: '16px' }}>Back</Link>
        </div>
      </div>
    </main>
  )
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '10px' }}>
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            ...
            <span style={{ background: '#fff3ed', color: COLOR, padding: '6px 16px', borderRadius: '20px', fontSize: '14px', fontWeight: '600' }}>🌟 {ageGroup}</span>
          </div>
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            ...
            <span style={{ background: '#fff3ed', color: COLOR, padding: '6px 16px', borderRadius: '20px', fontSize: '14px', fontWeight: '600' }}>Q{current + 1}/{questions.length}</span>
