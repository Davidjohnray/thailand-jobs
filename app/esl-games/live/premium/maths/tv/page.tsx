...
const optionColors = ['#E85D26', '#0891b2', '#2D6BE4', '#16a34a']
...
function TVGame() {
  ...
  function awardPoint(i: number) {
    if (pointsAwarded[i]) {
      setScores(prev => prev.map((s, idx) => idx === i ? s - 1 : s))
      setPointsAwarded(prev => prev.map((p, idx) => idx === i ? false : p))
    } else {
      setScores(prev => prev.map((s, idx) => idx === i ? s + 1 : s))
      setPointsAwarded(prev => prev.map((p, idx) => idx === i ? true : p))
    }
  }
  ...
  if (!authed) return (
    <main style={{ minHeight: '100vh', background: '#1a1a2e', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ background: 'white', borderRadius: '16px', padding: '40px', maxWidth: '400px', width: '100%', textAlign: 'center' }}>
        ...
        <Link href="/esl-games/live/premium" style={{ background: '#2D6BE4', color: 'white', padding: '12px 24px', borderRadius: '8px', textDecoration: 'none', fontWeight: 'bold' }}>Get Access →</Link>
      </div>
    </main>
  )
    ...
    <div style={{ background: '#1a1a2e', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '32px' }}>
      <div style={{ background: 'white', borderRadius: '20px', padding: '40px', maxWidth: '560px', width: '100%' }}>
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          ...
          <p style={{ color: '#666', fontSize: '15px', margin: 0 }}>🔢 Maths Quiz — {ageGroup}</p>
        </div>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            {[2, 3, 4].map(n => (
              <button key={n} onClick={() => { setTeamCount(n); setTeamNames(TEAM_NAMES.slice(0, n)) }}
                style={{ flex: 1, padding: '14px', borderRadius: '10px', border: teamCount === n ? '3px solid #2D6BE4' : '2px solid #eee', background: teamCount === n ? '#eff6ff' : 'white', fontWeight: 'bold', fontSize: '18px', color: teamCount === n ? '#2D6BE4' : '#1a1a2e', cursor: 'pointer' }}>
                {n} Teams
              </button>
            ))}
          </div>
        ...
        <button onClick={() => setSetupDone(true)} style={{ width: '100%', background: '#2D6BE4', color: 'white', padding: '16px', borderRadius: '12px', border: 'none', fontWeight: 'bold', fontSize: '18px', cursor: 'pointer' }}>
          🎮 Start Game →
        </button>
      </div>
    </div>
    ...
    return (
      <div style={{ background: '#1a1a2e', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '24px', padding: '40px' }}>
        ...
        <div style={{ display: 'flex', gap: '16px', marginTop: '16px' }}>
          <button onClick={() => window.location.reload()} style={{ background: '#2D6BE4', color: 'white', padding: '14px 36px', borderRadius: '12px', border: 'none', fontSize: '18px', fontWeight: 'bold', cursor: 'pointer' }}>Play Again</button>
          <Link href="/esl-games/live/premium/maths" style={{ background: 'rgba(255,255,255,0.15)', color: 'white', padding: '14px 36px', borderRadius: '12px', textDecoration: 'none', fontSize: '18px', fontWeight: 'bold' }}>Change Age Group</Link>
        </div>
      </div>
    )
  }
  ...
  return (
    <div style={{ background: '#1a1a2e', minHeight: '100vh', padding: '24px 40px', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        <div style={{ color: 'white', fontSize: '18px', fontWeight: 'bold', opacity: 0.7 }}>🔢 Maths Quiz — {ageGroup}</div>
      ...
      <div style={{ display: 'flex', gap: '24px', flex: 1 }}>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
            ...
            {q.options.map((opt, i) => {
              ...
              return (
                <div key={i} style={{ background: bg, border, borderRadius: '14px', padding: '20px 24px', display: 'flex', alignItems: 'center', gap: '14px', transition: 'all 0.3s' }}>
                  <div style={{ width: '44px', height: '44px', background: 'rgba(255,255,255,0.2)', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '22px', fontWeight: 'bold', color: 'white', flexShrink: 0 }}>{optionLabels[i]}</div>
          ...
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
            ...
            {revealed && <button onClick={next} style={{ background: '#2D6BE4', color: 'white', padding: '12px 28px', borderRadius: '10px', border: 'none', fontSize: '16px', fontWeight: 'bold', cursor: 'pointer' }}>{current + 1 >= questions.length ? '🏆 See Final Scores' : 'Next Question →'}</button>}
            <Link href="/esl-games/live/premium/maths" style={{ background: 'rgba(255,255,255,0.1)', color: 'white', padding: '12px 28px', borderRadius: '10px', textDecoration: 'none', fontSize: '16px', fontWeight: 'bold' }}>✕ Exit</Link>
          </div>
        </div>
        ...
        <div style={{ width: '220px', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          {teamNames.slice(0, teamCount).map((name, i) => (
            <div key={i} style={{ background: TEAM_COLORS[i], borderRadius: '14px', padding: '16px', textAlign: 'center' }}>
              ...
              {revealed && (
                <button onClick={() => awardPoint(i)} style={{ marginTop: '8px', width: '100%', padding: '8px', borderRadius: '8px', border: 'none', fontWeight: 'bold', fontSize: '13px', cursor: 'pointer', background: pointsAwarded[i] ? 'rgba(0,0,0,0.3)' : 'rgba(255,255,255,0.25)', color: 'white' }}>
