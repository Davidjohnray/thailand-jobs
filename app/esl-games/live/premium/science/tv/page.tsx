...
const optionColors = ['#E85D26', '#0891b2', '#16a34a', '#7C3AED']
    ...
    <div style={{ background: '#1a1a2e', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '32px' }}>
      <div style={{ background: 'white', borderRadius: '20px', padding: '40px', maxWidth: '560px', width: '100%' }}>
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          ...
          <p style={{ color: '#666', fontSize: '15px', margin: 0 }}>🔬 Science Quiz — {ageGroup}</p>
        </div>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            {[2, 3, 4].map(n => (
              <button key={n} onClick={() => { setTeamCount(n); setTeamNames(TEAM_NAMES.slice(0, n)) }}
                style={{ flex: 1, padding: '14px', borderRadius: '10px', border: teamCount === n ? '3px solid #16a34a' : '2px solid #eee', background: teamCount === n ? '#f0fdf4' : 'white', fontWeight: 'bold', fontSize: '18px', color: teamCount === n ? '#16a34a' : '#1a1a2e', cursor: 'pointer' }}>
                {n} Teams
              </button>
            ))}
          </div>
        ...
        <button onClick={() => setSetupDone(true)} style={{ width: '100%', background: '#16a34a', color: 'white', padding: '16px', borderRadius: '12px', border: 'none', fontWeight: 'bold', fontSize: '18px', cursor: 'pointer' }}>🎮 Start Game →</button>
      </div>
    </div>
  )
    ...
    return (
      <div style={{ background: '#1a1a2e', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '24px', padding: '40px' }}>
        ...
        <div style={{ display: 'flex', gap: '16px', marginTop: '16px' }}>
          <button onClick={() => window.location.reload()} style={{ background: '#16a34a', color: 'white', padding: '14px 36px', borderRadius: '12px', border: 'none', fontSize: '18px', fontWeight: 'bold', cursor: 'pointer' }}>Play Again</button>
          <Link href="/esl-games/live/premium/science" style={{ background: 'rgba(255,255,255,0.15)', color: 'white', padding: '14px 36px', borderRadius: '12px', textDecoration: 'none', fontSize: '18px', fontWeight: 'bold' }}>Change Age Group</Link>
        </div>
      </div>
    )
  }
  ...
  return (
    <div style={{ background: '#1a1a2e', minHeight: '100vh', padding: '24px 40px', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        <div style={{ color: 'white', fontSize: '18px', fontWeight: 'bold', opacity: 0.7 }}>🔬 Science Quiz — {ageGroup}</div>
      ...
      <div style={{ display: 'flex', gap: '24px', flex: 1 }}>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
          <div style={{ background: 'rgba(255,255,255,0.08)', borderRadius: '20px', padding: '32px 40px', textAlign: 'center', marginBottom: '20px' }}>
            <div style={{ color: 'white', fontSize: '34px', fontWeight: 'bold', lineHeight: '1.3' }}>{q.q}</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '20px' }}>
            {q.options.map((opt, i) => {
              ...
              return (
                <div key={i} style={{ background: bg, border, borderRadius: '14px', padding: '18px 22px', display: 'flex', alignItems: 'center', gap: '14px', transition: 'all 0.3s' }}>
                  <div style={{ width: '42px', height: '42px', background: 'rgba(255,255,255,0.2)', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px', fontWeight: 'bold', color: 'white', flexShrink: 0 }}>{optionLabels[i]}</div>
                  <div style={{ color: 'white', fontSize: '20px', fontWeight: 'bold' }}>{opt}</div>
                  {revealed && isCorrect && <div style={{ marginLeft: 'auto', fontSize: '30px' }}>✅</div>}
                </div>
              )
            })}
          </div>
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
            ...
            {revealed && <button onClick={next} style={{ background: '#16a34a', color: 'white', padding: '12px 28px', borderRadius: '10px', border: 'none', fontSize: '16px', fontWeight: 'bold', cursor: 'pointer' }}>{current + 1 >= questions.length ? '🏆 See Final Scores' : 'Next Question →'}</button>}
            <Link href="/esl-games/live/premium/science" style={{ background: 'rgba(255,255,255,0.1)', color: 'white', padding: '12px 28px', borderRadius: '10px', textDecoration: 'none', fontSize: '16px', fontWeight: 'bold' }}>✕ Exit</Link>
          </div>
        </div>
