...
function HostGame() {
  ...
  useEffect(() => { if (!authed) return; supabase.from('live_game_rooms').insert([{ code: roomCode, game_type: 'science-quiz', topic: ageGroup, status: 'waiting', current_question: 0, question_order: JSON.stringify(questions) }]).then(({ error }: any) => { if (error && error.code !== '23505') console.error(error.message) }) }, [authed])
  ...
  async function handleReveal() {
    ...
    if (roundAnswers) { for (const ans of roundAnswers) { if (ans.answer === correctAnswer) { const player = players.find((p: any) => p.id === ans.player_id); await supabase.from('live_game_players').update({ score: (player?.score || 0) + 1 }).eq('id', ans.player_id) } }; const { data }: any = await supabase.from('live_game_players').select('*').eq('room_code', roomCode); setPlayers(data || []) }
  }
  ...
  async function nextQuestion() {
    ...
    const next = current + 1; setCurrent(next); setRevealed(false); setRunning(true); setTimeLeft(15); setAnswers([])
  ...
  if (phase === 'lobby') return (
    <main style={{ fontFamily: 'sans-serif', background: '#f8f9fa', minHeight: '100vh', padding: '32px 24px' }}>
      <div style={{ maxWidth: '700px', margin: '0 auto' }}>
        <Link href="/esl-games/live/premium/science" style={{ color: '#888', textDecoration: 'none', fontSize: '14px' }}>← Back</Link>
        <div style={{ background: 'white', borderRadius: '20px', padding: '40px', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', marginTop: '20px', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', marginBottom: '24px' }}>
            <span style={{ background: '#f0fdf4', color: '#16a34a', padding: '6px 16px', borderRadius: '20px', fontSize: '13px', fontWeight: '600' }}>🔬 Science Quiz</span>
          ...
          <button onClick={startGame} disabled={players.length === 0} style={{ background: players.length > 0 ? '#16a34a' : '#cbd5e1', color: 'white', padding: '14px 48px', borderRadius: '12px', border: 'none', fontSize: '18px', fontWeight: 'bold', cursor: players.length > 0 ? 'pointer' : 'default', width: '100%' }}>
    ...
    <main style={{ fontFamily: 'sans-serif', background: '#1a1a2e', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ maxWidth: '600px', width: '100%', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
          <button onClick={clearRoom} style={{ background: '#16a34a', color: 'white', padding: '12px 28px', borderRadius: '10px', border: 'none', fontWeight: 'bold', fontSize: '16px', cursor: 'pointer' }}>🔄 New Game</button>
          <Link href="/esl-games/live/premium/science" style={{ background: 'rgba(255,255,255,0.15)', color: 'white', padding: '12px 28px', borderRadius: '10px', textDecoration: 'none', fontWeight: 'bold', fontSize: '16px' }}>Back to Science Quiz</Link>
        </div>
      </div>
    </main>
  )
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '10px' }}>
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            ...
            <span style={{ background: '#f0fdf4', color: '#16a34a', padding: '6px 16px', borderRadius: '20px', fontSize: '14px', fontWeight: '600' }}>🔬 {ageGroup}</span>
          </div>
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: '20px' }}>
          <div style={{ background: 'white', borderRadius: '20px', padding: '28px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
            ...
            <h2 style={{ fontSize: '20px', fontWeight: 'bold', color: '#1a1a2e', margin: '0 0 24px', lineHeight: '1.4' }}>{q.q}</h2>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
              {q.options.map((opt, i) => {
                ...
                return (
                  <div key={i} style={{ background: bg, border, borderRadius: '10px', padding: '12px', display: 'flex', alignItems: 'center', gap: '10px', transition: 'all 0.3s' }}>
                    <div style={{ width: '30px', height: '30px', background: revealed ? (isCorrect ? '#16a34a' : '#e2e8f0') : 'rgba(255,255,255,0.25)', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', color: 'white', fontSize: '13px', flexShrink: 0 }}>{optionLabels[i]}</div>
                    <span style={{ fontWeight: '600', fontSize: '14px', color }}>{opt}</span>
            ...
            <div style={{ display: 'flex', gap: '12px', marginTop: '20px' }}>
              {!revealed && <button onClick={handleReveal} style={{ background: '#E85D26', color: 'white', padding: '12px 24px', borderRadius: '10px', border: 'none', fontWeight: 'bold', fontSize: '15px', cursor: 'pointer', flex: 1 }}>💡 Reveal Answer ({answers.length}/{players.length} answered)</button>}
              {revealed && <button onClick={nextQuestion} style={{ background: '#16a34a', color: 'white', padding: '12px 24px', borderRadius: '10px', border: 'none', fontWeight: 'bold', fontSize: '15px', cursor: 'pointer', flex: 1 }}>{current + 1 >= questions.length ? '🏆 Final Scores' : 'Next Question →'}</button>}
            </div>
          </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {sorted.map((p: any, i: number) => (
                  <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', background: i === 0 ? '#fffbeb' : '#f8f9fa', borderRadius: '10px', border: i === 0 ? '1px solid #fde68a' : '1px solid transparent' }}>
                    ...
                    <span style={{ fontWeight: 'bold', fontSize: '16px', color: '#16a34a' }}>{p.score}</span>
                  </div>
                ))}
              </div>
            )}
