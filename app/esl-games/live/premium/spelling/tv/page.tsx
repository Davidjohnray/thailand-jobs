...
const teamColors = ['#db2777', '#2D6BE4', '#16a34a', '#7C3AED']
...
function TVGame() {
  ...
  function next() {
    ...
    setCurrent(c => c + 1); setRevealed(false); setTimeLeft(20); setRunning(true); setLastGiven(null)
  }
      ...
      <div style={{ maxWidth: '600px', margin: '0 auto' }}>
        ...
        <div style={{ background: 'white', borderRadius: '20px', padding: '36px', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', marginTop: '20px' }}>
          <div style={{ textAlign: 'center', marginBottom: '28px' }}>
            ...
            <p style={{ color: '#666', fontSize: '14px', marginTop: '6px' }}>🔤 Spelling Bee — {ageGroup}</p>
          </div>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              {[2, 3, 4].map(n => (
                <button key={n} onClick={() => setNumTeams(n)}
                  style={{ flex: 1, padding: '12px', borderRadius: '10px', border: `2px solid ${numTeams === n ? COLOR : '#e5e7eb'}`, background: numTeams === n ? '#fdf2f8' : 'white', fontWeight: 'bold', fontSize: '18px', cursor: 'pointer', color: numTeams === n ? COLOR : '#666' }}>
                  {n}
                </button>
              ))}
            </div>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px', flexWrap: 'wrap', gap: '8px' }}>
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            <span style={{ background: 'rgba(255,255,255,0.15)', color: 'white', padding: '6px 14px', borderRadius: '20px', fontSize: '14px', fontWeight: 'bold' }}>🔤 Spelling Bee</span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: '16px' }}>
          <div>
            ...
            <div style={{ background: '#fdf2f8', borderRadius: '14px', padding: '20px', marginBottom: '12px', borderLeft: `5px solid ${COLOR}` }}>
              <p style={{ color: '#888', fontSize: '11px', margin: '0 0 6px', textTransform: 'uppercase', letterSpacing: '1px' }}>Definition</p>
              <p style={{ fontWeight: 'bold', fontSize: '22px', color: '#1a1a2e', margin: 0, lineHeight: '1.4' }}>{q.q}</p>
            </div>
            <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: '13px', marginBottom: '10px', textAlign: 'center' }}>Which spelling is correct?</p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '14px' }}>
              {q.options.map((opt, i) => {
                ...
                return (
                  <div key={i} style={{ background: bg, border, borderRadius: '12px', padding: '16px', display: 'flex', alignItems: 'center', gap: '12px', transition: 'all 0.3s' }}>
                    <div style={{ width: '36px', height: '36px', background: revealed ? (isCorrect ? '#16a34a' : 'rgba(255,255,255,0.15)') : 'rgba(255,255,255,0.2)', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', color: 'white', fontSize: '16px', flexShrink: 0 }}>{optionLabels[i]}</div>
                    <span style={{ fontWeight: '700', fontSize: '18px', color: textColor, fontFamily: 'monospace' }}>{opt}</span>
