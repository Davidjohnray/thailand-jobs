  ...
  if (finished) return (
    <main style={{ minHeight: '100vh', background: '#1a1a2e', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ maxWidth: '500px', width: '100%', textAlign: 'center' }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '18px', marginBottom: '8px' }}>You scored</p>
      ...
      <div style={{ maxWidth: '640px', margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '8px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            <span style={{ background: '#fdf2f8', color: COLOR, padding: '6px 14px', borderRadius: '20px', fontSize: '14px', fontWeight: '600' }}>Q{current + 1}/{questions.length}</span>
        ...
        <div style={{ background: 'white', borderRadius: '20px', padding: '28px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
            <span style={{ color: '#888', fontSize: '14px' }}>🔤 Spelling Bee — {ageGroup}</span>
          ...
          <div style={{ background: '#fdf2f8', borderRadius: '12px', padding: '16px', marginBottom: '20px', borderLeft: `4px solid ${COLOR}` }}>
            <p style={{ color: '#888', fontSize: '12px', margin: '0 0 4px', textTransform: 'uppercase', letterSpacing: '1px' }}>Definition</p>
            <p style={{ fontWeight: 'bold', fontSize: '18px', color: '#1a1a2e', margin: 0, lineHeight: '1.4' }}>{q.q}</p>
          </div>
          <p style={{ color: '#888', fontSize: '14px', marginBottom: '12px', textAlign: 'center' }}>Which spelling is correct?</p>
            ...
            {q.options.map((opt, i) => {
              ...
              return (
                <button key={i} onClick={() => handleSelect(opt)} disabled={revealed}
                  style={{ background: bg, border, borderRadius: '10px', padding: '16px', display: 'flex', alignItems: 'center', gap: '10px', cursor: revealed ? 'default' : 'pointer', textAlign: 'left' }}>
                  <div style={{ width: '30px', height: '30px', background: revealed ? (isCorrect ? '#16a34a' : isSelected ? '#ef4444' : '#e2e8f0') : 'rgba(255,255,255,0.25)', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', color: 'white', fontSize: '13px', flexShrink: 0 }}>{optionLabels[i]}</div>
