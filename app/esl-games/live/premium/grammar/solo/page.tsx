...
function SoloGame() {
  ...
  function next() {
    ...
    setCurrent(c => c + 1); setSelected(null); setRevealed(false); setTimeLeft(25); setRunning(true)
  }
    ...
    <main style={{ minHeight: '100vh', background: '#1a1a2e', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ maxWidth: '500px', width: '100%', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
          <button onClick={() => { setCurrent(0); setScore(0); setSelected(null); setRevealed(false); setTimeLeft(25); setRunning(true); setFinished(false) }}
      ...
      <div style={{ maxWidth: '720px', margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '8px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            <span style={{ background: '#f5f3ff', color: COLOR, padding: '6px 14px', borderRadius: '20px', fontSize: '14px', fontWeight: '600' }}>Q{current + 1}/{questions.length}</span>
        ...
        <div style={{ background: 'white', borderRadius: '20px', padding: '28px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
            <span style={{ color: '#888', fontSize: '14px' }}>✏️ Grammar Challenge — {ageGroup}</span>
          ...
          <div style={{ background: '#f5f3ff', borderRadius: '12px', padding: '18px', marginBottom: '20px', borderLeft: `4px solid ${COLOR}` }}>
            ...
            {q.options.map((opt, i) => {
              ...
              return (
                <button key={i} onClick={() => handleSelect(opt)} disabled={revealed}
                  ...
                  {revealed && isSelected && !isCorrect && <span style={{ marginLeft: 'auto', flexShrink: 0 }}>❌</span>}
                </button>
              )
            })}
          </div>
