...
const COLOR = '#7C3AED'
const optionColors = [COLOR, '#2D6BE4', '#E85D26', '#16a34a']
const optionLabels = ['A', 'B', 'C', 'D']
const teamColors = ['#7C3AED', '#2D6BE4', '#E85D26', '#16a34a']
...
function TVGame() {
  ...
  function next() {
    ...
    setCurrent(c => c + 1); setRevealed(false); setTimeLeft(25); setRunning(false); setLastGiven(null)
  }
  ...
  if (phase === 'setup') return (
    <main style={{ background: '#f8f9fa', minHeight: '100vh', padding: '32px 24px' }}>
      <div style={{ maxWidth: '600px', margin: '0 auto' }}>
        <Link href="/esl-games/live/premium/grammar" style={{ color: '#888', textDecoration: 'none', fontSize: '14px' }}>← Back</Link>
        <div style={{ background: 'white', borderRadius: '20px', padding: '36px', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', marginTop: '20px' }}>
          <div style={{ textAlign: 'center', marginBottom: '28px' }}>
            ...
            <p style={{ color: '#666', fontSize: '14px', marginTop: '6px' }}>✏️ Grammar Challenge — {ageGroup}</p>
          </div>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              {[2, 3, 4].map(n => (
                <button key={n} onClick={() => setNumTeams(n)}
                  style={{ flex: 1, padding: '12px', borderRadius: '10px', border: `2px solid ${numTeams === n ? COLOR : '#e5e7eb'}`, background: numTeams === n ? '#f5f3ff' : 'white', fontWeight: 'bold', fontSize: '18px', cursor: 'pointer', color: numTeams === n ? COLOR : '#666' }}>
                  {n}
                </button>
              ))}
            </div>
    ...
    <main style={{ background: '#1a1a2e', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ maxWidth: '600px', width: '100%', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
          <button onClick={() => { setPhase('setup'); setCurrent(0); setTeamScores([0, 0, 0, 0]); setRevealed(false); setRunning(false); setLastGiven(null) }}
          ...
          <Link href="/esl-games/live/premium/grammar" style={{ background: 'rgba(255,255,255,0.15)', color: 'white', padding: '12px 28px', borderRadius: '10px', textDecoration: 'none', fontWeight: 'bold', fontSize: '16px' }}>Back</Link>
        </div>
      </div>
    </main>
  )
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px', flexWrap: 'wrap', gap: '8px' }}>
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            <span style={{ background: 'rgba(255,255,255,0.15)', color: 'white', padding: '6px 14px', borderRadius: '20px', fontSize: '14px', fontWeight: 'bold' }}>✏️ Grammar Challenge</span>
          ...
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            ...
            <span style={{ fontSize: '20px', fontWeight: 'bold', color: running ? timerColor : 'rgba(255,255,255,0.4)' }}>{timeLeft}s</span>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: '16px' }}>
          <div>
            <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: '16px', padding: '8px', marginBottom: '12px' }}>
              <div style={{ background: running ? timerColor : 'rgba(255,255,255,0.1)', height: '6px', borderRadius: '6px', width: running ? `${(timeLeft / 25) * 100}%` : '100%', transition: running ? 'width 1s linear' : 'none' }} />
            </div>

            <div style={{ background: '#f5f3ff', borderRadius: '14px', padding: '20px', marginBottom: '12px', borderLeft: `5px solid ${COLOR}` }}>
              <p style={{ fontWeight: 'bold', fontSize: '20px', color: '#1a1a2e', margin: 0, lineHeight: '1.5' }}>{q.q}</p>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '14px' }}>
              {q.options.map((opt, i) => {
                ...
                return (
                  <div key={i} style={{ background: bg, border, borderRadius: '12px', padding: '13px 18px', display: 'flex', alignItems: 'center', gap: '12px', transition: 'all 0.3s' }}>
                    ...
                    <span style={{ fontWeight: '600', fontSize: '14px', color: textColor, lineHeight: '1.4' }}>{opt}</span>
                    {revealed && isCorrect && <span style={{ marginLeft: 'auto', fontSize: '20px', flexShrink: 0 }}>✅</span>}
                  </div>
                )
              })}
            </div>

            {/* 3-state button — resets to ▶ Start Timer for every question */}
            <div style={{ display: 'flex', gap: '10px' }}>
              {!running && !revealed && (
                <button onClick={() => setRunning(true)}
                  style={{ background: '#1a1a2e', color: 'white', padding: '14px 24px', borderRadius: '10px', border: '2px solid rgba(255,255,255,0.2)', fontWeight: 'bold', fontSize: '16px', cursor: 'pointer', flex: 1 }}>
                  ▶ Start Timer
                </button>
              )}
              {running && !revealed && (
                <button onClick={() => { setRevealed(true); setRunning(false) }}
                  style={{ background: COLOR, color: 'white', padding: '14px 24px', borderRadius: '10px', border: 'none', fontWeight: 'bold', fontSize: '16px', cursor: 'pointer', flex: 1 }}>
                  💡 Reveal Answer
                </button>
              )}
              {revealed && (
                <button onClick={next}
                  style={{ background: '#16a34a', color: 'white', padding: '14px 24px', borderRadius: '10px', border: 'none', fontWeight: 'bold', fontSize: '16px', cursor: 'pointer', flex: 1 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {activeTeams.map((name, i) => (
              <div key={i} style={{ background: 'white', borderRadius: '14px', padding: '16px 18px', border: `2px solid ${lastGiven === i ? teamColors[i] : 'transparent'}` }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: revealed ? '10px' : '0' }}>
