...
function HostGame() {
  ...
  useEffect(() => { if (!authed) return; supabase.from('live_game_rooms').insert([{ code: roomCode, game_type: 'grammar-quiz', topic: ageGroup, status: 'waiting', current_question: 0, question_order: JSON.stringify(questions) }]).then(({ error }: any) => { if (error && error.code !== '23505') console.error(error.message) }) }, [authed])
  ...
  async function nextQuestion() {
    ...
    const next = current + 1; setCurrent(next); setRevealed(false); setRunning(true); setTimeLeft(25); setAnswers([])
      ...
      <div style={{ maxWidth: '700px', margin: '0 auto' }}>
        ...
        <div style={{ background: 'white', borderRadius: '20px', padding: '40px', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', marginTop: '20px', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', marginBottom: '24px' }}>
            <span style={{ background: '#f5f3ff', color: COLOR, padding: '6px 16px', borderRadius: '20px', fontSize: '13px', fontWeight: '600' }}>✏️ Grammar Challenge</span>
      ...
      <div style={{ maxWidth: '960px', margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '10px' }}>
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            ...
            <span style={{ background: '#f5f3ff', color: COLOR, padding: '6px 16px', borderRadius: '20px', fontSize: '14px', fontWeight: '600' }}>✏️ {ageGroup}</span>
          </div>
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            ...
            <span style={{ background: '#f5f3ff', color: COLOR, padding: '6px 16px', borderRadius: '20px', fontSize: '14px', fontWeight: '600' }}>Q{current + 1}/{questions.length}</span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: '20px' }}>
          <div style={{ background: 'white', borderRadius: '20px', padding: '28px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
            ...
            <div style={{ background: '#f0f0f0', borderRadius: '6px', height: '6px', marginBottom: '16px' }}>
              <div style={{ background: timerColor, height: '6px', borderRadius: '6px', width: `${(timeLeft / 25) * 100}%`, transition: 'width 1s linear' }} />
            </div>
            <div style={{ background: '#f5f3ff', borderRadius: '12px', padding: '18px', marginBottom: '16px', borderLeft: `4px solid ${COLOR}` }}>
              <p style={{ fontWeight: 'bold', fontSize: '17px', color: '#1a1a2e', margin: 0, lineHeight: '1.5' }}>{q.q}</p>
            </div>
              ...
              {q.options.map((opt, i) => {
                ...
                return (
                  <div key={i} style={{ background: bg, border, borderRadius: '10px', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: '10px', transition: 'all 0.3s' }}>
                    ...
                    <span style={{ fontWeight: '600', fontSize: '13px', color: textColor, lineHeight: '1.4' }}>{opt}</span>
                    {revealed && isCorrect && <span style={{ marginLeft: 'auto', flexShrink: 0 }}>✅</span>}
                  </div>
                )
              })}
            </div>
