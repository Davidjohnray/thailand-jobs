...
function ElectricityTVInner() {
  ...
  const startTimer = () => { setTimeLeft(timerSetting); setRevealed(false); setTimerRunning(true) }
  ...
  const applyCustomTime = () => {
    ...
    if (!isNaN(val) && val >= 5 && val <= 300) { setTimerSetting(val); setCustomInput('') }
  }

  const nextQuestion = () => {
    if (current + 1 >= qs.length) { setPhase('finished'); return }
    setCurrent(c => c + 1); setTimeLeft(timerSetting); setTimerRunning(false); setRevealed(false)
  }
  ...
  if (phase === 'setup') return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #1a1a2e, #1e3a5f)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ background: 'white', borderRadius: '32px', padding: '48px 40px', maxWidth: '520px', width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.4)', border: '3px solid #f59e0b' }}>
        ...
        {/* Timer setting */}
        <div style={{ background: '#f9fafb', border: '2px solid #e5e7eb', borderRadius: '16px', padding: '18px', marginBottom: '28px' }}>
          <p style={{ fontWeight: '800', color: '#374151', marginBottom: '12px', fontSize: '15px' }}>⏱️ Timer Per Question</p>
          <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', flexWrap: 'wrap', marginBottom: '12px' }}>
            {TIMER_PRESETS.map(t => (
              <button key={t} onClick={() => setTimerSetting(t)}
                style={{ padding: '8px 16px', borderRadius: '10px', border: '2px solid', borderColor: timerSetting === t ? '#f59e0b' : '#e5e7eb', background: timerSetting === t ? '#f59e0b' : 'white', color: timerSetting === t ? 'white' : '#374151', fontWeight: '800', fontSize: '15px', cursor: 'pointer', minWidth: '52px' }}>
                {t}s
              </button>
            ))}
          </div>
          <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', alignItems: 'center' }}>
            <input type="number" min="5" max="300" value={customInput} onChange={e => setCustomInput(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && applyCustomTime()}
              placeholder="Custom (5–300s)"
              style={{ padding: '8px 12px', borderRadius: '10px', border: '2px solid #e5e7eb', fontSize: '14px', width: '140px', outline: 'none', textAlign: 'center' }} />
            <button onClick={applyCustomTime}
              style={{ padding: '8px 16px', borderRadius: '10px', border: 'none', background: '#e5e7eb', color: '#374151', fontWeight: '700', fontSize: '13px', cursor: 'pointer' }}>
              Set
            </button>
          </div>
        ...
        <button onClick={() => { setTimeLeft(timerSetting); setPhase('game') }}
          style={{ width: '100%', padding: '18px', borderRadius: '16px', border: 'none', background: 'linear-gradient(135deg, #f59e0b, #d97706)', color: 'white', fontWeight: '900', fontSize: '20px', cursor: 'pointer', boxShadow: '0 8px 28px rgba(245,158,11,0.4)' }}>
          ⚡ Start the Quiz!
        </button>
      </div>
    </main>
    ...
    return (
      <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #1a1a2e, #1e3a5f)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
        <div style={{ background: 'white', borderRadius: '32px', padding: '48px', maxWidth: '560px', width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.4)', border: '3px solid #f59e0b' }}>
          ...
          {sorted.map((t, i) => (
            <div key={i} style={{ background: i === 0 ? '#fffbeb' : '#f9fafb', borderRadius: '16px', padding: '16px 24px', marginBottom: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: i === 0 ? '3px solid #fde68a' : '2px solid #e5e7eb' }}>
              <span style={{ fontWeight: '800', fontSize: '20px', color: '#1a1a2e' }}>{['🥇','🥈','🥉','4️⃣'][i]} {t.emoji} {t.name}</span>
              <span style={{ fontWeight: '900', fontSize: '28px', color: TEAM_COLORS[i] }}>{t.score}</span>
            </div>
          ))}
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', marginTop: '24px' }}>
            <button onClick={() => { setCurrent(0); setTeams(DEFAULT_TEAMS); setTimeLeft(timerSetting); setPhase('setup'); setRevealed(false); setTimerRunning(false) }}
              style={{ background: 'linear-gradient(135deg, #f59e0b, #d97706)', color: 'white', padding: '14px 28px', borderRadius: '16px', border: 'none', fontWeight: '800', fontSize: '16px', cursor: 'pointer' }}>
              Play Again!
            </button>
    ...
    <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #1a1a2e 0%, #1e3a5f 100%)', display: 'flex', flexDirection: 'column', padding: '16px', gap: '12px' }}>
      ...
      <div style={{ background: 'rgba(255,255,255,0.1)', borderRadius: '16px', padding: '10px 20px', display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        {timerRunning && (
          <button onClick={() => setTimerRunning(false)}
            style={{ background: 'rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.7)', border: 'none', padding: '10px 16px', borderRadius: '12px', fontWeight: '800', fontSize: '14px', cursor: 'pointer' }}>
            ⏸ Pause
          </button>
        )}
        {/* Show current timer setting */}
        <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: '12px', whiteSpace: 'nowrap' }}>⏱ {timerSetting}s</span>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
        {q.options.map((opt, idx) => {
          ...
          if (revealed) {
            if (isCorrect) { bg = '#dcfce7'; border = '3px solid #16a34a' }
            else { bg = 'rgba(255,255,255,0.05)'; border = '3px solid rgba(255,255,255,0.1)' }
          }
          return (
      ...
      <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
        {!revealed
          ...
          : <button onClick={nextQuestion} style={{ background: 'linear-gradient(135deg, #1d4ed8, #3b82f6)', color: 'white', padding: '14px 40px', borderRadius: '16px', border: 'none', fontWeight: '800', fontSize: '18px', cursor: 'pointer', boxShadow: '0 6px 20px rgba(29,78,216,0.4)' }}>
