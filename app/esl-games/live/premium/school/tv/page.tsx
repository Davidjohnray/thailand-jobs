...
const TOTAL_TIME = 20
const TEAM_COLORS = ['#f97316', '#eab308', '#3b82f6', '#22c55e']
...
function SchoolTVInner() {
  ...
  if (phase === 'setup') return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #fff7ed, #fef9c3, #f0fdf4)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ background: 'white', borderRadius: '32px', padding: '48px 40px', maxWidth: '520px', width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.12)', border: '3px solid #fdba74' }}>
        <div style={{ fontSize: '64px', marginBottom: '12px' }}>📺</div>
        <h1 style={{ fontSize: '28px', fontWeight: '900', color: '#7c2d12', marginBottom: '6px' }}>TV Classroom Mode</h1>
        <p style={{ color: '#f97316', fontSize: '15px', marginBottom: '32px', fontWeight: '700' }}>🏫 School — {qs.length} questions</p>
        <div style={{ marginBottom: '24px' }}>
          <p style={{ fontWeight: '800', color: '#374151', marginBottom: '12px' }}>How many teams?</p>
          <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
            {[2,3,4].map(n => <button key={n} onClick={() => setNumTeams(n)} style={{ padding: '12px 24px', borderRadius: '12px', border: '3px solid', borderColor: numTeams === n ? '#f97316' : '#fed7aa', background: numTeams === n ? '#f97316' : '#fff7ed', color: numTeams === n ? 'white' : '#7c2d12', fontWeight: '800', fontSize: '18px', cursor: 'pointer' }}>{n}</button>)}
          </div>
        </div>
        ...
        <button onClick={() => setPhase('game')} style={{ width: '100%', padding: '18px', borderRadius: '16px', border: 'none', background: 'linear-gradient(135deg, #f97316, #eab308)', color: 'white', fontWeight: '900', fontSize: '20px', cursor: 'pointer', boxShadow: '0 8px 28px rgba(249,115,22,0.4)' }}>
          🏫 Time for School!
        </button>
      </div>
    </main>
  ...
  if (phase === 'finished') {
    ...
    return (
      <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #fff7ed, #fef9c3, #f0fdf4)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
        <div style={{ background: 'white', borderRadius: '32px', padding: '48px', maxWidth: '560px', width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.12)', border: '3px solid #fdba74' }}>
          <div style={{ fontSize: '80px', marginBottom: '16px' }}>🏆</div>
          <h1 style={{ fontSize: '36px', fontWeight: '900', color: '#7c2d12', marginBottom: '24px' }}>Game Over!</h1>
          {sorted.map((t, i) => <div key={i} style={{ background: i === 0 ? '#fff7ed' : '#f9fafb', borderRadius: '16px', padding: '16px 24px', marginBottom: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: i === 0 ? '3px solid #fdba74' : '2px solid #e5e7eb' }}><span style={{ fontWeight: '800', fontSize: '20px', color: '#7c2d12' }}>{['🥇','🥈','🥉','4️⃣'][i]} {t.emoji} {t.name}</span><span style={{ fontWeight: '900', fontSize: '28px', color: TEAM_COLORS[i] }}>{t.score}</span></div>)}
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', marginTop: '24px' }}>
            <button onClick={() => { setCurrent(0); setTeams(DEFAULT_TEAMS); setPhase('setup'); setRevealed(false); setTimerRunning(false) }} style={{ background: 'linear-gradient(135deg, #f97316, #eab308)', color: 'white', padding: '14px 28px', borderRadius: '16px', border: 'none', fontWeight: '800', fontSize: '16px', cursor: 'pointer' }}>Play Again!</button>
            <Link href="/esl-games/live/premium/school" style={{ background: '#fff7ed', color: '#7c2d12', padding: '14px 24px', borderRadius: '16px', textDecoration: 'none', fontWeight: '700', fontSize: '15px' }}>← Back</Link>
          </div>
        </div>
      </main>
    )
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #fff7ed 0%, #fef9c3 50%, #f0fdf4 100%)', display: 'flex', flexDirection: 'column', padding: '16px', gap: '12px' }}>
      <style>{`@keyframes schoolBounce{0%,100%{transform:scale(1)}50%{transform:scale(1.1)}}`}</style>
      <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap' }}>
        {teams.slice(0, numTeams).map((team, idx) => (
          <div key={idx} style={{ background: 'white', borderRadius: '20px', padding: '10px 16px', display: 'flex', alignItems: 'center', gap: '10px', boxShadow: '0 4px 16px rgba(0,0,0,0.08)', border: `3px solid ${TEAM_COLORS[idx]}30` }}>
            ...
            <span style={{ fontWeight: '800', color: '#7c2d12', fontSize: '15px' }}>{team.name}</span>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <button onClick={() => removePoint(idx)} style={{ background: '#fff7ed', border: 'none', width: '28px', height: '28px', borderRadius: '8px', cursor: 'pointer', fontWeight: '800', fontSize: '16px', color: '#f97316' }}>−</button>
      ...
      <div style={{ background: 'white', borderRadius: '16px', padding: '10px 20px', display: 'flex', alignItems: 'center', gap: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
        <span style={{ fontWeight: '800', fontSize: '14px', color: '#f97316' }}>Q{current + 1}/{qs.length}</span>
        <div style={{ flex: 1, background: '#fff7ed', borderRadius: '8px', height: '12px', overflow: 'hidden' }}>
        ...
        {timerRunning && <button onClick={() => setTimerRunning(false)} style={{ background: '#fff7ed', color: '#f97316', border: 'none', padding: '10px 16px', borderRadius: '12px', fontWeight: '800', fontSize: '14px', cursor: 'pointer' }}>⏸ Pause</button>}
      </div>
      <div style={{ background: 'white', borderRadius: '28px', padding: '24px', textAlign: 'center', boxShadow: '0 8px 32px rgba(0,0,0,0.1)', border: '3px solid #fdba74', flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ fontSize: '120px', animation: 'schoolBounce 2.5s ease-in-out infinite', marginBottom: '12px' }}>{q.emoji}</div>
        <h2 style={{ fontSize: '30px', fontWeight: '900', color: '#7c2d12', margin: 0 }}>{q.question}</h2>
      </div>
      ...
      <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
        {!revealed ? <button onClick={revealAnswer} style={{ background: 'linear-gradient(135deg, #f97316, #eab308)', color: 'white', padding: '14px 36px', borderRadius: '16px', border: 'none', fontWeight: '800', fontSize: '18px', cursor: 'pointer', boxShadow: '0 6px 20px rgba(249,115,22,0.4)' }}>👁 Show Answer!</button>
          : <button onClick={nextQuestion} style={{ background: 'linear-gradient(135deg, #eab308, #f97316)', color: 'white', padding: '14px 36px', borderRadius: '16px', border: 'none', fontWeight: '800', fontSize: '18px', cursor: 'pointer', boxShadow: '0 6px 20px rgba(234,179,8,0.4)' }}>{current + 1 >= qs.length ? '🏁 See Results!' : 'Next! →'}</button>}
        <Link href="/esl-games/live/premium/school" style={{ background: 'white', color: '#f97316', padding: '14px 20px', borderRadius: '16px', textDecoration: 'none', fontWeight: '700', fontSize: '15px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>Exit</Link>
      </div>
    </main>
  )
}
