...
// Vocabulary lesson — all school words
const VOCAB = [
  { emoji: '✏️', word: 'PENCIL', spelling: 'P – E – N – C – I – L', sentence: 'I write with a pencil.', tip: 'You use it to write and draw!' },
  { emoji: '📚', word: 'BOOK', spelling: 'B – O – O – K', sentence: 'I read my book.', tip: 'Books have words and pictures inside.' },
  { emoji: '🎒', word: 'BAG', spelling: 'B – A – G', sentence: 'My bag is heavy.', tip: 'You carry your books in your bag.' },
  { emoji: '📏', word: 'RULER', spelling: 'R – U – L – E – R', sentence: 'I measure with a ruler.', tip: 'A ruler helps you draw straight lines.' },
  { emoji: '✂️', word: 'SCISSORS', spelling: 'S – C – I – S – S – O – R – S', sentence: 'I cut paper with scissors.', tip: 'Scissors have two sharp blades.' },
  { emoji: '🩹', word: 'ERASER', spelling: 'E – R – A – S – E – R', sentence: 'I rub out mistakes with an eraser.', tip: 'Also called a rubber! It removes pencil marks.' },
  { emoji: '🫧', word: 'GLUE', spelling: 'G – L – U – E', sentence: 'I stick paper with glue.', tip: 'Glue is sticky — it joins things together.' },
  { emoji: '🖍️', word: 'CRAYON', spelling: 'C – R – A – Y – O – N', sentence: 'I colour with a crayon.', tip: 'Crayons come in many colours!' },
  { emoji: '📓', word: 'NOTEBOOK', spelling: 'N – O – T – E – B – O – O – K', sentence: 'I write in my notebook.', tip: 'A notebook is for writing your notes.' },
  { emoji: '🖩', word: 'CALCULATOR', spelling: 'C – A – L – C – U – L – A – T – O – R', sentence: 'I use a calculator for maths.', tip: 'A calculator helps you add and subtract numbers.' },
  { emoji: '👩‍🏫', word: 'TEACHER', spelling: 'T – E – A – C – H – E – R', sentence: 'My teacher is kind.', tip: 'The teacher helps you learn new things.' },
  { emoji: '🏫', word: 'CLASSROOM', spelling: 'C – L – A – S – S – R – O – O – M', sentence: 'I learn in the classroom.', tip: 'This is where you study at school.' },
  { emoji: '📋', word: 'BLACKBOARD', spelling: 'B – L – A – C – K – B – O – A – R – D', sentence: 'The teacher writes on the blackboard.', tip: 'Teachers write on the blackboard with chalk.' },
  { emoji: '🪑', word: 'DESK', spelling: 'D – E – S – K', sentence: 'I sit at my desk.', tip: 'You sit at a desk to do your work.' },
  { emoji: '👨‍🎓', word: 'STUDENT', spelling: 'S – T – U – D – E – N – T', sentence: 'I am a good student.', tip: 'A student is someone who goes to school to learn.' },
]
...
export default function SchoolPage() {
  ...
  const nextWord = () => {
    if (lessonIdx + 1 >= VOCAB.length) {
      ...
      router.push(`/esl-games/live/premium/school/${mode}`)
    } else {
      setLessonIdx(i => i + 1)
      setShowSpelling(false)
      setShowSentence(false)
    ...
    return (
      <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #fff7ed 0%, #fef9c3 50%, #f0fdf4 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
        ...
        <div style={{ width: '100%', maxWidth: '560px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          ...
          <div style={{ background: 'white', padding: '8px 16px', borderRadius: '20px', fontWeight: '800', fontSize: '14px', color: '#f97316', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
        ...
        <div style={{ display: 'flex', gap: '6px', justifyContent: 'center', marginBottom: '16px', flexWrap: 'wrap', maxWidth: '560px' }}>
          {VOCAB.map((_, i) => (
            <div key={i} style={{ width: '10px', height: '10px', borderRadius: '50%', background: i < lessonIdx ? '#22c55e' : i === lessonIdx ? '#f97316' : '#e5e7eb', transition: 'background 0.3s' }} />
          ))}
        </div>
        ...
        <div key={lessonIdx} style={{ background: 'white', borderRadius: '32px', padding: '36px 32px', maxWidth: '560px', width: '100%', textAlign: 'center', boxShadow: '0 12px 40px rgba(0,0,0,0.12)', border: '3px solid #fdba74', animation: 'wordPop 0.35s ease', marginBottom: '16px' }}>
          ...
          {/* Word */}
          <div style={{ fontSize: '42px', fontWeight: '900', color: '#7c2d12', marginBottom: '8px', letterSpacing: '3px' }}>{v.word}</div>
              ...
              <div style={{ background: '#fff7ed', border: '2px solid #fdba74', borderRadius: '16px', padding: '16px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', flexWrap: 'wrap' }}>
                  {v.word.split('').map((letter, i) => (
                    <div key={i} style={{ background: 'white', border: '2px solid #f97316', borderRadius: '10px', width: '38px', height: '44px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: '900', fontSize: '20px', color: '#f97316', boxShadow: '0 2px 6px rgba(249,115,22,0.2)' }}>
                      {letter}
                    </div>
                  ))}
                </div>
        ...
        <div style={{ display: 'flex', gap: '12px', width: '100%', maxWidth: '560px' }}>
          {lessonIdx > 0 && (
            <button onClick={prevWord}
              style={{ background: 'white', border: '2px solid #fdba74', color: '#7c2d12', padding: '14px 24px', borderRadius: '16px', fontWeight: '800', fontSize: '16px', cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
              ← Back
            </button>
          )}
      ...
      <div style={{ background: 'white', borderRadius: '32px', padding: '48px 40px', maxWidth: '480px', width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.12)', border: '3px solid #fdba74', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ background: '#fff7ed', border: '2px solid #fdba74', borderRadius: '20px', padding: '20px', marginBottom: '16px', textAlign: 'left' }}>
          ...
          <p style={{ color: '#c2410c', fontSize: '13px', margin: '0 0 14px', lineHeight: '1.6' }}>
            See every school word with its picture, spelling and an example sentence before you play.
          </p>
