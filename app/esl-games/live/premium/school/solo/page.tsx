...
function SchoolSoloInner() {
  ...
  // Fix: check localStorage after mount, not in useState initializer
  useEffect(() => {
    setAuthed(!!localStorage.getItem(SESSION_KEY))
    setChecking(false)
  }, [])
  ...
  if (!authed) return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #fff7ed, #fef9c3)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <Link href="/esl-games/live/premium/school" style={{ color: '#f97316', fontWeight: 'bold' }}>← Please log in first</Link>
    </main>
  )
