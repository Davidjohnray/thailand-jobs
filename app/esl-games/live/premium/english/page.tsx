...
export default function EnglishQuizPage() {
  ...
  if (!authed) return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #1a1a2e, #2d2d4e)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ background: 'white', borderRadius: '16px', padding: '40px', maxWidth: '420px', width: '100%', textAlign: 'center' }}>
        ...
        <Link href="/esl-games/live/premium" style={{ background: '#7C3AED', color: 'white', padding: '12px 28px', borderRadius: '8px', textDecoration: 'none', fontWeight: 'bold', fontSize: '15px', display: 'inline-block' }}>
          🔓 Get Premium Access
        </Link>
      </div>
    </main>
  ...
  return (
    <main style={{ fontFamily: 'sans-serif', background: '#f8f9fa', minHeight: '100vh' }}>
      <div style={{ background: 'linear-gradient(135deg, #7C3AED, #5b21b6)', padding: '52px 24px', color: 'white', textAlign: 'center' }}>
        ...
        <h1 style={{ fontSize: '36px', fontWeight: 'bold', margin: '0 0 10px' }}>English Quiz</h1>
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '40px 24px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
            {ageGroups.map(ag => (
              <button key={ag} onClick={() => setSelectedAge(ag)} style={{
                ...
                border: '2px solid', borderColor: selectedAge === ag ? '#7C3AED' : '#e2e8f0',
                background: selectedAge === ag ? '#f5f3ff' : 'white', color: selectedAge === ag ? '#7C3AED' : '#444',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <Link href={`/esl-games/live/premium/english/solo?ageGroup=${selectedAge}`} style={{ textDecoration: 'none' }}>
            <div style={{ background: 'white', borderRadius: '16px', padding: '24px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', display: 'flex', gap: '20px', alignItems: 'center', cursor: 'pointer', border: '2px solid transparent', transition: 'border-color 0.2s' }}
              ...
              <div style={{ background: '#7C3AED', color: 'white', padding: '10px 20px', borderRadius: '8px', fontWeight: 'bold', fontSize: '14px', whiteSpace: 'nowrap' as const }}>Play Solo →</div>
            </div>
          </Link>

          <Link href={`/esl-games/live/premium/english/host?ageGroup=${selectedAge}`} style={{ textDecoration: 'none' }}>
            <div style={{ background: 'white', borderRadius: '16px', padding: '24px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', display: 'flex', gap: '20px', alignItems: 'center', cursor: 'pointer', border: '2px solid transparent', transition: 'border-color 0.2s' }}
          ...
          <Link href={`/esl-games/live/premium/english/tv?ageGroup=${selectedAge}`} style={{ textDecoration: 'none' }}>
            <div style={{ background: 'white', borderRadius: '16px', padding: '24px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', display: 'flex', gap: '20px', alignItems: 'center', cursor: 'pointer', border: '2px solid transparent', transition: 'border-color 0.2s' }}
              ...
              <div style={{ flex: 1 }}>
                ...
                <p style={{ color: '#666', fontSize: '14px', margin: 0 }}>Show on a smart TV or projector. Students answer out loud — no phones needed.</p>
              </div>
