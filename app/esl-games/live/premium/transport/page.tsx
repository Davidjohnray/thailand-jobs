...
export default function TransportPage() {
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #dbeafe 0%, #eff6ff 50%, #fef9c3 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px', fontFamily: 'system-ui, sans-serif', position: 'relative', overflow: 'hidden' }}>
      <style>{`
      ...
      <div style={{ background: 'white', borderRadius: '32px', padding: '48px 40px', maxWidth: '480px', width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.12)', border: '3px solid #93c5fd', position: 'relative', zIndex: 1 }}>
        ...
        <p style={{ color: '#3b82f6', fontSize: '16px', marginBottom: '32px', fontWeight: '700' }}>Cars, planes, boats and more! 🌟</p>
        ...
        <button onClick={() => mode && router.push(`/esl-games/live/premium/transport/${mode}`)} disabled={!mode}
          style={{ width: '100%', padding: '18px', borderRadius: '16px', border: 'none', background: mode ? 'linear-gradient(135deg, #3b82f6, #f97316)' : '#e0f2fe', color: mode ? 'white' : '#93c5fd', fontWeight: '900', fontSize: '20px', cursor: mode ? 'pointer' : 'not-allowed', boxShadow: mode ? '0 8px 28px rgba(59,130,246,0.4)' : 'none', transition: 'all 0.2s' }}>
          {mode ? '🚗 Vroom! Let\'s Go!' : 'Choose a mode first!'}
        </button>
