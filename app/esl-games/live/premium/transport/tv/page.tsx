...
import { transportQuestions } from '../questions'

const SESSION_KEY = 'premium_games_session'
const TOTAL_TIME = 20
const TEAM_COLORS = ['#3b82f6', '#f97316', '#8b5cf6', '#22c55e']
const OPTION_COLORS = ['#3b82f6', '#f97316', '#8b5cf6', '#22c55e']
const OPTION_BG = ['#eff6ff', '#fff7ed', '#f5f3ff', '#f0fdf4']
...
function TransportTVInner() {
  const qs = transportQuestions
  ...
  if (!authed) return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #dbeafe, #eff6ff)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <Link href="/esl-games/live/premium/transport" style={{ color: '#3b82f6', fontWeight: 'bold' }}>← Please log in first</Link>
    </main>
  )

  if (phase === 'setup') return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #dbeafe, #eff6ff, #fef9c3)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ background: 'white', borderRadius: '32px', padding: '48px 40px', maxWidth: '520px', width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.12)', border: '3px solid #93c5fd' }}>
        ...
        <p style={{ color: '#3b82f6', fontSize: '15px', marginBottom: '32px', fontWeight: '700' }}>🚗 Transport — {qs.length} questions</p>
        <div style={{ marginBottom: '24px' }}>
          <p style={{ fontWeight: '800', color: '#374151', marginBottom: '12px' }}>How many teams?</p>
          <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
            {[2, 3, 4].map(n => (
              <button key={n} onClick={() => setNumTeams(n)}
                style={{ padding: '12px 24px', borderRadius: '12px', border: '3px solid', borderColor: numTeams === n ? '#3b82f6' : '#e0f2fe', background: numTeams === n ? '#3b82f6' : '#f0f9ff', color: numTeams === n ? 'white' : '#1e3a5f', fontWeight: '800', fontSize: '18px', cursor: 'pointer' }}>
                {n}
              </button>
            ))}
          </div>
        ...
        <button onClick={() => setPhase('game')}
          style={{ width: '100%', padding: '18px', borderRadius: '16px', border: 'none', background: 'linear-gradient(135deg, #3b82f6, #f97316)', color: 'white', fontWeight: '900', fontSize: '20px', cursor: 'pointer', boxShadow: '0 8px 28px rgba(59,130,246,0.4)' }}>
          🚗 Vroom! Start!
        </button>
      </div>
    </main>
    ...
    return (
      <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #dbeafe, #eff6ff, #fef9c3)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
        <div style={{ background: 'white', borderRadius: '32px', padding: '48px', maxWidth: '560px', width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.12)', border: '3px solid #93c5fd' }}>
          ...
          {sorted.map((t, i) => (
            <div key={i} style={{ background: i === 0 ? '#eff6ff' : '#f9fafb', borderRadius: '16px', padding: '16px 24px', marginBottom: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: i === 0 ? '3px solid #93c5fd' : '2px solid #e5e7eb' }}>
              <span style={{ fontWeight: '800', fontSize: '20px', color: '#1e3a5f' }}>{['🥇','🥈','🥉','4️⃣'][i]} {t.emoji} {t.name}</span>
          ...
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', marginTop: '24px' }}>
            <button onClick={() => { setCurrent(0); setTeams(DEFAULT_TEAMS); setPhase('setup'); setRevealed(false); setTimerRunning(false) }}
              style={{ background: 'linear-gradient(135deg, #3b82f6, #f97316)', color: 'white', padding: '14px 28px', borderRadius: '16px', border: 'none', fontWeight: '800', fontSize: '16px', cursor: 'pointer' }}>Play Again!</button>
            <Link href="/esl-games/live/premium/transport" style={{ background: '#eff6ff', color: '#1e3a5f', padding: '14px 24px', borderRadius: '16px', textDecoration: 'none', fontWeight: '700', fontSize: '15px' }}>← Back</Link>
          </div>
        </div>
      </main>
    )
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #dbeafe 0%, #eff6ff 50%, #fef9c3 100%)', display: 'flex', flexDirection: 'column', padding: '16px', gap: '12px' }}>
      <style>{`@keyframes drive{0%,100%{transform:translateX(-8px) scale(1)}50%{transform:translateX(8px) scale(1.05)}}`}</style>
      ...
      <div style={{ background: 'white', borderRadius: '16px', padding: '10px 20px', display: 'flex', alignItems: 'center', gap: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
        ...
        {timerRunning && (
          <button onClick={() => setTimerRunning(false)}
            style={{ background: '#eff6ff', color: '#3b82f6', border: 'none', padding: '10px 16px', borderRadius: '12px', fontWeight: '800', fontSize: '14px', cursor: 'pointer' }}>
            ⏸ Pause
          </button>
        )}
      </div>
      ...
      <div style={{ background: 'white', borderRadius: '28px', padding: '24px', textAlign: 'center', boxShadow: '0 8px 32px rgba(0,0,0,0.1)', border: '3px solid #93c5fd', flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ fontSize: '130px', animation: 'drive 2s ease-in-out infinite', marginBottom: '12px' }}>{q.emoji}</div>
      ...
      <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
        {!revealed ? (
          <button onClick={revealAnswer}
            style={{ background: 'linear-gradient(135deg, #f97316, #3b82f6)', color: 'white', padding: '14px 36px', borderRadius: '16px', border: 'none', fontWeight: '800', fontSize: '18px', cursor: 'pointer', boxShadow: '0 6px 20px rgba(249,115,22,0.4)' }}>
            👁 Show Answer!
          </button>
        ) : (
          <button onClick={nextQuestion}
            style={{ background: 'linear-gradient(135deg, #3b82f6, #f97316)', color: 'white', padding: '14px 36px', borderRadius: '16px', border: 'none', fontWeight: '800', fontSize: '18px', cursor: 'pointer', boxShadow: '0 6px 20px rgba(59,130,246,0.4)' }}>
            {current + 1 >= qs.length ? '🏁 See Results!' : 'Next! →'}
          </button>
        )}
        <Link href="/esl-games/live/premium/transport" style={{ background: 'white', color: '#3b82f6', padding: '14px 20px', borderRadius: '16px', textDecoration: 'none', fontWeight: '700', fontSize: '15px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>Exit</Link>
      </div>
    </main>
  )
}

export default function TransportTVPage() {
  return (
    <Suspense fallback={<main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #dbeafe, #eff6ff)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontSize: '64px' }}>🚗</div></main>}>
      <TransportTVInner />
    </Suspense>
  )
}
