...
function ColoursSoloInner() {
  ...
  if (finished) {
    ...
    return (
      <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #fce7f3, #ede9fe, #dbeafe)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
        ...
        <div style={{ background: 'white', borderRadius: '32px', padding: '48px', maxWidth: '480px', width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.12)', border: '3px solid #ddd6fe', animation: 'popIn 0.5s ease' }}>
          ...
          <p style={{ color: '#6b7280', fontSize: '18px', marginBottom: '24px' }}>You got <strong style={{ color: '#8b5cf6' }}>{score}</strong> out of <strong>{qs.length}</strong> right!</p>
          <div style={{ fontSize: '48px', fontWeight: '900', color: '#8b5cf6', marginBottom: '28px' }}>{pct}%</div>
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
            <button onClick={restart}
              style={{ background: 'linear-gradient(135deg, #8b5cf6, #ec4899)', color: 'white', padding: '14px 32px', borderRadius: '16px', border: 'none', fontWeight: '800', fontSize: '18px', cursor: 'pointer', boxShadow: '0 6px 20px rgba(139,92,246,0.4)' }}>
              🔄 Play Again!
            </button>
            <Link href="/esl-games/live/colours-shapes" style={{ background: '#f3f4f6', color: '#374151', padding: '14px 24px', borderRadius: '16px', textDecoration: 'none', fontWeight: '700', fontSize: '16px' }}>
              ← Back
            </Link>
          </div>
        </div>
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #fce7f3 0%, #ede9fe 50%, #dbeafe 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
      <style>{`
      ...
      <div style={{ width: '100%', maxWidth: '560px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <Link href="/esl-games/live/colours-shapes" style={{ background: 'white', padding: '8px 16px', borderRadius: '20px', textDecoration: 'none', color: '#6b7280', fontWeight: '700', fontSize: '14px', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>← Exit</Link>
          <div style={{ background: 'white', padding: '8px 20px', borderRadius: '20px', fontWeight: '900', fontSize: '18px', color: '#8b5cf6', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>⭐ {score}</div>
        </div>

        {/* Progress dots */}
        <div style={{ display: 'flex', gap: '6px', justifyContent: 'center', marginBottom: '16px', flexWrap: 'wrap' }}>
          {qs.map((_, i) => (
            <div key={i} style={{ width: '10px', height: '10px', borderRadius: '50%', background: i < current ? '#22c55e' : i === current ? '#8b5cf6' : '#e5e7eb', transition: 'background 0.3s' }} />
          ))}
        </div>
        ...
        {/* Question card */}
        <div style={{ background: 'white', borderRadius: '28px', padding: '32px', textAlign: 'center', marginBottom: '16px', boxShadow: '0 8px 32px rgba(0,0,0,0.1)', border: '3px solid #ddd6fe' }}>
          ...
          <div style={{ fontSize: '110px', marginBottom: '12px', animation: 'spinPop 2.5s ease-in-out infinite' }}>{q.emoji}</div>
          <h2 style={{ fontSize: '26px', fontWeight: '900', color: '#1e1b4b', margin: 0 }}>{q.question}</h2>
        </div>
          ...
          {q.options.map((opt, idx) => {
            ...
            return (
              <button key={idx} onClick={() => handleAnswer(idx)} disabled={selected !== null}
                ...
                <div style={{ fontWeight: '800', fontSize: '15px', color: border }}>{opt.name}</div>
