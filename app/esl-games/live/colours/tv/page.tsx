...
import { colourQuestions } from '../questions'

const TOTAL_TIME = 20
const OPTION_COLORS = ['#8b5cf6', '#ec4899', '#f59e0b', '#22c55e']
const OPTION_BG = ['#f5f3ff', '#fdf2f8', '#fffbeb', '#f0fdf4']
const TEAM_COLORS = ['#8b5cf6', '#ec4899', '#f59e0b', '#22c55e']
...
function ColoursTVInner() {
  ...
  if (phase === 'setup') return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #fce7f3, #ede9fe, #dbeafe)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ background: 'white', borderRadius: '32px', padding: '48px 40px', maxWidth: '520px', width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.12)', border: '3px solid #ddd6fe' }}>
        <div style={{ fontSize: '64px', marginBottom: '12px' }}>🌈</div>
        <h1 style={{ fontSize: '28px', fontWeight: '900', color: '#1e1b4b', marginBottom: '6px' }}>TV Classroom Mode</h1>
        <p style={{ color: '#6b7280', fontSize: '15px', marginBottom: '32px' }}>Colours & Shapes — {qs.length} questions</p>
          ...
          <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
            {[2, 3, 4].map(n => (
              <button key={n} onClick={() => setNumTeams(n)}
                style={{ padding: '12px 24px', borderRadius: '12px', border: '3px solid', borderColor: numTeams === n ? '#8b5cf6' : '#e5e7eb', background: numTeams === n ? '#8b5cf6' : 'white', color: numTeams === n ? 'white' : '#374151', fontWeight: '800', fontSize: '18px', cursor: 'pointer' }}>
                {n}
              </button>
            ))}
          </div>
        ...
        <button onClick={() => setPhase('game')}
          style={{ width: '100%', padding: '18px', borderRadius: '16px', border: 'none', background: 'linear-gradient(135deg, #8b5cf6, #ec4899)', color: 'white', fontWeight: '900', fontSize: '20px', cursor: 'pointer', boxShadow: '0 8px 24px rgba(139,92,246,0.4)' }}>
          🎨 Start the Game!
        </button>
      </div>
    </main>
  ...
  if (phase === 'finished') {
    ...
    return (
      <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #fce7f3, #ede9fe, #dbeafe)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
        <div style={{ background: 'white', borderRadius: '32px', padding: '48px', maxWidth: '560px', width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.12)', border: '3px solid #ddd6fe' }}>
          <div style={{ fontSize: '80px', marginBottom: '16px' }}>🏆</div>
          <h1 style={{ fontSize: '36px', fontWeight: '900', color: '#1e1b4b', marginBottom: '24px' }}>Game Over!</h1>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '32px' }}>
            {sorted.map((t, i) => (
              <div key={i} style={{ background: i === 0 ? '#f5f3ff' : '#f9fafb', borderRadius: '16px', padding: '16px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: i === 0 ? '3px solid #ddd6fe' : '2px solid #e5e7eb' }}>
          ...
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
            <button onClick={() => { setCurrent(0); setTeams(DEFAULT_TEAMS); setPhase('setup'); setRevealed(false); setTimerRunning(false) }}
              style={{ background: 'linear-gradient(135deg, #8b5cf6, #ec4899)', color: 'white', padding: '14px 28px', borderRadius: '16px', border: 'none', fontWeight: '800', fontSize: '16px', cursor: 'pointer' }}>Play Again!</button>
            <Link href="/esl-games/live/colours-shapes" style={{ background: '#f3f4f6', color: '#374151', padding: '14px 24px', borderRadius: '16px', textDecoration: 'none', fontWeight: '700', fontSize: '15px' }}>← Back</Link>
          </div>
        </div>
      </main>
    )
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #fce7f3 0%, #ede9fe 50%, #dbeafe 100%)', display: 'flex', flexDirection: 'column', padding: '16px', gap: '12px' }}>
      <style>{`
      ...
      {/* QUESTION */}
      <div style={{ background: 'white', borderRadius: '28px', padding: '24px', textAlign: 'center', boxShadow: '0 8px 32px rgba(0,0,0,0.1)', border: '3px solid #ddd6fe', flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ fontSize: '130px', animation: 'spinPop 2.5s ease-in-out infinite', marginBottom: '16px' }}>{q.emoji}</div>
        <h2 style={{ fontSize: '34px', fontWeight: '900', color: '#1e1b4b', margin: 0 }}>{q.question}</h2>
      </div>
        ...
        {q.options.map((opt, idx) => {
          ...
          return (
            <div key={idx} style={{ background: bg, border, borderRadius: '20px', padding: '16px', textAlign: 'center', transition: 'all 0.3s', animation: anim }}>
              <div style={{ fontSize: '72px', marginBottom: '8px' }}>{opt.emoji}</div>
              <div style={{ fontWeight: '800', fontSize: '20px', color: revealed && isCorrect ? '#16a34a' : OPTION_COLORS[idx] }}>{opt.name}</div>
      ...
      <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
        {!revealed ? (
          ...
          <button onClick={nextQuestion}
            style={{ background: 'linear-gradient(135deg, #8b5cf6, #ec4899)', color: 'white', padding: '14px 36px', borderRadius: '16px', border: 'none', fontWeight: '800', fontSize: '18px', cursor: 'pointer', boxShadow: '0 6px 20px rgba(139,92,246,0.4)' }}>
            {current + 1 >= qs.length ? '🏁 See Results!' : 'Next One! →'}
          </button>
        )}
        <Link href="/esl-games/live/colours-shapes" style={{ background: 'white', color: '#6b7280', padding: '14px 20px', borderRadius: '16px', textDecoration: 'none', fontWeight: '700', fontSize: '15px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>Exit</Link>
      </div>
    </main>
  )
}
...
export default function ColoursTVPage() {
  return (
    <Suspense fallback={
      <main style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #fce7f3, #ede9fe, #dbeafe)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
