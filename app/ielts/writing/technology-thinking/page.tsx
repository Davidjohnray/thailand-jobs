...
const TOPIC = 'Technology & Independent Thinking'
const ESSAY_TYPE = 'Opinion Essay'
const TASK = 'Some people believe that technology is making people less able to think for themselves. To what extent do you agree or disagree?'

const ESSAY_TYPE_GUIDE = {
  what: 'An opinion essay asks you to give your own view and defend it with reasons and examples. You should take a clear position — fully agree, fully disagree, or partially agree — and maintain it throughout.',
  structure: [
    { label: 'Introduction (2–3 sentences)', desc: 'Paraphrase the statement. Give your thesis — state your position clearly.' },
    ...
    { label: 'Body Paragraph 2 (5–6 sentences)', desc: 'Second argument or acknowledge the other side then counter it. Topic sentence → explanation → example → link back.' },
    { label: 'Conclusion (2–3 sentences)', desc: 'Restate your position in different words. Summarise your two main points. Optional: final thought or recommendation.' },
  ],
  tips: [
...
const USEFUL_LANGUAGE = [
  ...
  { category: 'Giving Examples', phrases: ['This is exemplified by...', 'A clear illustration of this is...', 'For instance, research has shown that...', 'Take, for example, the case of...'] },
...
const PLAN_PROMPTS = {
  thesis: 'Your position — do you agree, disagree, or partially agree? State it in one sentence.',
  ...
  body2Topic: 'Your second argument — or acknowledge the opposing view and counter it.',
...
export default function WritingLesson1Page() {
  ...
  function buildEssayFromPlan() {
    const intro = `Technology has become deeply embedded in modern life, and there is growing debate about whether it is eroding our capacity for independent thought. ${plan.thesis}`
    ...
    <main style={{ minHeight: '100vh', background: '#f8fafc', padding: '40px 24px' }}>
      <div style={{ maxWidth: '720px', margin: '0 auto' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, #064e3b, #059669)', borderRadius: '16px', padding: '32px', margin: '20px 0', color: 'white' }}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.15)', borderRadius: '10px', padding: '16px', marginBottom: '0' }}>
        ...
        <div style={{ background: 'white', borderRadius: '16px', padding: '28px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: '20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '12px' }}>
            {[
              { label: 'Task Achievement', color: '#2563eb', desc: 'Did you answer the question fully? Is your position clear and well-supported?' },
              { label: 'Coherence & Cohesion', color: '#059669', desc: 'Is your essay logically organised? Do paragraphs flow? Are linking words used naturally?' },
              { label: 'Lexical Resource', color: '#7c3aed', desc: 'Do you use a range of vocabulary accurately? Do you avoid repeating the same words?' },
              { label: 'Grammatical Range', color: '#d97706', desc: 'Do you use a variety of sentence structures? Are there few grammar errors?' },
            ].map(c => (
          ...
          <p style={{ fontSize: '12px', color: '#94a3b8', marginTop: '12px', marginBottom: 0 }}>Each criterion is marked equally. Your overall band is the average of the four scores.</p>
        </div>
    ...
    <main style={{ minHeight: '100vh', background: '#f8fafc', padding: '40px 24px' }}>
      <div style={{ maxWidth: '720px', margin: '0 auto' }}>
        ...
        <div style={{ background: 'white', borderRadius: '16px', padding: '28px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: '20px' }}>
          ...
          <p style={{ fontSize: '13px', color: '#64748b', marginBottom: '24px' }}>Good essays start with a good plan. Fill in your key ideas before writing — this takes 5 minutes and saves you marks.</p>
          ...
          {Object.entries(PLAN_PROMPTS).map(([key, prompt]) => (
            <div key={key} style={{ marginBottom: '20px' }}>
              <label style={{ display: 'block', fontSize: '13px', fontWeight: 'bold', color: '#374151', marginBottom: '6px' }}>
                {key === 'thesis' ? '📌 Your Thesis (Introduction)' :
                  key === 'body1Topic' ? '💡 Body 1 — Main Argument' :
                  key === 'body1Example' ? '📖 Body 1 — Example or Evidence' :
                  key === 'body2Topic' ? '💡 Body 2 — Second Argument' :
                  key === 'body2Example' ? '📖 Body 2 — Example or Evidence' :
                  '🔚 Conclusion — Restate & Summarise'}
              </label>
              ...
              <textarea
                ...
                rows={2}
                placeholder={
                  key === 'thesis' ? 'e.g. I firmly believe that while technology offers valuable tools, it ultimately encourages passive consumption rather than critical engagement.' :
                  key === 'body1Topic' ? 'e.g. Technology reduces the need for mental effort by providing instant answers.' :
                  key === 'body1Example' ? 'e.g. Search engines like Google mean people rarely memorise information or work through problems independently.' :
                  key === 'body2Topic' ? 'e.g. Some argue technology enhances thinking, but evidence suggests it creates dependency.' :
                  key === 'body2Example' ? 'e.g. Studies show people who rely on GPS struggle to navigate without it, suggesting skill atrophy.' :
                  'e.g. In conclusion, I maintain that technology, despite its convenience, poses a genuine threat to independent thinking by making cognitive effort unnecessary.'
                }
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          <button
            ...
            style={{ flex: 1, padding: '14px', background: '#f1f5f9', color: '#555', border: 'none', borderRadius: '10px', fontSize: '14px', fontWeight: '600', cursor: 'pointer' }}
          >
            Skip Plan — Write Directly
          </button>
          <button
            onClick={buildEssayFromPlan}
            disabled={!planComplete}
            style={{ flex: 2, padding: '14px', background: planComplete ? 'linear-gradient(135deg, #064e3b, #059669)' : '#94a3b8', color: 'white', border: 'none', borderRadius: '10px', fontSize: '14px', fontWeight: 'bold', cursor: planComplete ? 'pointer' : 'not-allowed' }}
          >
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 320px', gap: '20px', alignItems: 'start' }}>
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '10px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: '6px', alignItems: 'center', background: timerRunning ? '#fee2e2' : '#f1f5f9', borderRadius: '8px', padding: '6px 12px' }}>
                ...
                <button onClick={() => setTimerRunning(!timerRunning)} style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '4px', border: '1px solid #e2e8f0', background: 'white', cursor: 'pointer', color: '#555' }}>
          ...
          {showTeacherMode && (
            <div style={{ background: '#064e3b', borderRadius: '12px', padding: '20px 24px', marginBottom: '16px', color: 'white' }}>
              ...
              <div style={{ fontSize: '13px', lineHeight: 1.7, color: 'rgba(255,255,255,0.85)' }}>
                ...
                <p style={{ margin: '0 0 6px' }}><strong>Before writing:</strong> Discuss the topic as a class — what does "thinking for yourself" mean? Give 2–3 examples of technology that helps or hinders this.</p>
                <p style={{ margin: '0 0 6px' }}><strong>Peer review:</strong> After submitting, have students swap essays and check each other for linking words and paragraph structure.</p>
                <p style={{ margin: 0 }}><strong>Extension:</strong> After AI feedback, ask students to rewrite one body paragraph incorporating the vocabulary upgrades.</p>
              </div>
            </div>
          )}
          ...
          <textarea
            ...
            placeholder="Write your essay here. Start with your introduction — paraphrase the question and state your position clearly..."
            rows={20}
            style={{ width: '100%', padding: '16px', borderRadius: '12px', border: '2px solid #e2e8f0', fontSize: '15px', lineHeight: 1.8, resize: 'vertical', boxSizing: 'border-box', fontFamily: 'Georgia, serif', outline: 'none', color: '#1a1a2e' }}
          />
          ...
          <button
            onClick={getFeedback}
            disabled={loadingFeedback || wordCount < 200}
            style={{ width: '100%', padding: '14px', background: loadingFeedback || wordCount < 200 ? '#94a3b8' : 'linear-gradient(135deg, #059669, #10b981)', color: 'white', border: 'none', borderRadius: '12px', fontSize: '15px', fontWeight: 'bold', cursor: loadingFeedback || wordCount < 200 ? 'not-allowed' : 'pointer' }}
          >
        ...
        <div style={{ position: 'sticky', top: '24px' }}>
          ...
          <div style={{ background: 'white', borderRadius: '14px', padding: '16px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginTop: '12px' }}>
            ...
            {[
              { label: 'Intro', hint: 'Paraphrase + thesis' },
              { label: 'Body 1', hint: 'Main argument + example' },
              { label: 'Body 2', hint: 'Second point + counter' },
              { label: 'Conclusion', hint: 'Restate + summarise' },
            ].map(s => (
    ...
    <main style={{ minHeight: '100vh', background: '#f8fafc', padding: '24px' }}>
      <div style={{ maxWidth: '820px', margin: '0 auto' }}>
          ...
          {showParagraphFeedback && (
            <div style={{ marginTop: '20px' }}>
              {[
                ...
                { label: 'Body Paragraph 1', key: 'body1', color: '#059669' },
                { label: 'Body Paragraph 2', key: 'body2', color: '#7c3aed' },
        ...
        <div style={{ background: 'white', borderRadius: '16px', padding: '28px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: '20px' }}>
          ...
          {modelView === '6' && (
            <div>
              <p style={{ fontSize: '12px', color: '#854d0e', margin: '0 0 10px', fontWeight: '600' }}>Adequate development, simple vocabulary, some linking words, basic grammar — passes but lacks range and depth.</p>
          ...
          {modelView === '7' && (
            <div>
              <p style={{ fontSize: '12px', color: '#166534', margin: '0 0 10px', fontWeight: '600' }}>Well-developed arguments, good vocabulary range, varied grammar structures, natural use of linking devices.</p>
        ...
        <div style={{ background: '#fefce8', borderRadius: '16px', padding: '28px', border: '1px solid #fef08a', marginBottom: '20px' }}>
          ...
          <p style={{ fontSize: '13px', color: '#713f12', marginBottom: '20px', lineHeight: 1.5 }}>The hardest paragraphs to write well. See how AI rewrites them at Band 7–8 using your ideas.</p>
        ...
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'space-between' }}>
          ...
          <Link href="/ielts/writing/education-success" style={{ padding: '12px 24px', borderRadius: '8px', background: 'linear-gradient(135deg, #064e3b, #059669)', color: 'white', fontSize: '14px', fontWeight: 'bold', textDecoration: 'none' }}>Next Lesson →</Link>
        </div>
