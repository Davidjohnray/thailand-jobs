...
const CUE_CARD = {
  title: 'Describe a meal or food experience you really enjoyed.',
  bullets: [
    'what the food was',
    'where you ate it',
    'who you were with',
...
export default function SpeakingPart2Page() {
  ...
  useEffect(() => {
    if (timerMode === 'prep') {
      ...
      timerRef.current = setInterval(() => {
        setTimeLeft(prev => {
          if (prev <= 1) { clearInterval(timerRef.current!); setTimerMode('speaking'); startRecording(); return 0 }
          return prev - 1
        })
      }, 1000)
  ...
  function startExamMode() {
    setAnswer(''); setFeedback(null); setRewrittenAnswer(null); setTimerMode('prep')
  }
  ...
  function manualStopSpeaking() {
    if (timerRef.current) clearInterval(timerRef.current)
    stopRecording(); setTimerMode('done')
  }
  ...
  async function getFeedback() {
    ...
    try {
      const res = await fetch('/api/ielts/speaking-feedback', {
        ...
        body: JSON.stringify({ question: CUE_CARD.title, answer, topic: TOPIC, part: 2, bullets: CUE_CARD.bullets })
      })
  ...
  async function getRewrite() {
    ...
    try {
      const res = await fetch('/api/ielts/speaking-feedback', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ question: CUE_CARD.title, answer, topic: TOPIC, mode: 'rewrite', part: 2 })
      })
  ...
  function resetAll() {
    if (timerRef.current) clearInterval(timerRef.current)
    if (recognitionRef.current) recognitionRef.current.stop()
    setIsRecording(false); setTimerMode('idle'); setAnswer(''); setFeedback(null); setRewrittenAnswer(null)
  }
  ...
  if (showIntro) return (
    <main style={{ minHeight: '100vh', background: '#f8fafc', padding: '40px 24px' }}>
      <div style={{ maxWidth: '680px', margin: '0 auto' }}>
        ...
        <div style={{ background: '#fffbeb', border: '2px dashed #f59e0b', borderRadius: '12px', padding: '24px', marginBottom: '20px' }}>
          ...
          <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
            {CUE_CARD.bullets.map((b, i) => (
              <li key={i} style={{ fontSize: '14px', color: '#374151', padding: '4px 0', display: 'flex', gap: '8px' }}>
                <span style={{ color: '#f59e0b', fontWeight: 'bold' }}>•</span> {b}
              </li>
            ))}
          </ul>
        </div>
        <div style={{ background: '#eff6ff', borderRadius: '12px', padding: '20px', marginBottom: '16px', border: '1px solid #bfdbfe' }}>
          <p style={{ fontSize: '14px', color: '#1e40af', margin: '0 0 8px', fontWeight: 'bold' }}>What you get on this task:</p>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
            {['🎯 Estimated band score','✅ What you did well','📈 How to improve','📝 Band 6 & 7–8 model answers','💬 Vocabulary upgrades','❓ Rounding-off question','✍️ AI rewrite of your answer','⏱️ Full exam timer (1min prep + 2min talk)'].map(item => (
      ...
      <div style={{ maxWidth: '720px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
          <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
            ...
            <Link href="/ielts/speaking/street-food/part-1" style={{ color: '#94a3b8', fontSize: '13px', textDecoration: 'none' }}>Part 1</Link>
            <span style={{ fontSize: '13px', fontWeight: 'bold', color: '#1e3a5f', borderBottom: '2px solid #2563eb', paddingBottom: '2px' }}>Part 2</span>
            <Link href="/ielts/speaking/street-food/part-3" style={{ color: '#94a3b8', fontSize: '13px', textDecoration: 'none' }}>Part 3</Link>
          </div>
          {hasAccess && (
        ...
        {showTeacherMode && (
          <div style={{ background: '#1e3a5f', borderRadius: '12px', padding: '20px 24px', marginBottom: '20px', color: 'white' }}>
            ...
            <div style={{ fontSize: '13px', lineHeight: 1.7, color: 'rgba(255,255,255,0.85)' }}>
              ...
              <p style={{ margin: '0 0 6px' }}><strong>Exam mode:</strong> Use the full timer — students often underestimate how long 2 minutes feels. Let them experience the pressure.</p>
              <p style={{ margin: '0 0 6px' }}><strong>Classroom discussion:</strong> Compare Band 6 vs Band 7–8 model answers. Ask students to identify the specific vocabulary and grammar differences.</p>
        ...
        <div style={{ background: '#fffbeb', border: '2px dashed #f59e0b', borderRadius: '16px', padding: '28px', marginBottom: '20px' }}>
          ...
          <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
            {CUE_CARD.bullets.map((b, i) => (
              <li key={i} style={{ fontSize: '15px', color: '#374151', padding: '6px 0', display: 'flex', gap: '10px', borderBottom: i < CUE_CARD.bullets.length - 1 ? '1px solid #fde68a' : 'none' }}>
                <span style={{ color: '#f59e0b', fontWeight: 'bold', flexShrink: 0 }}>•</span> {b}
              </li>
            ))}
          </ul>
        </div>
        ...
        {timerMode === 'speaking' && (
          <div style={{ background: '#fee2e2', border: '2px solid #ef4444', borderRadius: '12px', padding: '20px', marginBottom: '16px', textAlign: 'center' }}>
            ...
            <button onClick={manualStopSpeaking} style={{ marginTop: '8px', padding: '8px 20px', borderRadius: '8px', border: 'none', background: '#ef4444', color: 'white', fontSize: '13px', fontWeight: 'bold', cursor: 'pointer' }}>⏹ Finish Early</button>
          </div>
        )}
        {timerMode === 'done' && (
        ...
        <div style={{ background: 'white', borderRadius: '16px', padding: '24px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px', flexWrap: 'wrap', gap: '8px' }}>
            ...
            {wordCount > 0 && (
              <span style={{ fontSize: '12px', color: wordCount >= 100 ? '#059669' : wordCount >= 60 ? '#d97706' : '#ef4444', fontWeight: '600' }}>
                ~{speakingDuration}s spoken · {wordCount} words {wordCount >= 100 ? '✓ Good length' : wordCount >= 60 ? '— Try to say more' : '— Too short'}
              </span>
            )}
          </div>
          {timerMode === 'idle' && (
            <div style={{ display: 'flex', gap: '10px', marginBottom: '14px', flexWrap: 'wrap' }}>
              ...
              <button onClick={startExamMode} style={{ padding: '10px 20px', borderRadius: '8px', border: '2px solid #f59e0b', background: 'white', color: '#d97706', fontSize: '13px', fontWeight: 'bold', cursor: 'pointer' }}>
                ⏱️ Exam Mode
              </button>
              {isRecording && <span style={{ fontSize: '13px', color: '#ef4444', fontWeight: '600', alignSelf: 'center' }}>● Recording...</span>}
            </div>
          )}
          <textarea value={answer} onChange={e => setAnswer(e.target.value)} placeholder="Your answer will appear here after speaking, or type directly. Aim for at least 100 words — that is roughly 40 seconds of natural speaking." rows={8} style={{ width: '100%', padding: '14px', borderRadius: '10px', border: '2px solid #e2e8f0', fontSize: '15px', lineHeight: 1.7, resize: 'vertical', boxSizing: 'border-box', fontFamily: 'inherit', outline: 'none', color: '#1a1a2e' }} />
        ...
        {answer.trim() && !feedback && (
          <div style={{ background: 'white', borderRadius: '12px', padding: '16px 20px', boxShadow: '0 2px 6px rgba(0,0,0,0.04)', marginBottom: '16px', border: '1px solid #e2e8f0' }}>
            ...
            {CUE_CARD.bullets.map((b, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '13px', color: '#475569', padding: '4px 0' }}>
                <span style={{ fontSize: '16px' }}>☐</span> {b}
              </div>
            ))}
            <p style={{ fontSize: '12px', color: '#94a3b8', margin: '10px 0 0' }}>Tick each point mentally before submitting for feedback.</p>
          </div>
        )}
        ...
        {feedback && (
          <div style={{ background: 'white', borderRadius: '16px', padding: '28px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: '16px' }}>
            ...
            <div style={{ background: '#f0fdf4', borderRadius: '10px', padding: '16px 18px', border: '1px solid #bbf7d0', marginBottom: '16px' }}>
              ...
              {modelBandView === '6' && (
                <div>
                  <p style={{ fontSize: '11px', color: '#854d0e', margin: '0 0 8px', fontWeight: '600' }}>Simple vocabulary, basic structure, limited development — but correct and covers the points.</p>
                  <p style={{ fontSize: '14px', color: '#92400e', margin: 0, lineHeight: 1.7, fontStyle: 'italic' }}>"{feedback.modelAnswerBand6}"</p>
                </div>
              )}
              {modelBandView === '7' && (
                <div>
                  <p style={{ fontSize: '11px', color: '#166534', margin: '0 0 8px', fontWeight: '600' }}>Wider vocabulary, varied grammar, well-developed ideas with feelings and details.</p>
            ...
            {feedback.roundingOffResponse && (
              <div style={{ background: '#eff6ff', borderRadius: '10px', padding: '16px 18px', border: '1px solid #bfdbfe', marginBottom: '16px' }}>
                ...
                <p style={{ fontSize: '12px', color: '#3b82f6', margin: 0 }}>The examiner asks 1–2 short questions after your talk. Answer briefly — this leads into Part 3.</p>
              </div>
            )}
            {/* Rewrite */}
        ...
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'space-between', marginTop: '8px' }}>
          <Link href="/ielts/speaking/street-food/part-1" style={{ padding: '12px 20px', borderRadius: '8px', border: '1px solid #e2e8f0', background: 'white', color: '#1a1a2e', fontSize: '14px', fontWeight: '600', textDecoration: 'none' }}>← Part 1</Link>
          <Link href="/ielts/speaking/street-food/part-3" style={{ padding: '12px 24px', borderRadius: '8px', background: 'linear-gradient(135deg, #059669, #10b981)', color: 'white', fontSize: '14px', fontWeight: 'bold', textDecoration: 'none' }}>Continue to Part 3 →</Link>
        </div>
