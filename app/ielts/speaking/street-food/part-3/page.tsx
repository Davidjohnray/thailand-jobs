...
const QUESTIONS = [
  {
    id: 1,
    question: 'Why do you think street food culture is so popular in many Asian countries?',
    tip: 'Give a clear opinion and support it with 2–3 reasons. Think about history, economy, culture, and lifestyle.',
    example: 'I think one of the main reasons is... Another factor is probably... and this ties in with...',
    targetBand: 'Band 7+ tip: Use phrases like "One significant factor is...", "This can largely be attributed to...", "Historically speaking..."',
  },
  {
    id: 2,
    question: 'How do you think the way people eat in public has changed over the last few decades?',
    tip: 'Compare past and present. Use language of change: "used to", "has become", "increasingly", "a growing trend".',
    example: 'In the past, people tended to... whereas nowadays... I think this shift is mainly due to...',
    targetBand: 'Band 7+ tip: Speculate about reasons using "This is likely because...", "A possible explanation is..."',
  },
  {
    id: 3,
    question: 'Some people argue that fast food chains are destroying local food culture. To what extent do you agree?',
    tip: 'Acknowledge both sides before giving your view. Show balanced thinking — do not just agree or disagree.',
    example: 'There is certainly some truth in this argument... however, I would argue that... because...',
    targetBand: 'Band 7+ tip: Use concession language — "While it is true that...", "Having said that...", "Nevertheless..."',
  },
  {
    id: 4,
    question: 'What role do you think governments should play in regulating street food hygiene and safety?',
    tip: 'Discuss responsibility, regulation, and balance. Think about public health vs economic freedom for street vendors.',
    example: 'I believe governments have a responsibility to... At the same time, it is important not to... because...',
    targetBand: 'Band 7+ tip: Use modal verbs — "should be required to", "ought to ensure", "it would be beneficial if..."',
  },
]
...
export default function SpeakingPart3Page() {
  ...
  async function getFeedback() {
    ...
    try {
      const res = await fetch('/api/ielts/speaking-feedback', {
        ...
        body: JSON.stringify({ question: QUESTIONS[currentQ].question, answer, topic: TOPIC, part: 3 })
      })
  ...
  async function getRewrite() {
    ...
    try {
      const res = await fetch('/api/ielts/speaking-feedback', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ question: QUESTIONS[currentQ].question, answer, topic: TOPIC, mode: 'rewrite', part: 3 })
      })
    ...
    <main style={{ minHeight: '100vh', background: '#f8fafc', padding: '40px 24px' }}>
      <div style={{ maxWidth: '680px', margin: '0 auto' }}>
        ...
        <div style={{ background: '#eff6ff', borderRadius: '12px', padding: '20px', marginBottom: '16px', border: '1px solid #bfdbfe' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
            {['🎯 Estimated band score','✅ What you did well','📈 How to improve','📝 Band 6 & 7–8 model answers','💬 Vocabulary upgrades','🗣️ A discussion phrase to learn','✍️ AI rewrite of your answer','⏱️ Exam mode timer'].map(item => (
      ...
      <div style={{ maxWidth: '720px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
          <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
            ...
            <Link href="/ielts/speaking/street-food/part-2" style={{ color: '#94a3b8', fontSize: '13px', textDecoration: 'none' }}>Part 2</Link>
        ...
        {showTeacherMode && (
          <div style={{ background: '#1e3a5f', borderRadius: '12px', padding: '20px 24px', marginBottom: '20px', color: 'white' }}>
            ...
            <div style={{ fontSize: '13px', lineHeight: 1.7, color: 'rgba(255,255,255,0.85)' }}>
              ...
              <p style={{ margin: '0 0 6px' }}><strong>Part 3 focus:</strong> Push students to discuss, not just answer. Encourage use of the discussion phrases shown in the intro.</p>
              <p style={{ margin: '0 0 6px' }}><strong>Class activity:</strong> After AI feedback, open the question to the class. Ask students to give opposing views to each other.</p>
              <p style={{ margin: '0 0 6px' }}><strong>Board work:</strong> Write the discussion phrase from feedback. Have students use it in a new sentence.</p>
              <p style={{ margin: 0 }}><strong>Extension:</strong> Ask students to write a 150-word essay on any Part 3 question as homework — bridges directly to Writing Task 2.</p>
            </div>
          </div>
        )}
        ...
        {feedback && (
          <div style={{ background: 'white', borderRadius: '16px', padding: '28px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: '16px' }}>
            ...
            <div style={{ background: '#f0fdf4', borderRadius: '10px', padding: '16px 18px', border: '1px solid #bbf7d0', marginBottom: '16px' }}>
              ...
              {modelBandView === '6' && (
                <div>
                  <p style={{ fontSize: '11px', color: '#854d0e', margin: '0 0 8px', fontWeight: '600' }}>Basic opinion, simple vocabulary, limited discussion — on topic but not well developed.</p>
                  <p style={{ fontSize: '14px', color: '#92400e', margin: 0, lineHeight: 1.7, fontStyle: 'italic' }}>"{feedback.modelAnswerBand6}"</p>
                </div>
              )}
              {modelBandView === '7' && (
                <div>
                  <p style={{ fontSize: '11px', color: '#166534', margin: '0 0 8px', fontWeight: '600' }}>Clear opinion, well developed with reasons, considers other views, uses academic vocabulary naturally.</p>
            ...
            {feedback.followUpQuestion && (
              <div style={{ background: '#eff6ff', borderRadius: '10px', padding: '16px 18px', border: '1px solid #bfdbfe', marginBottom: '16px' }}>
                ...
                <p style={{ fontSize: '12px', color: '#3b82f6', margin: 0 }}>Practice answering this to extend your Part 3 discussion skills.</p>
              </div>
            )}
        ...
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'space-between', marginTop: '8px' }}>
          ...
          {currentQ < QUESTIONS.length - 1 ? (
            ...
            <Link href="/ielts/speaking" style={{ padding: '12px 24px', borderRadius: '8px', background: 'linear-gradient(135deg, #059669, #10b981)', color: 'white', fontSize: '14px', fontWeight: 'bold', textDecoration: 'none' }}>
