...
export default function FilipinoJobsPage() {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      ...
      const { data: featured } = await supabase.from('jobs').select('*')
        .eq('source_logo', FILIPINO_LOGO).eq('featured', true).gt('expires_at', now)
        .order('created_at', { ascending: false })
      const { data: regular } = await supabase.from('jobs').select('*')
        .eq('source_logo', FILIPINO_LOGO).eq('featured', false).gt('expires_at', now)
        .order('created_at', { ascending: false })
  ...
  const resetFilters = () => { setLocation('All Locations'); setJobType([]); setVisaOnly(false); setSearch('') }
  ...
  return (
    <main style={{ background: '#f9f9f9', minHeight: '100vh' }}>
      <section style={{ background: '#0038A8', padding: '40px 24px', textAlign: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '16px', marginBottom: '12px' }}>
          <img src={FILIPINO_LOGO} alt="Filipino Jobs" style={{ width: '56px', height: '56px', borderRadius: '50%', border: '3px solid white', objectFit: 'cover' }} />
          <h1 style={{ color: 'white', fontSize: '32px', fontWeight: 'bold', margin: 0 }}>Filipino Teacher Jobs in Thailand</h1>
        </div>
        <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: '15px', margin: 0 }}>{filtered.length} jobs available for Filipino teachers</p>
      </section>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '32px 16px' }}>
        <div style={{ background: '#e8f0fe', borderRadius: '12px', padding: '16px 20px', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '12px' }}>
          <span style={{ fontSize: '24px' }}>🇵🇭</span>
          <p style={{ color: '#1a1a2e', fontSize: '14px', margin: 0 }}>
            These jobs are posted specifically welcoming applications from <strong>Filipino teachers</strong>. Schools in Thailand regularly hire qualified Filipino English teachers.
          </p>
        </div>
        ...
        <div style={{ display: 'flex', gap: '24px', alignItems: 'flex-start' }} className="mobile-stack">
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', gap: '12px', marginBottom: '20px' }} className="mobile-stack mobile-gap">
              <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search Filipino teacher jobs..."
                style={{ flex: 1, padding: '12px 16px', borderRadius: '8px', border: '1px solid #ddd', fontSize: '15px', outline: 'none' }} />
              <button style={{ background: '#0038A8', color: 'white', padding: '12px 24px', borderRadius: '8px', border: 'none', fontWeight: 'bold', cursor: 'pointer', fontSize: '15px', whiteSpace: 'nowrap' }}>
                Search
              </button>
            </div>
            {loading ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                {filtered.length > 0 ? filtered.map((job: any) => (
                        ...
                        <div className="job-card-inner" style={{ display: 'flex', justifyContent: 'space-between', gap: '12px' }}>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px', flexWrap: 'wrap' }}>
                              ...
                              <span style={{ background: '#e8f0fe', color: '#0038A8', fontSize: '10px', padding: '2px 7px', borderRadius: '20px', fontWeight: 'bold' }}>🇵🇭 Filipino Welcome</span>
                            </div>
                  ...
                  <div style={{ textAlign: 'center', padding: '60px', color: '#666', background: 'white', borderRadius: '12px' }}>
                    ...
                    <p style={{ marginBottom: '12px' }}>No Filipino teacher jobs match your filters</p>
                    <button onClick={resetFilters} style={{ color: '#0038A8', background: 'none', border: 'none', fontWeight: 'bold', cursor: 'pointer', fontSize: '15px' }}>
                      Clear filters →
                    </button>
                  </div>
                )}
