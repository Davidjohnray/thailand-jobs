...
export default function PremiumBuilderPage() {
  ...
  const AIButton = ({ field, text, type }: { field: string, text: string, type: string }) => (
    <button
      onClick={() => field.startsWith('exp_') ? improveExperienceWithAI(parseInt(field.replace('exp_', '')), text) : improveWithAI(field, text, type)}
          ...
          <div style={{ width: '200px', background: accentColor, padding: '32px 20px', flexShrink: 0 }}>
            ...
            {cv.skills && (
              <div style={{ marginTop: '24px' }}>
                <h3 style={{ color: 'white', fontSize: '11px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '8px' }}>Skills</h3>
                <p style={{ color: 'rgba(255,255,255,0.9)', fontSize: '11px', lineHeight: '1.6', margin: 0 }}>{cv.skills}</p>
              </div>
            )}
            {cv.languages && (
              <div style={{ marginTop: '16px' }}>
                <h3 style={{ color: 'white', fontSize: '11px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '8px' }}>Languages</h3>
                <p style={{ color: 'rgba(255,255,255,0.9)', fontSize: '11px', lineHeight: '1.6', margin: 0 }}>{cv.languages}</p>
              </div>
            )}
            {cv.hobbies && (
              <div style={{ marginTop: '16px' }}>
                <h3 style={{ color: 'white', fontSize: '11px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '8px' }}>Interests</h3>
                <p style={{ color: 'rgba(255,255,255,0.9)', fontSize: '11px', lineHeight: '1.6', margin: 0 }}>{cv.hobbies}</p>
              </div>
            )}
          </div>
          <div style={{ flex: 1, padding: '32px 28px' }}>
            {cv.summary && (
              <div style={{ marginBottom: '20px' }}>
                <h2 style={{ fontSize: '13px', fontWeight: 'bold', color: accentColor, textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '8px' }}>Profile</h2>
                ...
                {cv.experience.map((exp, i) => exp.title || exp.company ? (
                  <div key={i} style={{ marginBottom: '14px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <b style={{ fontSize: '13px', color: '#1a1a2e' }}>{exp.title}</b>
              ...
              <div style={{ marginBottom: '20px' }}>
                ...
                {cv.education.map((edu, i) => edu.degree || edu.school ? (
                  <div key={i} style={{ marginBottom: '8px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <b style={{ fontSize: '13px', color: '#1a1a2e' }}>{edu.degree}</b>
                      <span style={{ fontSize: '11px', color: '#888' }}>{edu.dates}</span>
                    </div>
  ...
  return (
    <main style={{ background: '#f9f9f9', minHeight: '100vh' }}>
      ...
      <div className="no-print" style={{ background: 'white', padding: '12px 24px', borderBottom: '1px solid #eee', display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <span style={{ fontSize: '12px', color: '#999' }}>⬛ = sidebar layout</span>
      </div>
            ...
            <div className="no-print" style={{ flex: 1, minWidth: '300px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div style={{ background: 'white', borderRadius: '12px', padding: '24px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
                ...
                <textarea value={cv.summary} onChange={e => updateField('summary', e.target.value)}
                  placeholder="Write a short summary about yourself..." rows={4}
                ...
                {cv.experience.map((exp, i) => (
                  <div key={i} style={{ marginBottom: '20px', paddingBottom: '20px', borderBottom: i < cv.experience.length - 1 ? '1px solid #eee' : 'none' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                      ...
                      <button
                        onClick={() => improveExperienceWithAI(i, exp.description)}
                        disabled={aiLoading === `exp_${i}` || !exp.description.trim()}
              ...
              <div style={{ background: 'white', borderRadius: '12px', padding: '24px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
                ...
                <textarea value={cv.skills} onChange={e => updateField('skills', e.target.value)}
                  placeholder="e.g. Classroom management, ESL teaching..." rows={3}
                  style={{ width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid #ddd', fontSize: '14px', outline: 'none', resize: 'vertical', boxSizing: 'border-box' }} />
              ...
              <div style={{ background: 'white', borderRadius: '12px', padding: '24px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
                ...
                <textarea value={cv.languages} onChange={e => updateField('languages', e.target.value)}
                  placeholder="e.g. English (Native), Thai (Conversational)..." rows={2}
                  style={{ width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid #ddd', fontSize: '14px', outline: 'none', resize: 'vertical', boxSizing: 'border-box' }} />
              </div>
              ...
              <div style={{ background: 'white', borderRadius: '12px', padding: '24px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
                ...
                <textarea value={cv.certifications} onChange={e => updateField('certifications', e.target.value)}
                  placeholder="e.g. TEFL Certificate (2023)..." rows={3}
                  style={{ width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid #ddd', fontSize: '14px', outline: 'none', resize: 'vertical', boxSizing: 'border-box' }} />
              </div>
              ...
              <div style={{ background: 'white', borderRadius: '12px', padding: '24px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
                ...
                <textarea value={cv.hobbies} onChange={e => updateField('hobbies', e.target.value)}
                  placeholder="e.g. Travelling, Photography, Yoga..." rows={2}
                  style={{ width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid #ddd', fontSize: '14px', outline: 'none', resize: 'vertical', boxSizing: 'border-box' }} />
              </div>
              ...
              <div style={{ background: 'white', borderRadius: '12px', padding: '24px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
                ...
                <textarea value={cv.references} onChange={e => updateField('references', e.target.value)}
                  placeholder="e.g. Available upon request..." rows={3}
                  style={{ width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid #ddd', fontSize: '14px', outline: 'none', resize: 'vertical', boxSizing: 'border-box' }} />
              </div>
              ...
              <div style={{ background: 'white', borderRadius: '12px', padding: '24px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  <div>
                    ...
                    <input value={coverLetter.hiringManager} onChange={e => setCoverLetter(p => ({ ...p, hiringManager: e.target.value }))}
                      placeholder="e.g. Mr. Smith" style={{ width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid #ddd', fontSize: '14px', outline: 'none', boxSizing: 'border-box' }} />
                  </div>
                  <div>
                    <label style={{ fontSize: '13px', fontWeight: 'bold', color: '#555', display: 'block', marginBottom: '4px' }}>Company / School</label>
                    <input value={coverLetter.company} onChange={e => setCoverLetter(p => ({ ...p, company: e.target.value }))}
                      placeholder="e.g. Bangkok International School" style={{ width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid #ddd', fontSize: '14px', outline: 'none', boxSizing: 'border-box' }} />
                  </div>
                  <div>
                    <label style={{ fontSize: '13px', fontWeight: 'bold', color: '#555', display: 'block', marginBottom: '4px' }}>Position</label>
                    <input value={coverLetter.position} onChange={e => setCoverLetter(p => ({ ...p, position: e.target.value }))}
                      placeholder="e.g. English Teacher" style={{ width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid #ddd', fontSize: '14px', outline: 'none', boxSizing: 'border-box' }} />
                  </div>
                  <div>
                    <label style={{ fontSize: '13px', fontWeight: 'bold', color: '#555', display: 'block', marginBottom: '4px' }}>Cover Letter Body</label>
                    <textarea value={coverLetter.body} onChange={e => setCoverLetter(p => ({ ...p, body: e.target.value }))}
                      placeholder="Write your cover letter here..." rows={10}
                      style={{ width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid #ddd', fontSize: '14px', outline: 'none', resize: 'vertical', boxSizing: 'border-box' }} />
