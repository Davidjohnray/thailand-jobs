      ...
      if (form.add_to_directory) {
        ...
        if (!existing) {
          ...
          await supabase.from('teachers').insert([{
            ...
            photo_url: photoUrl,
            video_url: form.video_url.trim() || null,  // ← carries video to directory profile
            slug: teacherSlug,
            status: 'pending',
            active: false,
            template: 'modern',
              ...
              <>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '16px', marginBottom: '16px' }}>
                  <div>
                    ...
                    <input value={form.full_name} onChange={e => updateForm('full_name', e.target.value)} placeholder="John Smith" style={inputStyle} />
                  </div>
                  <div>
                    <label style={labelStyle}>Email Address *</label>
                    <input value={form.email} onChange={e => updateForm('email', e.target.value)} placeholder="john@email.com" type="email" style={inputStyle} />
                  </div>
                  <div>
                    <label style={labelStyle}>Phone Number</label>
                    <input value={form.phone} onChange={e => updateForm('phone', e.target.value)} placeholder="+66 12 345 6789" style={inputStyle} />
                  </div>
                  <div>
                    <label style={labelStyle}>Nationality</label>
                    <input value={form.nationality} onChange={e => updateForm('nationality', e.target.value)} placeholder="e.g. British" style={inputStyle} />
                  </div>
                  <div>
                    <label style={labelStyle}>Current Location</label>
                    <input value={form.location} onChange={e => updateForm('location', e.target.value)} placeholder="e.g. Bangkok, Thailand" style={inputStyle} />
                  </div>
                  <div>
                ...
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <input value={form.video_url} onChange={e => updateForm('video_url', e.target.value)}
                    placeholder="Paste your Google Drive / YouTube / Loom link here..." style={inputStyle} />
                </div>

                <div style={{ marginBottom: '24px' }}>
                  <label style={labelStyle}>Upload CV (PDF only) *</label>
                  <div onClick={() => fileRef.current?.click()}
                    style={{ border: '2px dashed', borderColor: cvFile ? '#22c55e' : '#ddd', borderRadius: '10px', padding: '28px', textAlign: 'center', cursor: 'pointer', background: cvFile ? '#f0fdf4' : '#fafafa' }}>
                ...
                <div style={{ background: '#f8f4ff', border: '2px solid #7C3AED', borderRadius: '12px', padding: '24px', marginBottom: '24px' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', marginBottom: '16px' }}>
                    ...
                    <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', flexShrink: 0 }}>
                      <input type="checkbox" checked={form.add_to_directory} onChange={e => updateForm('add_to_directory', e.target.checked)}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                      <div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
                          {photoPreview ? (
                            <img src={photoPreview} alt="Preview" style={{ width: '72px', height: '72px', borderRadius: '50%', objectFit: 'cover', border: '3px solid #7C3AED', flexShrink: 0 }} />
                          ) : (
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
                        <div>
                          ...
                          <input value={form.hourly_rate} onChange={e => updateForm('hourly_rate', e.target.value)} placeholder="e.g. 500 THB/hr or negotiable" style={inputStyle} />
                        </div>
                        <div>
                      ...
                      <div style={{ background: '#ede9fe', borderRadius: '8px', padding: '12px 14px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <span style={{ fontSize: '16px' }}>🎥</span>
                        <p style={{ color: '#7C3AED', fontSize: '13px', margin: 0 }}>
                          <strong>Your video intro will also appear on your directory profile</strong> — locked behind recruiter access so only paying schools can watch it.
                        </p>
                      </div>
                    </div>
                  )}
