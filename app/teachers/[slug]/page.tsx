...
// Reusable locked video block — shown in all templates
  ...
  return (
    <div style={{ background: '#1a1a2e', borderRadius: '12px', padding: '20px', marginTop: '16px' }}>
      ...
      <div style={{ background: '#f9f9f9', borderRadius: '8px', padding: '20px', textAlign: 'center', border: '2px dashed #ddd' }}>
        ...
        <a href="/teachers"
          style={{ display: 'inline-block', background: '#E85D26', color: 'white', padding: '8px 20px', borderRadius: '6px', textDecoration: 'none', fontWeight: 'bold', fontSize: '13px' }}>
      ...
      {teacher.subjects?.length > 0 && (
        <div style={{ background: '#1a1a2e', padding: '16px 24px' }}>
          <div style={{ maxWidth: '900px', margin: '0 auto', display: 'flex', gap: '10px', flexWrap: 'wrap', justifyContent: 'center' }}>
            {teacher.subjects.map((s: string) => <span key={s} style={{ color: 'white', fontSize: '14px', padding: '6px 16px', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.3)' }}>{s}</span>)}
          </div>
        </div>
      )}
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '40px 24px', display: 'flex', gap: '32px', flexWrap: 'wrap', alignItems: 'flex-start' }}>
        <div style={{ flex: 1, minWidth: '300px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          {teacher.levels?.length > 0 && (
            <div style={{ background: 'white', borderRadius: '16px', padding: '28px', boxShadow: '0 4px 16px rgba(0,0,0,0.08)', borderTop: '4px solid #16a34a' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
                {teacher.levels.map((l: string) => <span key={l} style={{ background: '#e8f5e9', color: '#2e7d32', padding: '8px 16px', borderRadius: '20px', fontWeight: 'bold', fontSize: '14px' }}>{l}</span>)}
              </div>
            </div>
          )}
          {(teacher.qualifications?.length > 0 || teacher.certifications?.length > 0) && (
            <div style={{ background: 'white', borderRadius: '16px', padding: '28px', boxShadow: '0 4px 16px rgba(0,0,0,0.08)', borderTop: '4px solid #2D6BE4' }}>
              ...
              {teacher.qualifications?.map((q: string) => <div key={q} style={{ display: 'flex', alignItems: 'center', gap: '10px', color: '#444', fontSize: '14px', marginBottom: '8px' }}><span style={{ color: '#2D6BE4', fontWeight: 'bold' }}>✓</span> {q}</div>)}
              {teacher.certifications?.map((c: string) => <div key={c} style={{ display: 'flex', alignItems: 'center', gap: '10px', color: '#444', fontSize: '14px', marginBottom: '8px' }}><span style={{ color: '#E85D26', fontWeight: 'bold' }}>🏅</span> {c}</div>)}
            </div>
          )}
        </div>
        <div style={{ width: '280px', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div style={{ background: 'linear-gradient(135deg, #E85D26 0%, #ff8c42 100%)', borderRadius: '16px', padding: '28px', boxShadow: '0 4px 16px rgba(232,93,38,0.3)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {teacher.line_id && <a href={`https://line.me/ti/p/~${teacher.line_id}`} target="_blank" rel="noopener noreferrer" style={{ display: 'block', background: '#06C755', color: 'white', padding: '12px', borderRadius: '8px', textDecoration: 'none', fontWeight: 'bold', fontSize: '14px', textAlign: 'center' }}>💬 Contact on LINE</a>}
              {teacher.whatsapp && <a href={`https://wa.me/${teacher.whatsapp.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer" style={{ display: 'block', background: '#25D366', color: 'white', padding: '12px', borderRadius: '8px', textDecoration: 'none', fontWeight: 'bold', fontSize: '14px', textAlign: 'center' }}>📱 WhatsApp</a>}
              {teacher.email && <a href={`mailto:${teacher.email}`} style={{ display: 'block', background: 'white', color: '#E85D26', padding: '12px', borderRadius: '8px', textDecoration: 'none', fontWeight: 'bold', fontSize: '14px', textAlign: 'center' }}>📧 Send Email</a>}
              {teacher.facebook && <a href={teacher.facebook} target="_blank" rel="noopener noreferrer" style={{ display: 'block', background: '#1877F2', color: 'white', padding: '12px', borderRadius: '8px', textDecoration: 'none', fontWeight: 'bold', fontSize: '14px', textAlign: 'center' }}>👍 Facebook</a>}
            </div>
          </div>
          {/* LOCKED VIDEO */}
          <VideoBlock videoUrl={teacher.video_url} />
          {teacher.languages?.length > 0 && (
            <div style={{ background: 'white', borderRadius: '12px', padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {teacher.languages.map((l: string) => <span key={l} style={{ background: '#fff3ed', color: '#E85D26', padding: '6px 12px', borderRadius: '20px', fontSize: '13px', fontWeight: 'bold' }}>{l}</span>)}
              </div>
            </div>
          )}
      ...
      {teacher.subjects?.length > 0 && (
        <div style={{ background: '#E85D26', padding: '12px 24px' }}>
          <div style={{ maxWidth: '1000px', margin: '0 auto', display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            {teacher.subjects.map((s: string) => <span key={s} style={{ background: 'rgba(255,255,255,0.2)', color: 'white', padding: '4px 12px', borderRadius: '4px', fontSize: '13px', fontWeight: 'bold' }}>{s}</span>)}
          </div>
        </div>
      )}
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '40px 24px', display: 'flex', gap: '32px', flexWrap: 'wrap', alignItems: 'flex-start' }}>
        <div style={{ flex: 1, minWidth: '300px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
          {teacher.bio && (
            <div style={{ background: 'white', borderRadius: '8px', padding: '28px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
              <h2 style={{ fontSize: '18px', fontWeight: 'bold', color: '#1a1a2e', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <span style={{ background: '#E85D26', color: 'white', width: '4px', height: '20px', borderRadius: '2px', display: 'inline-block' }}></span>Professional Profile
              </h2>
              <p style={{ color: '#555', lineHeight: '1.8', fontSize: '15px', whiteSpace: 'pre-line' }}>{teacher.bio}</p>
            </div>
          )}
          {teacher.teaching_style && (
            <div style={{ background: 'white', borderRadius: '8px', padding: '28px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
              <h2 style={{ fontSize: '18px', fontWeight: 'bold', color: '#1a1a2e', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <span style={{ background: '#E85D26', color: 'white', width: '4px', height: '20px', borderRadius: '2px', display: 'inline-block' }}></span>Teaching Methodology
              </h2>
              <p style={{ color: '#555', lineHeight: '1.8', fontSize: '15px', whiteSpace: 'pre-line' }}>{teacher.teaching_style}</p>
            </div>
          )}
          {teacher.levels?.length > 0 && (
            <div style={{ background: 'white', borderRadius: '8px', padding: '28px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
              <h2 style={{ fontSize: '18px', fontWeight: 'bold', color: '#1a1a2e', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <span style={{ background: '#E85D26', color: 'white', width: '4px', height: '20px', borderRadius: '2px', display: 'inline-block' }}></span>Student Levels
              </h2>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {teacher.levels.map((l: string) => <span key={l} style={{ background: '#f5f5f5', color: '#333', padding: '6px 14px', borderRadius: '4px', fontSize: '13px', fontWeight: 'bold', border: '1px solid #ddd' }}>{l}</span>)}
              </div>
            </div>
          )}
          {(teacher.qualifications?.length > 0 || teacher.certifications?.length > 0) && (
            <div style={{ background: 'white', borderRadius: '8px', padding: '28px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
              <h2 style={{ fontSize: '18px', fontWeight: 'bold', color: '#1a1a2e', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <span style={{ background: '#E85D26', color: 'white', width: '4px', height: '20px', borderRadius: '2px', display: 'inline-block' }}></span>Education & Certifications
              </h2>
              {teacher.qualifications?.map((q: string) => <div key={q} style={{ display: 'flex', alignItems: 'center', gap: '12px', color: '#444', fontSize: '14px', marginBottom: '10px', paddingBottom: '10px', borderBottom: '1px solid #f0f0f0' }}><span style={{ color: '#1a1a2e', fontSize: '18px' }}>🎓</span> {q}</div>)}
              {teacher.certifications?.map((c: string) => <div key={c} style={{ display: 'flex', alignItems: 'center', gap: '12px', color: '#444', fontSize: '14px', marginBottom: '10px', paddingBottom: '10px', borderBottom: '1px solid #f0f0f0' }}><span style={{ color: '#E85D26', fontSize: '18px' }}>🏅</span> {c}</div>)}
            </div>
          )}
        </div>
        <div style={{ width: '280px', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div style={{ background: '#1a1a2e', borderRadius: '8px', padding: '28px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {teacher.line_id && <a href={`https://line.me/ti/p/~${teacher.line_id}`} target="_blank" rel="noopener noreferrer" style={{ display: 'block', background: '#06C755', color: 'white', padding: '12px', borderRadius: '6px', textDecoration: 'none', fontWeight: 'bold', fontSize: '14px', textAlign: 'center' }}>💬 LINE</a>}
              {teacher.whatsapp && <a href={`https://wa.me/${teacher.whatsapp.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer" style={{ display: 'block', background: '#25D366', color: 'white', padding: '12px', borderRadius: '6px', textDecoration: 'none', fontWeight: 'bold', fontSize: '14px', textAlign: 'center' }}>📱 WhatsApp</a>}
              {teacher.email && <a href={`mailto:${teacher.email}`} style={{ display: 'block', background: '#E85D26', color: 'white', padding: '12px', borderRadius: '6px', textDecoration: 'none', fontWeight: 'bold', fontSize: '14px', textAlign: 'center' }}>📧 Email</a>}
              {teacher.facebook && <a href={teacher.facebook} target="_blank" rel="noopener noreferrer" style={{ display: 'block', background: '#1877F2', color: 'white', padding: '12px', borderRadius: '6px', textDecoration: 'none', fontWeight: 'bold', fontSize: '14px', textAlign: 'center' }}>👍 Facebook</a>}
            </div>
          </div>
          {/* LOCKED VIDEO */}
          <VideoBlock videoUrl={teacher.video_url} />
          {teacher.languages?.length > 0 && (
            <div style={{ background: 'white', borderRadius: '8px', padding: '20px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {teacher.languages.map((l: string) => <span key={l} style={{ background: '#f5f5f5', color: '#333', padding: '6px 12px', borderRadius: '4px', fontSize: '13px', border: '1px solid #ddd' }}>{l}</span>)}
              </div>
            </div>
          )}
      ...
      {teacher.subjects?.length > 0 && (
        <div style={{ padding: '24px', textAlign: 'center', background: 'white', borderBottom: '1px solid #eee' }}>
          <div style={{ maxWidth: '800px', margin: '0 auto' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', justifyContent: 'center' }}>
              {teacher.subjects.map((s: string) => <span key={s} style={{ background: '#fff3ed', color: '#E85D26', padding: '8px 20px', borderRadius: '30px', fontWeight: 'bold', fontSize: '14px', border: '2px solid #E85D26' }}>{s}</span>)}
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          {teacher.levels?.length > 0 && (
            <div style={{ background: 'white', borderRadius: '20px', padding: '28px', boxShadow: '0 4px 16px rgba(0,0,0,0.06)', border: '2px solid #e8f0fe' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
                {teacher.levels.map((l: string) => <span key={l} style={{ background: '#e8f0fe', color: '#2D6BE4', padding: '8px 20px', borderRadius: '30px', fontWeight: 'bold', fontSize: '14px' }}>{l}</span>)}
              </div>
            </div>
          )}
          {(teacher.qualifications?.length > 0 || teacher.certifications?.length > 0) && (
            <div style={{ background: 'white', borderRadius: '20px', padding: '28px', boxShadow: '0 4px 16px rgba(0,0,0,0.06)', border: '2px solid #fef9c3' }}>
              ...
              {teacher.qualifications?.map((q: string) => <div key={q} style={{ display: 'flex', alignItems: 'center', gap: '10px', color: '#444', fontSize: '14px', marginBottom: '10px' }}><span>🎓</span> {q}</div>)}
              {teacher.certifications?.map((c: string) => <div key={c} style={{ display: 'flex', alignItems: 'center', gap: '10px', color: '#444', fontSize: '14px', marginBottom: '10px' }}><span>✅</span> {c}</div>)}
            </div>
          )}

          {/* LOCKED VIDEO — friendly template inline */}
          {teacher.video_url && (
            <div style={{ background: 'white', borderRadius: '20px', padding: '28px', boxShadow: '0 4px 16px rgba(0,0,0,0.06)', border: '2px solid #e9d5ff' }}>
              <h2 style={{ fontSize: '22px', fontWeight: 'bold', color: '#7C3AED', marginBottom: '16px' }}>🎥 Introduction Video</h2>
              <div style={{ background: '#f8f4ff', borderRadius: '12px', padding: '24px', textAlign: 'center' }}>
                <div style={{ fontSize: '40px', marginBottom: '12px' }}>🔒</div>
                <p style={{ color: '#7C3AED', fontSize: '14px', fontWeight: 'bold', margin: '0 0 4px' }}>Video intro available</p>
                <p style={{ color: '#888', fontSize: '13px', margin: '0 0 16px' }}>Only available to recruiters with paid access</p>
                <a href="/teachers" style={{ display: 'inline-block', background: '#7C3AED', color: 'white', padding: '10px 24px', borderRadius: '20px', textDecoration: 'none', fontWeight: 'bold', fontSize: '13px' }}>
                  Get Recruiter Access →
                </a>
              </div>
            </div>
