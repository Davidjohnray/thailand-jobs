...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: teacher } = await supabase.from('teachers').select('*').eq('slug', slug).single()
...
export default async function TeacherPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const { data: teacher } = await supabase.from('teachers').select('*').eq('slug', slug).eq('active', true).single()
  if (!teacher) notFound()
    ...
    <main style={{ background: '#f9f9f9', minHeight: '100vh' }}>
      <div style={{ background: 'linear-gradient(135deg, #1a1a2e 0%, #2d1b69 100%)', padding: '60px 24px' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto', display: 'flex', gap: '40px', alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ width: '160px', height: '160px', borderRadius: '50%', overflow: 'hidden', border: '4px solid #E85D26', flexShrink: 0, background: '#333', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {teacher.photo_url ? <img src={teacher.photo_url} alt={teacher.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: '64px' }}>👤</span>}
          </div>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '40px 24px', display: 'flex', gap: '32px', flexWrap: 'wrap', alignItems: 'flex-start' }}>
        <div style={{ flex: 1, minWidth: '300px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
          {teacher.bio && <div style={{ background: 'white', borderRadius: '12px', padding: '28px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}><h2 style={{ fontSize: '20px', fontWeight: 'bold', color: '#1a1a2e', marginBottom: '16px', borderBottom: '3px solid #E85D26', paddingBottom: '10px' }}>👋 About Me</h2><p style={{ color: '#555', lineHeight: '1.8', fontSize: '15px', whiteSpace: 'pre-line' }}>{teacher.bio}</p></div>}
          {teacher.subjects?.length > 0 && <div style={{ background: 'white', borderRadius: '12px', padding: '28px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}><h2 style={{ fontSize: '20px', fontWeight: 'bold', color: '#1a1a2e', marginBottom: '16px', borderBottom: '3px solid #E85D26', paddingBottom: '10px' }}>📚 Subjects</h2><div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>{teacher.subjects.map((s: string) => <span key={s} style={{ background: '#fff3ed', color: '#E85D26', padding: '8px 16px', borderRadius: '20px', fontWeight: 'bold', fontSize: '14px' }}>{s}</span>)}</div></div>}
          {teacher.levels?.length > 0 && <div style={{ background: 'white', borderRadius: '12px', padding: '28px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}><h2 style={{ fontSize: '20px', fontWeight: 'bold', color: '#1a1a2e', marginBottom: '16px', borderBottom: '3px solid #E85D26', paddingBottom: '10px' }}>🎯 Teaching Levels</h2><div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>{teacher.levels.map((l: string) => <span key={l} style={{ background: '#e8f5e9', color: '#2e7d32', padding: '8px 16px', borderRadius: '20px', fontWeight: 'bold', fontSize: '14px' }}>{l}</span>)}</div></div>}
          {teacher.teaching_style && <div style={{ background: 'white', borderRadius: '12px', padding: '28px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}><h2 style={{ fontSize: '20px', fontWeight: 'bold', color: '#1a1a2e', marginBottom: '16px', borderBottom: '3px solid #E85D26', paddingBottom: '10px' }}>💡 My Teaching Style</h2><p style={{ color: '#555', lineHeight: '1.8', fontSize: '15px', whiteSpace: 'pre-line' }}>{teacher.teaching_style}</p></div>}
          {teacher.qualifications?.length > 0 && <div style={{ background: 'white', borderRadius: '12px', padding: '28px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}><h2 style={{ fontSize: '20px', fontWeight: 'bold', color: '#1a1a2e', marginBottom: '16px', borderBottom: '3px solid #E85D26', paddingBottom: '10px' }}>🎓 Qualifications</h2><div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>{teacher.qualifications.map((q: string) => <div key={q} style={{ display: 'flex', alignItems: 'center', gap: '10px', color: '#444', fontSize: '14px' }}><span style={{ color: '#E85D26', fontWeight: 'bold', fontSize: '16px' }}>✓</span> {q}</div>)}</div></div>}
          {teacher.certifications?.length > 0 && <div style={{ background: 'white', borderRadius: '12px', padding: '28px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}><h2 style={{ fontSize: '20px', fontWeight: 'bold', color: '#1a1a2e', marginBottom: '16px', borderBottom: '3px solid #E85D26', paddingBottom: '10px' }}>📜 Certifications</h2><div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>{teacher.certifications.map((c: string) => <span key={c} style={{ background: '#e8f0fe', color: '#2D6BE4', padding: '8px 16px', borderRadius: '20px', fontWeight: 'bold', fontSize: '14px' }}>🏅 {c}</span>)}</div></div>}
        </div>
        <div style={{ width: '280px', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div style={{ background: '#1a1a2e', borderRadius: '12px', padding: '28px', boxShadow: '0 4px 16px rgba(0,0,0,0.15)' }}>
            ...
            {teacher.hourly_rate && <div style={{ background: '#E85D26', borderRadius: '8px', padding: '14px', textAlign: 'center', marginBottom: '16px' }}><div style={{ color: 'rgba(255,255,255,0.8)', fontSize: '12px', marginBottom: '4px' }}>HOURLY RATE</div><div style={{ color: 'white', fontSize: '22px', fontWeight: 'bold' }}>{teacher.hourly_rate}</div></div>}
            <ContactBlock teacher={teacher} variant="dark" />
          </div>
          <VideoBlock videoUrl={teacher.video_url} />
          {teacher.languages?.length > 0 && <div style={{ background: 'white', borderRadius: '12px', padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}><h3 style={{ fontSize: '16px', fontWeight: 'bold', color: '#1a1a2e', marginBottom: '12px' }}>🗣️ Languages Spoken</h3><div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>{teacher.languages.map((l: string) => <span key={l} style={{ background: '#f0f0f0', color: '#555', padding: '6px 12px', borderRadius: '20px', fontSize: '13px' }}>{l}</span>)}</div></div>}
          <div style={{ background: '#fff3ed', borderRadius: '12px', padding: '20px', border: '1px solid #E85D26' }}>
            ...
            <Link href="/teachers" style={{ display: 'block', background: '#E85D26', color: 'white', padding: '8px', borderRadius: '6px', textDecoration: 'none', fontSize: '13px', fontWeight: 'bold', textAlign: 'center' }}>Browse All Teachers →</Link>
          </div>
        </div>
      </div>
    </main>
  ...
  if (template === 'bold') return (
    <main style={{ background: '#f9f9f9', minHeight: '100vh' }}>
      ...
      {teacher.subjects?.length > 0 && <div style={{ background: '#1a1a2e', padding: '16px 24px' }}><div style={{ maxWidth: '900px', margin: '0 auto', display: 'flex', gap: '10px', flexWrap: 'wrap', justifyContent: 'center' }}>{teacher.subjects.map((s: string) => <span key={s} style={{ color: 'white', fontSize: '14px', padding: '6px 16px', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.3)' }}>{s}</span>)}</div></div>}
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '40px 24px', display: 'flex', gap: '32px', flexWrap: 'wrap', alignItems: 'flex-start' }}>
        <div style={{ flex: 1, minWidth: '300px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
          {teacher.bio && <div style={{ background: 'white', borderRadius: '16px', padding: '28px', boxShadow: '0 4px 16px rgba(0,0,0,0.08)', borderTop: '4px solid #E85D26' }}><h2 style={{ fontSize: '22px', fontWeight: 'bold', color: '#E85D26', marginBottom: '16px' }}>👋 About Me</h2><p style={{ color: '#555', lineHeight: '1.8', fontSize: '15px', whiteSpace: 'pre-line' }}>{teacher.bio}</p></div>}
          {teacher.teaching_style && <div style={{ background: 'white', borderRadius: '16px', padding: '28px', boxShadow: '0 4px 16px rgba(0,0,0,0.08)', borderTop: '4px solid #7C3AED' }}><h2 style={{ fontSize: '22px', fontWeight: 'bold', color: '#7C3AED', marginBottom: '16px' }}>💡 My Teaching Style</h2><p style={{ color: '#555', lineHeight: '1.8', fontSize: '15px', whiteSpace: 'pre-line' }}>{teacher.teaching_style}</p></div>}
          {teacher.levels?.length > 0 && <div style={{ background: 'white', borderRadius: '16px', padding: '28px', boxShadow: '0 4px 16px rgba(0,0,0,0.08)', borderTop: '4px solid #16a34a' }}><h2 style={{ fontSize: '22px', fontWeight: 'bold', color: '#16a34a', marginBottom: '16px' }}>🎯 Who I Teach</h2><div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>{teacher.levels.map((l: string) => <span key={l} style={{ background: '#e8f5e9', color: '#2e7d32', padding: '8px 16px', borderRadius: '20px', fontWeight: 'bold', fontSize: '14px' }}>{l}</span>)}</div></div>}
          {(teacher.qualifications?.length > 0 || teacher.certifications?.length > 0) && <div style={{ background: 'white', borderRadius: '16px', padding: '28px', boxShadow: '0 4px 16px rgba(0,0,0,0.08)', borderTop: '4px solid #2D6BE4' }}><h2 style={{ fontSize: '22px', fontWeight: 'bold', color: '#2D6BE4', marginBottom: '16px' }}>🎓 Credentials</h2>{teacher.qualifications?.map((q: string) => <div key={q} style={{ display: 'flex', alignItems: 'center', gap: '10px', color: '#444', fontSize: '14px', marginBottom: '8px' }}><span style={{ color: '#2D6BE4', fontWeight: 'bold' }}>✓</span> {q}</div>)}{teacher.certifications?.map((c: string) => <div key={c} style={{ display: 'flex', alignItems: 'center', gap: '10px', color: '#444', fontSize: '14px', marginBottom: '8px' }}><span style={{ color: '#E85D26', fontWeight: 'bold' }}>🏅</span> {c}</div>)}</div>}
        </div>
        <div style={{ width: '280px', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div style={{ background: 'linear-gradient(135deg, #E85D26 0%, #ff8c42 100%)', borderRadius: '16px', padding: '28px', boxShadow: '0 4px 16px rgba(232,93,38,0.3)' }}>
            ...
            {teacher.hourly_rate && <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: '8px', padding: '14px', textAlign: 'center', marginBottom: '16px' }}><div style={{ color: 'rgba(255,255,255,0.8)', fontSize: '12px', marginBottom: '4px' }}>HOURLY RATE</div><div style={{ color: 'white', fontSize: '24px', fontWeight: 'bold' }}>{teacher.hourly_rate}</div></div>}
            <ContactBlock teacher={teacher} variant="orange" />
          </div>
          <VideoBlock videoUrl={teacher.video_url} />
          {teacher.languages?.length > 0 && <div style={{ background: 'white', borderRadius: '12px', padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}><h3 style={{ fontSize: '16px', fontWeight: 'bold', color: '#1a1a2e', marginBottom: '12px' }}>🗣️ Languages</h3><div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>{teacher.languages.map((l: string) => <span key={l} style={{ background: '#fff3ed', color: '#E85D26', padding: '6px 12px', borderRadius: '20px', fontSize: '13px', fontWeight: 'bold' }}>{l}</span>)}</div></div>}
          <Link href="/teachers" style={{ display: 'block', background: '#1a1a2e', color: 'white', padding: '12px', borderRadius: '8px', textDecoration: 'none', fontSize: '14px', fontWeight: 'bold', textAlign: 'center' }}>← Browse All Teachers</Link>
        </div>
      </div>
    </main>
  )
    ...
    <main style={{ background: '#f0f2f5', minHeight: '100vh' }}>
      <div style={{ background: '#1a1a2e', padding: '0 24px' }}>
        <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '50px 0', display: 'flex', gap: '40px', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {teacher.hourly_rate && <div style={{ background: '#E85D26', borderRadius: '8px', padding: '20px 28px', textAlign: 'center', flexShrink: 0 }}><div style={{ color: 'rgba(255,255,255,0.8)', fontSize: '11px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '4px' }}>Hourly Rate</div><div style={{ color: 'white', fontSize: '24px', fontWeight: 'bold' }}>{teacher.hourly_rate}</div></div>}
        </div>
      </div>
      {teacher.subjects?.length > 0 && <div style={{ background: '#E85D26', padding: '12px 24px' }}><div style={{ maxWidth: '1000px', margin: '0 auto', display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}><span style={{ color: 'rgba(255,255,255,0.8)', fontSize: '12px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px', marginRight: '8px' }}>Subjects:</span>{teacher.subjects.map((s: string) => <span key={s} style={{ background: 'rgba(255,255,255,0.2)', color: 'white', padding: '4px 12px', borderRadius: '4px', fontSize: '13px', fontWeight: 'bold' }}>{s}</span>)}</div></div>}
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '40px 24px', display: 'flex', gap: '32px', flexWrap: 'wrap', alignItems: 'flex-start' }}>
        <div style={{ flex: 1, minWidth: '300px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
          {teacher.bio && <div style={{ background: 'white', borderRadius: '8px', padding: '28px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}><h2 style={{ fontSize: '18px', fontWeight: 'bold', color: '#1a1a2e', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '10px' }}><span style={{ background: '#E85D26', color: 'white', width: '4px', height: '20px', borderRadius: '2px', display: 'inline-block' }}></span>Professional Profile</h2><p style={{ color: '#555', lineHeight: '1.8', fontSize: '15px', whiteSpace: 'pre-line' }}>{teacher.bio}</p></div>}
          {teacher.teaching_style && <div style={{ background: 'white', borderRadius: '8px', padding: '28px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}><h2 style={{ fontSize: '18px', fontWeight: 'bold', color: '#1a1a2e', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '10px' }}><span style={{ background: '#E85D26', color: 'white', width: '4px', height: '20px', borderRadius: '2px', display: 'inline-block' }}></span>Teaching Methodology</h2><p style={{ color: '#555', lineHeight: '1.8', fontSize: '15px', whiteSpace: 'pre-line' }}>{teacher.teaching_style}</p></div>}
          {teacher.levels?.length > 0 && <div style={{ background: 'white', borderRadius: '8px', padding: '28px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}><h2 style={{ fontSize: '18px', fontWeight: 'bold', color: '#1a1a2e', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '10px' }}><span style={{ background: '#E85D26', color: 'white', width: '4px', height: '20px', borderRadius: '2px', display: 'inline-block' }}></span>Student Levels</h2><div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>{teacher.levels.map((l: string) => <span key={l} style={{ background: '#f5f5f5', color: '#333', padding: '6px 14px', borderRadius: '4px', fontSize: '13px', fontWeight: 'bold', border: '1px solid #ddd' }}>{l}</span>)}</div></div>}
          {(teacher.qualifications?.length > 0 || teacher.certifications?.length > 0) && <div style={{ background: 'white', borderRadius: '8px', padding: '28px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}><h2 style={{ fontSize: '18px', fontWeight: 'bold', color: '#1a1a2e', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '10px' }}><span style={{ background: '#E85D26', color: 'white', width: '4px', height: '20px', borderRadius: '2px', display: 'inline-block' }}></span>Education & Certifications</h2>{teacher.qualifications?.map((q: string) => <div key={q} style={{ display: 'flex', alignItems: 'center', gap: '12px', color: '#444', fontSize: '14px', marginBottom: '10px', paddingBottom: '10px', borderBottom: '1px solid #f0f0f0' }}><span style={{ color: '#1a1a2e', fontSize: '18px' }}>🎓</span> {q}</div>)}{teacher.certifications?.map((c: string) => <div key={c} style={{ display: 'flex', alignItems: 'center', gap: '12px', color: '#444', fontSize: '14px', marginBottom: '10px', paddingBottom: '10px', borderBottom: '1px solid #f0f0f0' }}><span style={{ color: '#E85D26', fontSize: '18px' }}>🏅</span> {c}</div>)}</div>}
        </div>
        <div style={{ width: '280px', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div style={{ background: '#1a1a2e', borderRadius: '8px', padding: '28px' }}>
            ...
            <ContactBlock teacher={teacher} variant="professional" />
          </div>
          <VideoBlock videoUrl={teacher.video_url} />
          {teacher.languages?.length > 0 && <div style={{ background: 'white', borderRadius: '8px', padding: '20px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}><h3 style={{ fontSize: '14px', fontWeight: 'bold', color: '#1a1a2e', marginBottom: '12px', textTransform: 'uppercase', letterSpacing: '1px' }}>Languages</h3><div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>{teacher.languages.map((l: string) => <span key={l} style={{ background: '#f5f5f5', color: '#333', padding: '6px 12px', borderRadius: '4px', fontSize: '13px', border: '1px solid #ddd' }}>{l}</span>)}</div></div>}
          <Link href="/teachers" style={{ display: 'block', background: 'white', color: '#1a1a2e', padding: '12px', borderRadius: '8px', textDecoration: 'none', fontSize: '14px', fontWeight: 'bold', textAlign: 'center', border: '1px solid #ddd' }}>← Browse All Teachers</Link>
        </div>
      </div>
    </main>
  )
    ...
    <main style={{ background: '#fffbf7', minHeight: '100vh' }}>
      <div style={{ background: 'linear-gradient(135deg, #fff3ed 0%, #ffe4cc 100%)', padding: '50px 24px', borderBottom: '3px solid #E85D26' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
          ...
          {teacher.hourly_rate && <div style={{ display: 'inline-block', background: '#E85D26', color: 'white', padding: '12px 32px', borderRadius: '30px', fontSize: '18px', fontWeight: 'bold', boxShadow: '0 4px 12px rgba(232,93,38,0.4)' }}>💰 {teacher.hourly_rate} per hour</div>}
        </div>
      </div>
      {teacher.subjects?.length > 0 && <div style={{ padding: '24px', textAlign: 'center', background: 'white', borderBottom: '1px solid #eee' }}><div style={{ maxWidth: '800px', margin: '0 auto' }}><p style={{ color: '#888', fontSize: '13px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '12px' }}>I teach</p><div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', justifyContent: 'center' }}>{teacher.subjects.map((s: string) => <span key={s} style={{ background: '#fff3ed', color: '#E85D26', padding: '8px 20px', borderRadius: '30px', fontWeight: 'bold', fontSize: '14px', border: '2px solid #E85D26' }}>{s}</span>)}</div></div></div>}
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '40px 24px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          {teacher.bio && <div style={{ background: 'white', borderRadius: '20px', padding: '28px', boxShadow: '0 4px 16px rgba(0,0,0,0.06)', border: '2px solid #fff3ed' }}><h2 style={{ fontSize: '22px', fontWeight: 'bold', color: '#E85D26', marginBottom: '16px' }}>🙋 A Bit About Me</h2><p style={{ color: '#555', lineHeight: '1.9', fontSize: '15px', whiteSpace: 'pre-line' }}>{teacher.bio}</p></div>}
          ...
          <div style={{ background: 'linear-gradient(135deg, #1a1a2e 0%, #2d1b69 100%)', borderRadius: '20px', padding: '32px', textAlign: 'center' }}>
            ...
            <ContactBlock teacher={teacher} variant="friendly" />
            {teacher.languages?.length > 0 && <div style={{ marginTop: '20px', color: '#ccc', fontSize: '13px' }}>🗣️ I speak: {teacher.languages.join(', ')}</div>}
          </div>
          <div style={{ textAlign: 'center' }}>
            <Link href="/teachers" style={{ color: '#E85D26', textDecoration: 'none', fontSize: '14px', fontWeight: 'bold' }}>← Browse All Teachers</Link>
          </div>
        </div>
      </div>
    </main>
