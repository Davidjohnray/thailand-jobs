...
export default function Ages56Page() {
  ...
  useEffect(() => {
    supabase.from('lesson_plans').select('*').eq('is_active', true).eq('age_group', '4-5')
  ...
  const contactMessage = `Hi, I would like to purchase the following ${selected.length} lesson plan(s) for Ages 5-6:\n\n${selectedTitles.map((t, i) => `${i + 1}. ${t}`).join('\n')}\n\nTotal: ${selected.length * 10}฿\n\nPlease send me the payment details. Thank you!`
  ...
  return (
    <main style={{ fontFamily: 'sans-serif', background: '#f8f9fa', minHeight: '100vh', paddingBottom: selected.length > 0 ? '100px' : '0' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #7C3AED, #9f67f5)', padding: '52px 24px', color: 'white' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '20px', marginTop: '20px' }}>
            <div style={{ fontSize: '48px', background: 'rgba(255,255,255,0.2)', borderRadius: '12px', padding: '8px 16px' }}>K</div>
            <div>
              <h1 style={{ fontSize: '36px', fontWeight: 'bold', margin: '0 0 6px' }}>Ages 5-6</h1>
              <p style={{ opacity: 0.9, fontSize: '16px', margin: '0 0 4px' }}>K1-K2 Level</p>
              <p style={{ opacity: 0.8, fontSize: '14px', margin: 0 }}>Vocabulary, guided practice, and speaking activities.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '40px 24px' }}>
        ...
        {!search && (
          <>
            <div style={{ background: 'linear-gradient(135deg, #fdf4ff, #f3eeff)', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '20px 24px', marginBottom: '20px' }}>
...
function PlanCard({ plan, selectable, selected, onToggle, accent }: any) {
  const cardContent = (
    <div style={{ background: 'white', borderRadius: '12px', overflow: 'hidden', boxShadow: selected ? `0 0 0 3px ${accent}` : '0 2px 8px rgba(0,0,0,0.08)', cursor: selectable ? 'pointer' : 'default', position: 'relative', transition: 'box-shadow 0.2s' }}>
      ...
      <div style={{ height: '130px', background: 'linear-gradient(135deg, #fdf4ff, #f3eeff)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '48px', position: 'relative' }}>
