    ...
    <main style={{ fontFamily: 'sans-serif', background: '#f8f9fa', minHeight: '100vh', paddingBottom: selected.length > 0 ? '100px' : '0' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #f97316, #fb923c)', padding: '52px 24px', color: 'white' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '20px', marginTop: '20px' }}>
            <span style={{ fontSize: '64px' }}>U</span>
            <div>
      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '40px 24px' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, #fff3ed, #f3eeff)', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '20px 24px', marginBottom: '20px' }}>
          <div style={{ fontWeight: 'bold', color: '#1a1a2e', fontSize: '15px', marginBottom: '10px' }}>Premium Lesson Plans — Tick the ones you want and we calculate the best price!</div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '18px', flexWrap: 'wrap', gap: '10px' }}>
            ...
            {selected.length > 0 && (
              <button onClick={() => setSelected([])}
                style={{ background: '#ffeaea', color: '#c62828', border: 'none', padding: '6px 14px', borderRadius: '8px', cursor: 'pointer', fontSize: '13px' }}>
                Clear selection
              </button>
            )}
          </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '16px' }}>
              {paidPlans.map((plan: any) => (
                <PlanCard
                  ...
                  plan={plan}
                  selectable={true}
                  selected={selected.includes(plan.slug)}
                  onToggle={() => toggleSelect(plan.slug)}
                />
              ))}
            </div>
          )}
        ...
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: '#1a1a2e', padding: '16px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px', zIndex: 999, boxShadow: '0 -4px 20px rgba(0,0,0,0.2)' }}>
          <div>
            ...
            <div style={{ color: '#FBBF24', fontSize: '14px' }}>
              Total: <strong>${price} USD</strong> / <strong>{thb} THB</strong>
            </div>
          </div>
          <Link
            href={`/contact?message=${encodeURIComponent(contactMessage)}`}
...
function PlanCard({ plan, selectable, selected, onToggle }: { plan: any, selectable: boolean, selected: boolean, onToggle: () => void }) {
  return (
    <div
      ...
      style={{ background: 'white', borderRadius: '12px', overflow: 'hidden', boxShadow: selected ? '0 0 0 3px #f97316' : '0 2px 8px rgba(0,0,0,0.08)', cursor: selectable ? 'pointer' : 'default', position: 'relative', transition: 'box-shadow 0.2s' }}>
