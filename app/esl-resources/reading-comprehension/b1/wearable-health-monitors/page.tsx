...
const PARTS = [
  { number: 1, title: 'What Are Wearable Health Monitors?', emoji: '⌚', color: '#0ea5e9',
    ...
    questions: [{ n: 1, q: 'Have you seen or used a wearable health monitor before?' }, { n: 2, q: 'Why do you think these devices are popular?' }, { n: 3, q: 'Do you think everyone needs a health monitor? Why or why not?' }]
  },
  { number: 2, title: 'How Do They Help People?', emoji: '📊', color: '#22c55e',
    ...
    questions: [{ n: 4, q: 'Do you like seeing information about your daily habits?' }, { n: 5, q: 'Can technology help people make better health choices?' }, { n: 6, q: 'What habit would you like to improve?' }]
  },
  { number: 3, title: 'Motivation and Daily Life', emoji: '🎯', color: '#f59e0b',
    ...
    questions: [{ n: 7, q: 'Do goals motivate you or make you feel stressed?' }, { n: 8, q: 'Would you like to compete with friends using a health device?' }, { n: 9, q: 'What kind of reminder would help you most?' }]
  },
  { number: 4, title: 'Problems and Future Use', emoji: '🔋', color: '#8b5cf6',
    ...
    questions: [{ n: 10, q: 'Would wearing a device all day bother you? Why or why not?' }, { n: 11, q: 'How important is privacy when using technology?' }, { n: 12, q: 'Do you think wearable health monitors will be more popular in the future?' }]
  },
]
    ...
    try {
      if (translationLang === 'none') {
        const res = await fetch('/api/chat', {
          ...
          body: JSON.stringify({
            system: isPhrase
              ? `You are an English dictionary for B1 learners. Respond with ONLY one simple sentence (max 20 words) explaining what the phrase means. Easy vocabulary. No extra text.`
              : `You are an English dictionary for B1 learners. Respond with ONLY one simple sentence (max 15 words) defining this word. Easy vocabulary. No extra text.`,
