...
const VOCAB = [
  { word: 'Classroom', meaning: 'The room where students learn and study', emoji: '🏫' },
  { word: 'Teacher', meaning: 'The person who teaches students in school', emoji: '👩‍🏫' },
  { word: 'Subject', meaning: 'A topic you study at school — like Maths or English', emoji: '📚' },
  { word: 'Lesson', meaning: 'One class or period of learning — usually one hour', emoji: '⏰' },
  { word: 'Homework', meaning: 'School work you do at home after school', emoji: '📝' },
  { word: 'Student', meaning: 'A person who goes to school and learns', emoji: '🧑‍🎓' },
]

const QUESTIONS = [
  { q: 'How does Sara get to school?', options: ['She takes the bus', 'She walks', 'She drives'], answer: 1, emoji: '🚶' },
  { q: 'How many students are in Sara\'s class?', options: ['20 students', '25 students', '30 students'], answer: 1, emoji: '👩‍🏫' },
  { q: 'What is Sara\'s teacher\'s name?', options: ['Mr Kim', 'Mr Chan', 'Mr Lee'], answer: 2, emoji: '👨‍🏫' },
  { q: 'What is Sara\'s favourite subject?', options: ['Maths', 'English', 'Science'], answer: 1, emoji: '📚' },
  { q: 'When does Sara do her homework?', options: ['Before school', 'At lunchtime', 'After school'], answer: 2, emoji: '📝' },
]
...
My teacher is Mr Lee.
He is very kind and funny.
I like him very much.
We have six lessons every day.
My favourite subject is English.
I like to read and write in English.
I also like science.
After school, I do my homework.
Then I watch TV and eat dinner.
I love my school.`

const LISTENING_SCRIPT = `Sara walks to school every day. There are 25 students in her class. Her teacher is Mr Lee. He is kind and funny. Sara's favourite subject is English. She does her homework after school.`
...
export default function MySchoolPage() {
  ...
  const [writing, setWriting] = useState({ subject: '', students: '', after: '' })
          ...
          <div style={{ display: 'flex', gap: '24px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '12px' }}>
                ...
                <span style={{ background: 'rgba(255,255,255,0.25)', color: 'white', fontSize: '12px', fontWeight: 'bold', padding: '4px 12px', borderRadius: '20px' }}>School</span>
                <span style={{ background: 'rgba(255,255,255,0.15)', color: 'white', fontSize: '12px', fontWeight: '600', padding: '4px 12px', borderRadius: '20px' }}>Lesson 5</span>
              </div>
              <h1 style={{ color: 'white', fontSize: '32px', fontWeight: 'bold', margin: '0 0 8px' }}>My School</h1>
              <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: '16px', margin: 0, lineHeight: '1.6' }}>Learn to talk about school — your teacher, your class and your favourite subjects.</p>
            </div>
          </div>
          <div style={{ display: 'flex', gap: '20px', marginTop: '24px', flexWrap: 'wrap' }}>
            {[{ icon: '📖', label: 'Short reading text' }, { icon: '📚', label: '6 vocabulary words' }, { icon: '❓', label: '5 questions' }, { icon: '🌍', label: '13-language translation' }, { icon: '✍️', label: 'Writing practice' }, { icon: '⏱️', label: '30–45 min lesson' }].map(s => (
      ...
      <section style={{ background: 'white', borderBottom: '1px solid #eee', padding: '10px 24px', position: 'sticky', top: 0, zIndex: 10 }}>
        <div style={{ maxWidth: '860px', margin: '0 auto' }}>
          <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '8px' }}>
            {SECTIONS.map(s => (
              <button key={s.id} onClick={() => setSection(s.id as any)} style={{ padding: '5px 12px', borderRadius: '20px', border: 'none', background: section === s.id ? s.color : '#f0f0f0', color: section === s.id ? 'white' : '#555', fontWeight: '700', fontSize: '12px', cursor: 'pointer', transition: 'all 0.2s' }}>{s.label}</button>
            ))}
          </div>
          <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
              ...
              {SPEEDS.map(s => (
                <button key={s.value} onClick={() => setSpeed(s.value)} style={{ padding: '4px 12px', borderRadius: '20px', border: '2px solid', borderColor: speed === s.value ? '#16a34a' : '#e5e7eb', background: speed === s.value ? '#16a34a' : 'white', color: speed === s.value ? 'white' : '#555', fontWeight: '700', fontSize: '12px', cursor: 'pointer' }}>{s.label}</button>
              ))}
            </div>
      ...
      <div style={{ maxWidth: '860px', margin: '0 auto', padding: '32px 24px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        {section === 'warmup' && (
          <div style={{ background: 'white', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.07)' }}>
            ...
            <div style={{ padding: '24px 28px' }}>
              <p style={{ color: '#555', fontSize: '15px', marginBottom: '20px' }}>Ask your student these questions. Short answers are fine.</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {[
                  { q: 'Do you go to school?', hint: 'e.g. Yes, I do. / No, I don\'t.' },
                  { q: 'What is your favourite subject?', hint: 'e.g. My favourite subject is English / Maths.' },
                  { q: 'Do you have a lot of homework?', hint: 'e.g. Yes, I do. / Not much.' }
                ].map((item, i) => (
                    ...
                    <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap', marginBottom: '4px' }}>
                          ...
                          <TranslateBtn text={item.q} type="question" lang={translationLang} color="#f59e0b" onTranslated={(t) => setWarmupTranslations(prev => ({ ...prev, [i]: t }))} />
                        </div>
          ...
          <div style={{ background: 'white', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.07)' }}>
            <div style={{ background: 'linear-gradient(135deg, #3b82f622, #3b82f608)', borderLeft: '5px solid #3b82f6', padding: '20px 24px', display: 'flex', gap: '14px', alignItems: 'center', flexWrap: 'wrap' }}>
              <div style={{ flex: 1 }}>
                ...
                <h2 style={{ color: '#1a1a2e', fontSize: '20px', fontWeight: 'bold', margin: 0 }}>📖 My School</h2>
              </div>
                    ...
                    <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px', flexWrap: 'wrap' }}>
                          ...
                          <TranslateBtn text={`${v.word}: ${v.meaning}`} type="vocab" lang={translationLang} color="#8b5cf6" onTranslated={(t) => setVocabTranslations(prev => ({ ...prev, [v.word]: t }))} />
                        </div>
                        ...
                        {vocabTranslations[v.word] && (
                          <div style={{ marginTop: '8px', background: '#fdf4ff', border: '1px solid #e9d5ff', borderRadius: '8px', padding: '8px 10px' }}>
                            <span style={{ color: '#7c3aed', fontSize: '10px', fontWeight: '800', textTransform: 'uppercase', letterSpacing: '1px', display: 'block', marginBottom: '2px' }}>{currentLang?.label}</span>
                            <span style={{ color: '#374151', fontSize: '14px' }}>{vocabTranslations[v.word]}</span>
                          </div>
                        )}
                      </div>
                    </div>
            ...
            <div style={{ padding: '24px 28px' }}>
                ...
                {QUESTIONS.map((q, qi) => (
                  <div key={qi} style={{ background: '#f9fafb', borderRadius: '16px', padding: '20px', border: submitted && answers[qi] === q.answer ? '2px solid #22c55e' : submitted && answers[qi] !== q.answer ? '2px solid #ef4444' : '2px solid #e5e7eb' }}>
                    <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-start', marginBottom: '14px' }}>
                      ...
                      <div style={{ fontWeight: '700', fontSize: '16px', color: '#1a1a2e' }}><span style={{ color: '#E85D26', marginRight: '6px' }}>{qi + 1}.</span>{q.q}</div>
                    </div>
                      ...
                      {q.options.map((opt, oi) => {
                        ...
                        return (
                          <button key={oi} onClick={() => handleAnswer(qi, oi)} disabled={submitted}
                            style={{ background: bg, border: `2px solid ${border}`, borderRadius: '10px', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: '12px', cursor: submitted ? 'default' : 'pointer', textAlign: 'left' }}>
              ...
              {!submitted ? (
                <button onClick={() => setSubmitted(true)} disabled={!allAnswered}
                  style={{ width: '100%', background: allAnswered ? 'linear-gradient(135deg, #E85D26, #f97316)' : '#e5e7eb', color: allAnswered ? 'white' : '#9ca3af', border: 'none', padding: '14px', borderRadius: '12px', fontWeight: '800', fontSize: '16px', cursor: allAnswered ? 'pointer' : 'not-allowed' }}>
                ...
                <div>
                  <div style={{ background: score === QUESTIONS.length ? '#dcfce7' : score >= 3 ? '#fffbeb' : '#fee2e2', borderRadius: '16px', padding: '20px', textAlign: 'center', marginBottom: '16px', border: `2px solid ${score === QUESTIONS.length ? '#16a34a' : score >= 3 ? '#f59e0b' : '#ef4444'}` }}>
                    ...
                    <div style={{ color: '#555', fontSize: '15px' }}>{score === QUESTIONS.length ? 'Perfect — well done!' : score >= 3 ? 'Good work!' : 'Keep trying — read again!'}</div>
                  </div>
          ...
          <div style={{ background: 'white', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.07)' }}>
            ...
            <div style={{ padding: '24px 28px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', marginBottom: '20px' }}>
                {[
                  { label: 'What is Sara\'s teacher\'s name?', key: 'q1' as const, placeholder: 'e.g. Mr Lee' },
                  { label: 'What is Sara\'s favourite subject?', key: 'q2' as const, placeholder: 'e.g. English' }
                ].map((item, i) => (
          ...
          <div style={{ background: 'white', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.07)' }}>
            ...
            <div style={{ padding: '24px 28px' }}>
              <div style={{ background: '#f0fdf4', borderRadius: '14px', padding: '16px 20px', border: '2px solid #86efac', marginBottom: '20px' }}>
                ...
                <div style={{ color: '#14532d', fontSize: '16px', fontWeight: '700', fontFamily: 'Georgia, serif', lineHeight: '1.8' }}>My favourite subject is English.<br />I do my homework after school.</div>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', marginBottom: '24px' }}>
                {[
                  { q: 'What is your favourite subject at school?', hint: 'My favourite subject is _____ .', emoji: '📚' },
                  { q: 'How do you get to school?', hint: 'I _____ to school. (walk / take the bus / ride my bike)', emoji: '🚶' },
                  { q: 'Do you like your teacher? Why?', hint: 'Yes, my teacher is _____ . / No, because _____ .', emoji: '👩‍🏫' }
                ].map((item, i) => (
                    ...
                    <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap', marginBottom: '6px' }}>
                          <div style={{ fontWeight: '700', fontSize: '17px', color: '#1a1a2e' }}>{item.q}</div>
                          <TranslateBtn text={item.q} type="question" lang={translationLang} color="#22c55e" onTranslated={(t) => setSpeakingTranslations(prev => ({ ...prev, [i]: t }))} />
                        </div>
        ...
        {section === 'writing' && (
          <div style={{ background: 'white', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.07)' }}>
            ...
            <div style={{ padding: '24px 28px' }}>
              <p style={{ color: '#555', fontSize: '15px', marginBottom: '20px' }}>Complete the sentences about <strong>your own</strong> school.</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginBottom: '24px' }}>
                {[
                  { label: 'My favourite subject is', suffix: '.', key: 'subject' as const, placeholder: 'e.g. English / Maths / Science', emoji: '📚' },
                  { label: 'There are', suffix: 'students in my class.', key: 'students' as const, placeholder: 'e.g. twenty / thirty', emoji: '🧑‍🎓' },
                  { label: 'After school, I', suffix: '.', key: 'after' as const, placeholder: 'e.g. do my homework / watch TV / play sport', emoji: '⏰' }
                ].map(item => (
              ...
              <div style={{ background: '#1a1a2e', borderRadius: '16px', padding: '24px' }}>
                ...
                {['Read a short text about school ✅', 'Understand basic school vocabulary ✅', 'Answer simple comprehension questions ✅', 'Talk about your own school and favourite subjects ✅'].map((item, i) => (
                  <div key={i} style={{ color: 'rgba(255,255,255,0.85)', fontSize: '15px', display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '8px' }}>
