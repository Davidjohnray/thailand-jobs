...
export default function Primary710SciencePage() {
  ...
  useEffect(() => {
    supabase.from('lesson_plans').select('*').eq('is_active', true).eq('age_group', '7-10').eq('subject', 'science')
  ...
  return (
    <main style={{ fontFamily: 'sans-serif', background: '#f8f9fa', minHeight: '100vh', paddingBottom: selected.length > 0 ? '100px' : '0' }}>
      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '40px 24px' }}>
        ...
        {!search && (
          <>
            <div style={{ background: '#e8f5e9', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '20px 24px', marginBottom: '20px' }}>
...
function PlanCard({ plan, selectable, selected, onToggle, accent }: any) {
  const cardContent = (
    <div style={{ background: 'white', borderRadius: '12px', overflow: 'hidden', boxShadow: selected ? `0 0 0 3px ${accent}` : '0 2px 8px rgba(0,0,0,0.08)', cursor: selectable ? 'pointer' : 'default', position: 'relative', transition: 'box-shadow 0.2s' }}>
      ...
      <div style={{ height: '130px', background: '#e8f5e9', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '40px', fontWeight: 'bold', color: accent, position: 'relative' }}>
      ...
      <div style={{ padding: '14px' }}>
        {plan.theme && <span style={{ background: '#e8f5e9', color: accent, fontSize: '11px', padding: '2px 8px', borderRadius: '12px', display: 'inline-block', marginBottom: '8px' }}>{plan.theme}</span>}
