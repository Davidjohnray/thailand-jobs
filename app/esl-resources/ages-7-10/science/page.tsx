...
export default function Primary710SciencePage() {
  ...
  const contactMessage = `Hi, I would like to purchase the following ${selected.length} lesson plan(s) for Primary Science (Ages 7-10):\n\n${selectedTitles.map((t, i) => `${i + 1}. ${t}`).join('\n')}\n\nTotal: $${price} USD / ${thb} THB\n\nPlease send me the bank transfer details. Thank you!`
  ...
  return (
    <main style={{ fontFamily: 'sans-serif', background: '#f8f9fa', minHeight: '100vh', paddingBottom: selected.length > 0 ? '100px' : '0' }}>
      ...
      {/* BANNER */}
      <div style={{ background: 'linear-gradient(135deg, #16a34a, #22c55e)', padding: '52px 24px', color: 'white' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '20px', marginTop: '20px' }}>
            ...
            <div>
              <h1 style={{ fontSize: '36px', fontWeight: 'bold', margin: '0 0 6px' }}>Science</h1>
              <p style={{ opacity: 0.9, fontSize: '16px', margin: '0 0 4px' }}>Ages 7-10 · Primary Level</p>
              <p style={{ opacity: 0.8, fontSize: '14px', margin: 0 }}>Living things, materials, forces, and the environment.</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div style={{ marginBottom: '40px' }}>
          ...
          {freePlans.length === 0 ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '16px' }}>
              {freePlans.map((plan: any) => <PlanCard key={plan.id} plan={plan} selectable={false} selected={false} onToggle={() => {}} accentColor={accentColor} />)}
            </div>
          )}
        </div>

        {/* PRICING BANNER */}
        <div style={{ background: 'linear-gradient(135deg, #e8f5e9, #e0f7fa)', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '20px 24px', marginBottom: '20px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '8px' }}>
            {[
              ...
              <div key={item.label} style={{ background: 'white', borderRadius: '8px', padding: '8px 14px', border: '1px solid #e5e7eb', textAlign: 'center' }}>
                <div style={{ fontWeight: 'bold', color: accentColor, fontSize: '16px' }}>{item.usd} <span style={{ color: '#888', fontSize: '12px' }}>/ {item.thb}</span></div>
          ...
          {paidPlans.length === 0 ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '16px' }}>
              {paidPlans.map((plan: any) => (
                <PlanCard key={plan.id} plan={plan} selectable={true} selected={selected.includes(plan.slug)} onToggle={() => toggleSelect(plan.slug)} accentColor={accentColor} />
              ))}
            </div>
          )}
        </div>
      ...
      {selected.length > 0 && (
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: '#1a1a2e', padding: '16px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px', zIndex: 999, boxShadow: '0 -4px 20px rgba(0,0,0,0.2)' }}>
          ...
          <Link href={`/contact?message=${encodeURIComponent(contactMessage)}`}
            style={{ background: accentColor, color: 'white', padding: '12px 28px', borderRadius: '10px', textDecoration: 'none', fontWeight: 'bold', fontSize: '15px' }}>
            Contact Us to Order
          </Link>
        </div>
      )}
...
function PlanCard({ plan, selectable, selected, onToggle, accentColor }: any) {
  const cardContent = (
    <div style={{ background: 'white', borderRadius: '12px', overflow: 'hidden', boxShadow: selected ? `0 0 0 3px ${accentColor}` : '0 2px 8px rgba(0,0,0,0.08)', cursor: selectable ? 'pointer' : 'default', position: 'relative', transition: 'box-shadow 0.2s' }}>
      {selectable && (
      ...
      <div style={{ height: '130px', background: `linear-gradient(135deg, ${accentColor}15, ${accentColor}25)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '40px', fontWeight: 'bold', color: accentColor, position: 'relative' }}>
        {plan.preview_image_url ? <img src={plan.preview_image_url} alt={plan.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : 'S'}
      ...
      <div style={{ padding: '14px' }}>
        {plan.theme && <span style={{ background: '#e8f5e9', color: accentColor, fontSize: '11px', padding: '2px 8px', borderRadius: '12px', display: 'inline-block', marginBottom: '8px' }}>{plan.theme}</span>}
        ...
        <div style={{ marginTop: '12px', color: plan.is_free ? '#22c55e' : accentColor, fontSize: '13px', fontWeight: 'bold' }}>
