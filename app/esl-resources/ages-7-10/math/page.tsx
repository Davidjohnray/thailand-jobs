...
export default function Primary710MathPage() {
  ...
  useEffect(() => {
    supabase.from('lesson_plans').select('*').eq('is_active', true).eq('age_group', '7-10').eq('subject', 'math')
      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '40px 24px' }}>
        ...
        <div style={{ marginBottom: '28px' }}>
          <div style={{ position: 'relative', maxWidth: '420px' }}>
            ...
            <input type="text" placeholder="Search lesson plans..." value={search} onChange={e => setSearch(e.target.value)}
            ...
            {search && <button onClick={() => setSearch('')} style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', fontSize: '16px', color: '#888' }}>✕</button>}
          </div>
          {search && <p style={{ marginTop: '8px', color: '#666', fontSize: '13px' }}>{filtered.length} result{filtered.length !== 1 ? 's' : ''} for "<strong>{search}</strong>"</p>}
        </div>
        ...
        {!search && (
          <>
            <div style={{ background: '#e0f7fa', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '20px 24px', marginBottom: '20px' }}>
...
function PlanCard({ plan, selectable, selected, onToggle, accent }: any) {
  const cardContent = (
    <div style={{ background: 'white', borderRadius: '12px', overflow: 'hidden', boxShadow: selected ? `0 0 0 3px ${accent}` : '0 2px 8px rgba(0,0,0,0.08)', cursor: selectable ? 'pointer' : 'default', position: 'relative', transition: 'box-shadow 0.2s' }}>
      ...
      <div style={{ height: '130px', background: '#e0f7fa', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '40px', fontWeight: 'bold', color: accent, position: 'relative' }}>
        ...
        <div style={{ position: 'absolute', top: '10px', right: '10px' }}>
          {plan.is_free ? <span style={{ background: '#22c55e', color: 'white', fontSize: '11px', fontWeight: 'bold', padding: '3px 8px', borderRadius: '20px' }}>FREE</span>
            : <span style={{ background: accent, color: 'white', fontSize: '11px', fontWeight: 'bold', padding: '3px 8px', borderRadius: '20px' }}>10฿</span>}
        </div>
      </div>
      <div style={{ padding: '14px' }}>
        {plan.theme && <span style={{ background: '#e0f7fa', color: accent, fontSize: '11px', padding: '2px 8px', borderRadius: '12px', display: 'inline-block', marginBottom: '8px' }}>{plan.theme}</span>}
