...
export default function Primary710MathPage() {
  ...
  const contactMessage = `Hi, I would like to purchase the following ${selected.length} lesson plan(s) for Primary Math (Ages 7-10):\n\n${selectedTitles.map((t, i) => `${i + 1}. ${t}`).join('\n')}\n\nTotal: $${price} USD / ${thb} THB\n\nPlease send me the bank transfer details. Thank you!`
  ...
  return (
    <main style={{ fontFamily: 'sans-serif', background: '#f8f9fa', minHeight: '100vh', paddingBottom: selected.length > 0 ? '100px' : '0' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #0891b2, #06b6d4)', padding: '52px 24px', color: 'white' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '20px', marginTop: '20px' }}>
            <div style={{ fontSize: '48px', fontWeight: 'bold', background: 'rgba(255,255,255,0.2)', borderRadius: '12px', padding: '8px 16px' }}>M</div>
            <div>
              <h1 style={{ fontSize: '36px', fontWeight: 'bold', margin: '0 0 6px' }}>Math</h1>
              <p style={{ opacity: 0.9, fontSize: '16px', margin: '0 0 4px' }}>Ages 7-10 · Primary Level</p>
              <p style={{ opacity: 0.8, fontSize: '14px', margin: 0 }}>Numbers, operations, geometry, and problem solving.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '40px 24px' }}>
        ...
        {/* PRICING BANNER */}
        <div style={{ background: 'linear-gradient(135deg, #e0f7fa, #e8f0fe)', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '20px 24px', marginBottom: '20px' }}>
...
function PlanCard({ plan, selectable, selected, onToggle, accentColor }: any) {
  const cardContent = (
    <div style={{ background: 'white', borderRadius: '12px', overflow: 'hidden', boxShadow: selected ? `0 0 0 3px ${accentColor}` : '0 2px 8px rgba(0,0,0,0.08)', cursor: 'pointer', position: 'relative', transition: 'box-shadow 0.2s' }}>
      {selectable && (
      ...
      <div style={{ height: '130px', background: `linear-gradient(135deg, ${accentColor}15, ${accentColor}25)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '40px', fontWeight: 'bold', color: accentColor, position: 'relative' }}>
        {plan.preview_image_url ? <img src={plan.preview_image_url} alt={plan.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : 'M'}
      ...
      <div style={{ padding: '14px' }}>
        {plan.theme && <span style={{ background: '#e0f7fa', color: accentColor, fontSize: '11px', padding: '2px 8px', borderRadius: '12px', display: 'inline-block', marginBottom: '8px' }}>{plan.theme}</span>}
