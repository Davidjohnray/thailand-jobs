...
const LEVELS = [
  {
    ...
    topics: 12,
    sample: ['To be (am/is/are)', 'Articles: a, an, the', 'Simple present tense', 'Plural nouns'],
  },
  {
    code: 'a2',
    label: 'A2',
    ...
    who: 'Early learners',
    topics: 14,
    sample: ['Past simple', 'Can / can\'t', 'Comparatives & superlatives', 'Countable & uncountable'],
  },
  {
    code: 'b1',
    label: 'B1',
    ...
    emoji: '📗',
    gradient: 'linear-gradient(135deg, #2D6BE4, #4f8ef7)',
    shadow: 'rgba(45,107,228,0.3)',
    who: 'Developing learners',
    topics: 16,
    sample: ['Present perfect', 'First conditional', 'Gerunds vs infinitives', 'Used to / would'],
  },
  {
    code: 'b2',
    label: 'B2',
    title: 'Intermediate',
    emoji: '📘',
    gradient: 'linear-gradient(135deg, #7C3AED, #9f67f5)',
    shadow: 'rgba(124,58,237,0.3)',
    who: 'Confident learners',
    topics: 16,
    sample: ['Passive voice', 'Reported speech', 'Second & third conditional', 'Modal perfects'],
  },
  {
    code: 'c1',
    label: 'C1',
    ...
    topics: 14,
    sample: ['Inversion', 'Cleft sentences', 'Mixed conditionals', 'Ellipsis & substitution'],
  },
  {
    code: 'c2',
    label: 'C2',
    title: 'Advanced',
    emoji: '🏆',
    gradient: 'linear-gradient(135deg, #E85D26, #f97316)',
    shadow: 'rgba(232,93,38,0.3)',
    who: 'Near-native speakers',
    topics: 12,
    sample: ['Subjunctive mood', 'Nominalization', 'Complex aspect', 'Discourse markers'],
  },
]
...
const MODES = [
  ...
  },
  {
    ...
    points: [
      'Projector-ready explanation slides',
      'Class exercises you can run live',
      'Student answer reveal for TV mode',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
          {LEVELS.map(level => (
            <Link key={level.code} href={`/esl-resources/grammar/${level.code}`} style={{ textDecoration: 'none' }}>
              <div style={{ background: 'white', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 2px 16px rgba(0,0,0,0.07)', transition: 'transform 0.2s, box-shadow 0.2s', cursor: 'pointer' }}
                onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.transform = 'translateY(-4px)'; (e.currentTarget as HTMLDivElement).style.boxShadow = `0 8px 28px ${level.shadow}`; }}
