...
export default function SoloPage({ params }: { params: any }) {
  ...
  const restart = () => {
    ...
    setPhase(game.show_vocab_lesson ? 'vocab' : 'quiz'); setVocabIndex(0)
  }
      ...
      <main style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #0f172a, #1e3a5f)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
        <div style={{ width: '100%', maxWidth: '600px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
            ...
            <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '14px' }}>📚 Vocab · {vocabIndex + 1} / {rawVocab.length}</div>
          </div>
          <div style={{ background: 'white', borderRadius: '20px', padding: '40px', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.4)' }}>
            ...
            <div style={{ fontSize: '36px', fontWeight: '900', color: '#1a1a2e', marginBottom: '20px' }}>{word?.word || word?.question || word?.statement}</div>
          ...
          <div style={{ display: 'flex', gap: '12px', marginTop: '20px' }}>
            {vocabIndex > 0 && <button onClick={() => setVocabIndex(prev => prev - 1)} style={{ flex: 1, background: 'rgba(255,255,255,0.1)', color: 'white', border: 'none', padding: '14px', borderRadius: '12px', fontWeight: '700', fontSize: '15px', cursor: 'pointer' }}>← Previous</button>}
            {vocabIndex + 1 < rawVocab.length
              ? <button onClick={() => setVocabIndex(prev => prev + 1)} style={{ flex: 2, background: 'linear-gradient(135deg, #f59e0b, #d97706)', color: '#1a1a2e', border: 'none', padding: '14px', borderRadius: '12px', fontWeight: '900', fontSize: '16px', cursor: 'pointer' }}>Next Word →</button>
              : <button onClick={() => setPhase('quiz')} style={{ flex: 2, background: 'linear-gradient(135deg, #22c55e, #16a34a)', color: 'white', border: 'none', padding: '14px', borderRadius: '12px', fontWeight: '900', fontSize: '16px', cursor: 'pointer' }}>🎮 Start the Game! →</button>
            }
          </div>
        </div>
      </main>
    ...
    return (
      <main style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #0f172a, #1e3a5f)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
        <div style={{ width: '100%', maxWidth: '500px', background: 'white', borderRadius: '24px', padding: '48px 36px', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.4)' }}>
          ...
          <h2 style={{ fontSize: '28px', fontWeight: '900', color: '#1a1a2e', marginBottom: '8px' }}>{pct === 100 ? 'Perfect Score!' : pct >= 70 ? 'Great Job!' : pct >= 50 ? 'Good Try!' : 'Keep Practising!'}</h2>
          ...
          <div style={{ display: 'flex', gap: '6px', justifyContent: 'center', flexWrap: 'wrap', marginBottom: '32px' }}>
            {results.map((r, i) => <div key={i} style={{ width: '32px', height: '32px', borderRadius: '8px', background: r.correct ? '#dcfce7' : '#fee2e2', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px' }}>{r.correct ? '✓' : '✗'}</div>)}
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <button onClick={restart} style={{ background: 'linear-gradient(135deg, #f59e0b, #d97706)', color: '#1a1a2e', border: 'none', padding: '16px', borderRadius: '12px', fontWeight: '900', fontSize: '17px', cursor: 'pointer' }}>🔄 Play Again</button>
            <Link href={`/arcade/${slug}`} style={{ display: 'block', background: '#f3f4f6', color: '#374151', padding: '14px', borderRadius: '12px', textDecoration: 'none', fontWeight: '700', fontSize: '15px' }}>← Back to Arcade</Link>
          </div>
        </div>
      </main>
    )
    ...
    <main style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #0f172a, #1e3a5f)', display: 'flex', flexDirection: 'column', padding: '24px' }}>
      ...
      <div style={{ maxWidth: '700px', width: '100%', margin: '0 auto', flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.08)', borderRadius: '20px', padding: '28px 24px', marginBottom: '20px', textAlign: 'center', border: '2px solid rgba(255,255,255,0.1)' }}>
          {game.game_type === 'vocab_blast' && <div style={{ color: 'rgba(255,255,255,0.6)', fontSize: '14px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '2px', marginBottom: '12px' }}>What does this word mean?</div>}
          {q.imageUrl && <img src={q.imageUrl} alt="" style={{ width: '100%', maxHeight: '220px', objectFit: 'contain', borderRadius: '12px', marginBottom: '16px' }} />}
